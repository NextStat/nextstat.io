...
try:
    ...
    # nextstat is only required for the optional downstream "consumer smoke".
    nextstat = None  # type: ignore[assignment]
...
def _run_nextstat_demo(
    ...
    *,
    ...
    if nextstat is None:
        _write_json(out_dir / "note.json", {"skipped": True, "reason": "nextstat not importable in this environment"})
        return {"timings": timings, "nlme": {"ran": False}, "survival": {"ran": False}}
    ...
    return {
        "timings": timings,
        "survival": {"ran": True},
        "nlme": {"ran": True, "converged": bool(res.get("converged"))},
    }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--skip-nextstat-demo",
        action="store_true",
        help="Skip the downstream nextstat smoke demo (useful when nextstat is not installed).",
    )
    ...
    if args.sources_json is not None:
        ...
        _run([args.python, str(tools_dir / "run_internet_mocks.py"), "--backend", "heuristic", "--out-dir", str(sources_run_dir), "--quiet"])
    ...
    for backend in args.backends:
        ...
        for i in range(args.n_repeats):
            ...
            # --- Optional downstream consumer smoke demo (offline; uses the same backend instance) ---
            if args.skip_nextstat_demo:
                _write_json(demo_dir / "note.json", {"skipped": True, "reason": "--skip-nextstat-demo"})
                demo = {"timings": {}, "survival": {"ran": False}, "nlme": {"ran": False}}
            else:
