...
mod wasm_exports {
    ...
    fn validate_bayes_input(input: &BayesSampleInput) -> Result<(), JsValue> {
        if input.n_samples == 0 {
            return Err(js_err("n_samples must be >= 1"));
        }
        if input.n_warmup == 0 {
            return Err(js_err("n_warmup must be >= 1"));
        }
        ...
        {
            return Err(js_err(
                "target_accept must be a finite number in (0, 1). If your locale uses commas, enter 0.8 (dot), not 0,8.",
            ));
        }
        if input.max_treedepth == 0 || input.max_treedepth > 15 {
            return Err(js_err("max_treedepth must be in [1, 15] for browser runtime"));
        }
        Ok(())
    }
    ...
    const MAX_BAYES_SAMPLES: usize = 10_000;
    ...
    fn dispatch_sample(input: &BayesSampleInput) -> Result<BayesSampleResultJs, JsValue> {
        ...
        if input.n_samples > MAX_BAYES_SAMPLES {
            return Err(js_err(format!(
                "n_samples must be <= {} in browser, got {}",
                MAX_BAYES_SAMPLES, input.n_samples
            )));
        }
        if input.n_warmup > MAX_BAYES_SAMPLES {
            return Err(js_err(format!(
                "n_warmup must be <= {} in browser, got {}",
                MAX_BAYES_SAMPLES, input.n_warmup
            )));
        }
        ...
        macro_rules! run_sampler {
            ($model:expr, $input:expr) => {{
                ...
                if dim > MAX_BAYES_DIM {
                    return Err(js_err(format!(
                        "Model dimension {} exceeds browser limit {}",
                        dim, MAX_BAYES_DIM
                    )));
                }
