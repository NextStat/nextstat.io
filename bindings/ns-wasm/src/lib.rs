    ...
    fn dispatch_sample(input: &BayesSampleInput) -> Result<BayesSampleResultJs, JsValue> {
        ...
        let result = match input.model.as_str() {
            ...
            "glm_logistic" => {
                ...
                let model = ns_inference::LogisticRegressionModel::new(d.x, y_u8, true)
                    .map_err(|e| js_err(e.to_string()))?;
                run_sampler!(model, input)
            }
            "glm_linear" => {
                ...
                let model = ns_inference::LinearRegressionModel::new(d.x, d.y, true)
                    .map_err(|e| js_err(e.to_string()))?;
                run_sampler!(model, input)
            }
            "glm_poisson" => {
                ...
                let model = ns_inference::PoissonRegressionModel::new(d.x, y_u64, true, None)
                    .map_err(|e| js_err(e.to_string()))?;
                run_sampler!(model, input)
            }
            "std_normal" => {
