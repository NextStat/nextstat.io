...
// ---------------------------------------------------------------------------
// ns_scm — Stepwise Covariate Modeling
// ---------------------------------------------------------------------------

fn median_center(values: &[f64]) -> extendr_api::Result<f64> {
    if values.is_empty() {
        return Err(Error::Other("covariate column must be non-empty".into()));
    }
...
fn relationship_label(rel: ns_inference::CovariateRelationship) -> &'static str {
    match rel {
        ns_inference::CovariateRelationship::Power => "power",
        ns_inference::CovariateRelationship::Proportional => "proportional",
        ns_inference::CovariateRelationship::Exponential => "exponential",
    }
}

fn steps_to_r_list(steps: &[ns_inference::ScmStep]) -> List {
    List::from_values(
        steps
            .iter()
            .map(|s| {
                list!(
                    name = s.name.clone(),
                    param_index = s.param_index as i32,
                    relationship = relationship_label(s.relationship),
                    delta_ofv = s.delta_ofv,
                    p_value = s.p_value,
...
fn ns_scm(
    ...
    if n_subjects <= 0 {
        return Err(Error::Other("n_subjects must be positive".into()));
    }
    if times.len() != dv.len() || times.len() != id.len() {
        return Err(Error::Other("times/dv/id length mismatch".into()));
    }
    if theta.len() != 3 {
        return Err(Error::Other("theta must have length 3: c(CL, V, Ka)".into()));
    }
    if omega.len() != 3 {
        return Err(Error::Other("omega must have length 3: c(omega_CL, omega_V, omega_Ka)".into()));
    }
    if !sigma.is_finite() || sigma <= 0.0 {
        return Err(Error::Other("sigma must be finite and > 0".into()));
    }
    if !alpha_forward.is_finite()
        || !alpha_backward.is_finite()
        || alpha_forward <= 0.0
        || alpha_forward >= 1.0
        || alpha_backward <= 0.0
        || alpha_backward >= 1.0
    {
        return Err(Error::Other("alpha_forward/alpha_backward must be in (0, 1)".into()));
    }

    let n_sub = n_subjects as usize;
    if covariates.nrows() != n_sub {
        return Err(Error::Other(format!(
            "covariates must have one row per subject: got {}, expected {}",
            covariates.nrows(),
            n_sub
        )));
    }

    let subject_idx: Vec<usize> = id
        .iter()
        .map(|&sid| {
            if sid < 0 {
                return Err(Error::Other(format!("id must be non-negative, got {sid}")));
            }
            let u = sid as usize;
            if u >= n_sub {
                return Err(Error::Other(format!(
                    "id value {sid} out of range for n_subjects={n_subjects}"
                )));
            }
            Ok(u)
        })
        .collect::<extendr_api::Result<Vec<usize>>>()?;

    let n_cov = covariates.ncols();
    let names: Vec<String> = if covariate_names.len() == n_cov {
        covariate_names
    } else if n_cov == 0 {
        Vec::new()
    } else {
        (0..n_cov).map(|j| format!("cov{}", j + 1)).collect()
    };

    let mut candidates: Vec<ns_inference::CovariateCandidate> = Vec::new();
    for j in 0..n_cov {
        let mut values = Vec::with_capacity(n_sub);
        for i in 0..n_sub {
            let v = covariates[[i, j]];
            if !v.is_finite() {
                return Err(Error::Other(format!(
                    "covariates column '{}' contains non-finite values",
                    names[j]
                )));
            }
            values.push(v);
        }
        let center = median_center(&values)?;
        for (param_index, param_name) in [(0_usize, "CL"), (1_usize, "V"), (2_usize, "Ka")] {
            candidates.push(ns_inference::CovariateCandidate {
                name: format!("{}_on_{}", names[j], param_name),
                param_index,
                values: values.clone(),
                center,
                relationship: ns_inference::CovariateRelationship::Power,
            });
        }
    }

    let omega_init = ns_inference::OmegaMatrix::from_diagonal(&omega)
        .map_err(|e| Error::Other(format!("omega: {e}")))?;

    let mut cfg = ns_inference::ScmConfig::default();
    cfg.forward_alpha = alpha_forward;
    cfg.backward_alpha = alpha_backward;
    let scm = ns_inference::ScmEstimator::new(cfg);
    ...
    let result = scm
        .run_1cpt_oral(
            ...
            dose,
            1.0,
            ns_inference::ErrorModel::Additive(sigma),
            &theta,
            &omega_init,
            &candidates,
        )
        .map_err(|e| Error::Other(format!("SCM failed: {e}")))?;

    let omega_matrix = result.omega.to_matrix();
    let omega_dim = result.omega.dim();
    let selected_names: Vec<String> = result.selected.iter().map(|s| s.name.clone()).collect();

    Ok(list!(
        selected = steps_to_r_list(&result.selected),
        selected_names = selected_names,
        n_selected = result.selected.len() as i32,
        forward_trace = steps_to_r_list(&result.forward_trace),
        backward_trace = steps_to_r_list(&result.backward_trace),
        theta = result.theta,
        omega = RMatrix::new_matrix(omega_dim, omega_dim, |r, c| omega_matrix[r][c]),
        ofv = result.ofv,
