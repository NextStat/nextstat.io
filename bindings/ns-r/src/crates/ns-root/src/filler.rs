...
pub fn fill_histograms_with_jagged(
    ...
    for spec in specs {
        ...
        weight_vals.push(match &spec.weight {
            Some(w) => Some(eval_expr_columns_with_jagged(w, columns, jagged_columns, n_entries)?),
            None => None,
        });
        sel_vals.push(match &spec.selection {
            Some(s) => Some(eval_expr_columns_with_jagged(s, columns, jagged_columns, n_entries)?),
            None => None,
        });
    }
