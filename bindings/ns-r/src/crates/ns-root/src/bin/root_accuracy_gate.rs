...
fn main() -> Result<(), io::Error> {
    ...
    let file = RootFile::open(&cfg.root_path)
        .map_err(|e| io::Error::other(format!("failed to open root file: {e}")))?;

    let bytes = file.file_data();
    let _is_large = file.is_large();

    if cfg.bench_scan {
        let stats = bench_scan_root_for_zs_blocks(bytes, cfg.iters, cfg.verbose, cfg.fail_fast)?;
        println!(
        ...
        return Err(io::Error::other("root_accuracy_gate bench failed"));
    }
    ...
    struct BenchCompareStats {
        ...
        libzstd_out_mib_per_s: f64,
        speedup_vs_old_x: f64,
        speedup_vs_libzstd_x: f64,
    }
    ...
    fn decompress_root_zs_only_legacy(src: &[u8], expected_len: usize) -> Result<Vec<u8>, String> {
        ...
        while total_uncompressed < expected_len {
            ...
            ruz_dec.read_to_end(&mut block_out).map_err(|e| format!("ns_zstd read failed: {e}"))?;
    ...
    fn decompress_root_zs_only_libzstd(src: &[u8], expected_len: usize) -> Result<Vec<u8>, String> {
        let mut out = Vec::with_capacity(expected_len);
        ...
        while total_uncompressed < expected_len {
            ...
            if tag != b"ZS" {
                return Err(format!(
                    "non-ZS sub-block encountered in libzstd ZS-only decompressor: {:?}",
                    std::str::from_utf8(tag).unwrap_or("??")
                ));
            }

            let compressed = &src[offset..end];
            let mut block_out = Vec::with_capacity(u_size.max(1));
            zstd_safe::decompress(&mut block_out, compressed)
                .map_err(|e| format!("libzstd (zstd-safe) decompress failed: {e}"))?;
            if block_out.len() != u_size {
                return Err(format!(
                    "decoded size mismatch: got {} expected {}",
                    block_out.len(),
    ...
    fn bench_compare_tree_baskets(
            ...
            for i in 0..n {
                ...
                match basket_is_zs_only(compressed_data, obj_len) {
                    Ok(true) => {
                        ...
                        stats.out_bytes_per_iter += obj_len as u64;
                    }
                    Ok(false) => {
        ...
        if let Some(first) = jobs.first() {
            ...
            let new_out =
                ns_root::decompress::decompress(slice, first.expected_len).map_err(|e| {
                    io::Error::other(format!("bench-compare new decompress failed: {e}"))
                })?;
            let old_out = decompress_root_zs_only_legacy(slice, first.expected_len)
                .map_err(|e| io::Error::other(format!("bench-compare legacy failed: {e}")))?;
            let libzstd_out = decompress_root_zs_only_libzstd(slice, first.expected_len)
                .map_err(|e| io::Error::other(format!("bench-compare libzstd failed: {e}")))?;
            if new_out != libzstd_out {
                return Err(io::Error::other(
                    "bench-compare mismatch between new and libzstd outputs",
                ));
            }
            if old_out != libzstd_out {
                return Err(io::Error::other(
                    "bench-compare mismatch between legacy and libzstd outputs",
                ));
            }
        }

        let iters = iters.max(1);
        stats.out_bytes_total = stats.out_bytes_per_iter.saturating_mul(iters as u64);

        if iters > 1 {
            for job in &jobs {
                let slice = &bytes[job.start..job.end];
                let _ = ns_root::decompress::decompress(slice, job.expected_len);
                let _ = decompress_root_zs_only_legacy(slice, job.expected_len);
                let _ = decompress_root_zs_only_libzstd(slice, job.expected_len);
            }
        }
                    ...
                    Err(e) => {
                        ...
                        if fail_fast {
                            return Err(io::Error::other(format!(
                                "root_accuracy_gate bench failed: {e}"
                            )));
                        }
                    }
                }
        ...
        let start_libzstd = Instant::now();
        for _ in 0..iters {
            for job in &jobs {
                let slice = &bytes[job.start..job.end];
                match decompress_root_zs_only_libzstd(slice, job.expected_len) {
                    Ok(out) => {
                        if out.len() != job.expected_len {
                            stats.failures += 1;
                            if fail_fast {
        ...
        stats.libzstd_seconds = start_libzstd.elapsed().as_secs_f64();

        stats.baskets_checked = jobs.len();

        if stats.new_seconds > 0.0 {
            stats.new_out_mib_per_s =
                (stats.out_bytes_total as f64 / (1024.0 * 1024.0)) / stats.new_seconds;
        }
        if stats.old_seconds > 0.0 {
            stats.old_out_mib_per_s =
                (stats.out_bytes_total as f64 / (1024.0 * 1024.0)) / stats.old_seconds;
        }
        if stats.libzstd_seconds > 0.0 {
            stats.libzstd_out_mib_per_s =
                (stats.out_bytes_total as f64 / (1024.0 * 1024.0)) / stats.libzstd_seconds;
        }
        if stats.new_seconds > 0.0 {
            stats.speedup_vs_old_x = stats.old_seconds / stats.new_seconds;
            stats.speedup_vs_libzstd_x = stats.libzstd_seconds / stats.new_seconds;
        }
    ...
    if cfg.bench_compare {
        ...
        let tree = file.get_tree(&tree_name).map_err(|e| {
            io::Error::other(format!("failed to read tree '{tree_name}': {e} (try --bench-scan)"))
        })?;
        ...
        println!(
            "Bench(compare): iters={}, branches_checked={}, baskets_checked={}, baskets_uncompressed_skipped={}, baskets_non_zs_skipped={}, failures={}, out_bytes_per_iter={}, out_bytes_total={}, new_seconds={:.6}, new_out_mib_per_s={:.3}, old_seconds={:.6}, old_out_mib_per_s={:.3}, libzstd_seconds={:.6}, libzstd_out_mib_per_s={:.3}, speedup_vs_old_x={:.3}, speedup_vs_libzstd_x={:.3}",
            cfg.iters,
            ...
            stats.failures,
            stats.out_bytes_per_iter,
            stats.out_bytes_total,
            stats.new_seconds,
            stats.new_out_mib_per_s,
            stats.old_seconds,
            stats.old_out_mib_per_s,
            stats.libzstd_seconds,
            stats.libzstd_out_mib_per_s,
            stats.speedup_vs_old_x,
            stats.speedup_vs_libzstd_x,
        );
    ...
    if cfg.scan {
        ...
        return Err(io::Error::other("root_accuracy_gate failed"));
    }
    ...
    let tree = file.get_tree(&tree_name).map_err(|e| {
        io::Error::other(format!("failed to read tree '{tree_name}': {e} (try --scan)"))
    })?;
    ...
    for branch in &tree.branches {
        ...
        for i in 0..n {
            ...
            match verify_root_blocks_zs_only(compressed_data, obj_len, &ctx, cfg.verbose) {
                ...
                Err(e) => {
                    failures += 1;
                    eprintln!("FAIL branch='{}' basket={} seek={} : {}", branch.name, i, seek, e);
                    if cfg.fail_fast {
                        return Err(io::Error::other("root_accuracy_gate failed"));
                    }
                }
    ...
    if failures == 0 { Ok(()) } else { Err(io::Error::other("root_accuracy_gate failed")) }
}
            ...
            match arg.to_string_lossy().as_ref() {
                ...
                "--iters" => {
                    let Some(v) = args.next() else {
                        return Err(io::Error::new(
                            io::ErrorKind::InvalidInput,
                            "--iters requires a value",
                        ));
                    };
                    let s = v.to_string_lossy();
                    iters = s.parse().map_err(|_| {
                        io::Error::new(io::ErrorKind::InvalidInput, format!("invalid --iters: {s}"))
                    })?;
                }
...
fn scan_root_for_zs_blocks(
    ...
    while pos + 9 <= bytes.len() {
        ...
        let ctx = ScanCtx { file_offset: pos, block_c_size: c_size, block_u_size: u_size };
...
fn bench_scan_root_for_zs_blocks(
    ...
    // Pass 1: collect plausible ZS sub-block slices once (avoid timing the full scan).
    let mut blocks: Vec<(usize, usize, usize, usize)> = Vec::new(); // (pos, end, c_size, u_size)
    let mut pos = 0usize;
    while pos + 9 <= bytes.len() {
        if &bytes[pos..pos + 2] != b"ZS" {
            pos += 1;
        ...
        blocks.push((pos, end, c_size, u_size));
        pos = end;
    }
    ...
    for _ in 0..iters {
        for &(pos, end, c_size, u_size) in &blocks {
            let root_block = &bytes[pos..end];
            match ns_root::decompress::decompress_into(root_block, u_size, &mut out) {
                Ok(()) => {
                    if out.len() != u_size {
                        stats.failures += 1;
                        if verbose {
...
fn compare_zstd_block_scan(compressed: &[u8], ctx: &ScanCtx, verbose: bool) -> Result<(), String> {
    ...
    let mut ref_out = Vec::with_capacity(ctx.block_u_size.max(1));
    zstd_safe::decompress(&mut ref_out, compressed)
        .map_err(|e| format!("libzstd (zstd-safe) decompress failed: {e}"))?;
    let mut ref_dec = Cursor::new(ref_out);
    ...
    if verbose {
        println!("OK(scan) file_off={} out={} bytes", ctx.file_offset, bytes);
    }
    Ok(())
}
...
fn select_first_tree_name(file: &RootFile) -> Result<String, io::Error> {
    let keys: Vec<KeyInfo> =
        file.list_keys().map_err(|e| io::Error::other(format!("failed to list keys: {e}")))?;
...
fn bench_tree_baskets(
    ...
    for branch in branches {
        ...
        for i in 0..n {
            ...
            for _ in 0..iters {
                match ns_root::decompress::decompress_into(compressed_data, obj_len, &mut out) {
                    Ok(()) => {
                        if out.len() != obj_len {
                            stats.failures += 1;
                            eprintln!(
...
fn read_tkey_header_fast(file_data: &[u8], pos: usize) -> Result<(usize, usize, usize), io::Error> {
    ...
    let n_bytes =
        r.read_u32().map_err(|e| io::Error::other(format!("read TKey n_bytes: {e}")))? as usize;
    let _version = r.read_u16().map_err(|e| io::Error::other(format!("read TKey version: {e}")))?;
    let obj_len =
        r.read_u32().map_err(|e| io::Error::other(format!("read TKey obj_len: {e}")))? as usize;
    let _datime = r.read_u32().map_err(|e| io::Error::other(format!("read TKey datime: {e}")))?;
    let key_len =
        r.read_u16().map_err(|e| io::Error::other(format!("read TKey key_len: {e}")))? as usize;
...
fn compare_zstd_block(
    ...
    let mut ref_out = Vec::with_capacity(expected_u_size.max(1));
    zstd_safe::decompress(&mut ref_out, compressed)
        .map_err(|e| format!("libzstd (zstd-safe) decompress failed: {e}"))?;
    let mut ref_dec = Cursor::new(ref_out);
