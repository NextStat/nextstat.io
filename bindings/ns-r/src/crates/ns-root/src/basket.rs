...
/// Read and decompress a single basket, using the cache when provided.
///
/// On cache hit the decompressed payload is returned from cache (zero decompression cost).
/// On cache miss the basket is decompressed and inserted into the cache.
/// If `cache` is `None`, behaves identically to the uncached path.
pub fn read_basket_data_cached(
    file_data: &[u8],
    seek: u64,
    is_large: bool,
    cache: Option<&BasketCache>,
) -> Result<Arc<[u8]>> {
    if let Some(c) = cache {
        c.get_or_insert(seek, || read_basket_data(file_data, seek, is_large))
    } else {
        read_basket_data(file_data, seek, is_large).map(Arc::from)
    }
}
