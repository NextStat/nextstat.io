...
impl UnbinnedPdf for ChebyshevPdf {
    ...
    fn sample(
        ...
        if support.len() != 1 {
            return Err(Error::Validation(format!(
                "ChebyshevPdf sample expects 1D support, got {}D",
                support.len()
            )));
        }
        self.guardrail_check_positive(params)?;
        ...
        if !a.is_finite() || !b.is_finite() || a >= b {
            return Err(Error::Validation(format!(
                "ChebyshevPdf sample requires finite support with low < high, got ({a}, {b})"
            )));
        }
        ...
        // Upper bound: |T_k(x)| <= 1 on [-1,1] => f(x) <= 1 + Σ |c_k|.
        let mut f_max = 1.0f64;
        for &c in params {
            f_max += c.abs();
        }
        if !f_max.is_finite() || f_max <= 0.0 {
            return Err(Error::Validation(format!(
                "ChebyshevPdf sample has invalid f_max bound: {f_max}"
            )));
        }
        ...
        for _ in 0..n_events {
            ...
            loop {
                ...
                if !f.is_finite() || f <= 0.0 {
                    return Err(Error::Validation(format!(
                        "ChebyshevPdf is non-positive during sampling: f={f} (x={x}, x'={xp})"
                    )));
                }
                let u = u01(rng) * f_max;
                if u <= f {
                ...
                if tries > 100_000 {
                    return Err(Error::Computation(
                        "ChebyshevPdf sample rejection loop exceeded max iterations".into(),
                    ));
                }
            }
        }
