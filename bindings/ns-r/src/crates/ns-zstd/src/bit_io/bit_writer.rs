...
impl BitWriter<Vec<u8>> {
    ...
    pub fn new() -> Self {
        Self { output: Vec::new(), partial: 0, bits_in_partial: 0, bit_idx: 0 }
    }
}
...
impl<V: AsMut<Vec<u8>>> BitWriter<V> {
    ...
    pub fn from(mut output: V) -> BitWriter<V> {
        BitWriter { bit_idx: output.as_mut().len() * 8, output, partial: 0, bits_in_partial: 0 }
    }
    ...
    fn write_bits_64_cold(&mut self, bits: u64, num_bits: usize) {
        ...
        if full_bytes > 0 {
            self.output.as_mut().extend_from_slice(&bits.to_le_bytes()[..full_bytes]);
            num_bits -= full_bytes * 8;
            self.bit_idx += full_bytes * 8;
            bits >>= full_bytes * 8;
        }
...
mod tests {
    ...
    fn single_byte_written_4_4() {
        ...
        assert_eq!(0b0000_1111, output[0], "4 bits and 4 bits written into buffer");
    }
