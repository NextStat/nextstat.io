...
impl<V: AsMut<Vec<u8>>> FSEEncoder<'_, V> {
    ...
    pub fn encode_interleaved(&mut self, data: &[u8]) {
        ...
        if idx == 1 {
            ...
            self.writer.write_bits(state_2.index as u64, self.acc_log() as usize);
            self.writer.write_bits(state_1.index as u64, self.acc_log() as usize);
        } else {
            self.writer.write_bits(state_1.index as u64, self.acc_log() as usize);
            self.writer.write_bits(state_2.index as u64, self.acc_log() as usize);
        }
...
impl FSETable {
    pub(crate) fn try_next_state(&self, symbol: u8, idx: usize) -> Option<&State> {
        let states = &self.states[symbol as usize];
        if states.states.is_empty() {
            return None;
        }
        Some(states.get(idx, self.table_size))
    }
    ...
    pub(crate) fn try_start_state(&self, symbol: u8) -> Option<&State> {
        let states = &self.states[symbol as usize];
        states.states.first()
    }
...
pub(super) struct SymbolStates {
    ...
    /// Dense `idx -> states[]` lookup for encoding hot paths.
    lookup: Vec<u16>,
}
...
impl SymbolStates {
    ...
    fn get(&self, idx: usize, max_idx: usize) -> &State {
        if !self.lookup.is_empty() {
            ...
            // SAFETY: `idx` is always a valid table index (< table_size) in encoder hot paths, and
            // `lookup` is built to cover all indices for tables produced by this crate.
            let state_idx = unsafe { *self.lookup.get_unchecked(idx) as usize };
            debug_assert!(state_idx < self.states.len());
            return unsafe { self.states.get_unchecked(state_idx) };
        }

        // Fallback for tables built before `lookup` existed (should be rare).
        let start_search_at = (idx * self.states.len()) / max_idx;
        self.states[start_search_at..].iter().find(|state| state.contains(idx)).unwrap()
    }

    fn rebuild_lookup(&mut self, table_size: usize) {
        ...
        self.lookup.clear();
        self.lookup.resize(table_size, u16::MAX);

        for (state_idx, st) in self.states.iter().enumerate() {
            debug_assert!(state_idx <= u16::MAX as usize);
            let state_idx = state_idx as u16;
            debug_assert!(st.baseline < table_size);
            let end = st.last_index.min(table_size - 1);
            for idx in st.baseline..=end {
                self.lookup[idx] = state_idx;
            }
        }

        debug_assert!(
            self.lookup.iter().all(|&x| x != u16::MAX),
            "FSE table must cover all indices"
        );
    }
}
...
fn build_table_from_counts(counts: &[usize], max_log: u8, avoid_0_numbit: bool) -> FSETable {
    ...
    let (max_idx, max_val) = probs.iter().copied().enumerate().max_by_key(|(_i, v)| *v).unwrap();
    if avoid_0_numbit {
        let threshold = 1 << (acc_log - 1);
        if max_val > threshold {
            // Only redistribute when there's somewhere to put the mass.
            // If there is only one symbol (or all probs are identical), keep the original sum intact.
            if let Some(second_max_val) = probs.iter().copied().filter(|x| *x != max_val).max() {
                let redistribute = max_val - threshold;
                probs[max_idx] -= redistribute;
                let second_idx = probs
                    .iter()
                    .copied()
                    .enumerate()
                    .find(|(_i, v)| *v == second_max_val)
                    .unwrap()
                    .0;
                probs[second_idx] += redistribute;
                assert!(probs[second_idx] <= probs[max_idx]);
            }
        }
    }
...
pub(super) fn build_table_from_probabilities(probs: &[i32], acc_log: u8) -> FSETable {
    ...
    for (symbol, prob) in probs.iter().copied().enumerate() {
        ...
        for _ in 0..prob {
            states.push(State { num_bits: 0, baseline: 0, last_index: 0, index: idx });
    ...
    let table_size = 1 << acc_log;
    for sym in states.iter_mut() {
        sym.rebuild_lookup(table_size);
    }

    FSETable { table_size, states }
}
