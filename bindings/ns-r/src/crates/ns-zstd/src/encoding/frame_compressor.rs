...
pub(crate) struct EncodedSequence {
    ...
    pub ml_code: u8,
    pub of_code: u8,
    /// Packed extra bits for LL/ML/OF in the same order they are written to the bitstream:
    /// `ll_add_bits | (ml_add_bits << ll_num_bits) | (of_add_bits << (ll_num_bits + ml_num_bits))`
    pub extra_bits: u64,
    /// Number of valid bits in `extra_bits` (<= 64).
    ...
    pub fn compress(&mut self) {
        ...
        loop {
            ...
            match self.compression_level {
                ...
                CompressionLevel::Better | CompressionLevel::Best => {
                    // Not yet implemented — fall back to Default (highest implemented quality).
                    compress_default(&mut self.state, last_block, uncompressed_data, output)
                }
            }
            drain.write_all(output).unwrap();
