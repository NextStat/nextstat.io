use crate::{
    ...
    encoding::{
        block_header::BlockHeader, blocks::compress_block, blocks::FseTableStrategy,
        frame_compressor::CompressState, Matcher,
    },
};
use alloc::vec::Vec;
...
pub fn compress_default<M: Matcher>(
    ...
    compressed.clear();
    state.matcher.commit_space(uncompressed_data);
    compress_block(state, &mut compressed, FseTableStrategy::RepeatOrBuild);
