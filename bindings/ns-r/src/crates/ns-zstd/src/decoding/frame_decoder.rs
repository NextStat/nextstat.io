...
use crate::decoding::slice_output_buffer::{PtrOutputBuffer, SliceOutputBuffer};
...
/// ```
/// use ns_zstd::decoding::BlockDecodingStrategy;
///
/// # #[cfg(feature = "std")]
...
/// # #[cfg(not(feature = "std"))]
/// use ns_zstd::io::{Read, Write};
///
/// fn decode_this(mut file: impl Read) {
///     //Create a new decoder
///     let mut frame_dec = ns_zstd::decoding::FrameDecoder::new();
...
impl FrameDecoderState {
    ...
    pub fn reset(&mut self, source: impl Read) -> Result<(), FrameDecoderError> {
        ...
        if window_size > MAXIMUM_ALLOWED_WINDOW_SIZE {
            return Err(FrameDecoderError::WindowSizeTooBig { requested: window_size });
        }
...
impl FrameDecoder {
    ...
    pub fn new() -> FrameDecoder {
        FrameDecoder { state: None, dicts: BTreeMap::new() }
    }
    ...
    pub fn reset(&mut self, source: impl Read) -> Result<(), FrameDecoderError> {
        ...
        if let Some(dict_id) = state.frame_header.dictionary_id() {
            let dict = self.dicts.get(&dict_id).ok_or(err::DictNotProvided { dict_id })?;
    ...
    pub fn decode_blocks(
        ...
        loop {
            ...
            let (block_header, block_header_size) =
                block_dec.read_block_header(&mut source).map_err(err::FailedToReadBlockHeader)?;
            ...
            if block_header.last_block {
                ...
                if state.frame_header.descriptor.content_checksum_flag() {
                    ...
                    source.read_exact(&mut chksum).map_err(err::FailedToReadChecksum)?;
        ...
        while !input.is_empty() {
            match self.init(&mut input) {
                ...
                Err(FrameDecoderError::ReadFrameHeaderError(
                    ...
                    input =
                        input.get(length as usize..).ok_or(FrameDecoderError::FailedToSkipFrame)?;
                    continue;
                }
                Err(e) => return Err(e),
            };
            ...
            {
                let state = self.state.as_mut().expect("state must exist after init");
                ...
                loop {
                    ...
                    let bytes_read_in_block_body = match block_dec
                        .decode_block_content_from_slice_to(
                            &block_header,
                            &mut state.decoder_scratch,
                            &mut out,
                            &mut input,
    ...
    pub fn decode_all_to_vec(
        ...
        let out_ptr = spare.as_mut_ptr().cast::<u8>();
        let out_len = spare.len();

        let bytes_written = self.decode_all_to_ptr(input, out_ptr, out_len)?;
        unsafe {
            output.set_len(start_len + bytes_written);
        }
        Ok(())
    }

    fn decode_all_to_ptr(
        &mut self,
        mut input: &[u8],
        mut out_ptr: *mut u8,
        mut out_len: usize,
        ...
        while !input.is_empty() {
...

            {
                ...
                let dict_ptr = state.decoder_scratch.buffer.dict_content.as_ptr();
                let dict_len = state.decoder_scratch.buffer.dict_content.len();
                let mut out = PtrOutputBuffer::new(out_ptr, out_len, dict_ptr, dict_len);
                let mut block_dec = decoding::block_decoder::new();
                ...
                unsafe {
                    out_ptr = out_ptr.add(bytes_written);
                }
