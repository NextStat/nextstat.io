...
impl RingBuffer {
    ...
    fn reserve_amortized(&mut self, amount: usize) {
        ...
        let new_cap =
            usize::max(self.cap.next_power_of_two(), (self.cap + amount).next_power_of_two()) + 1;
        ...
        if self.cap > 0 {
            ...
            unsafe {
                ...
                new_buf.as_ptr().add(s1_len).copy_from_nonoverlapping(s2_ptr, s2_len);
    ...
    pub unsafe fn extend_from_within_unchecked_branchless(&mut self, start: usize, len: usize) {
        ...
        copy_with_checks(m1_ptr, m2_ptr, f1_ptr, f2_ptr, m1_in_f1, m2_in_f1, m1_in_f2, m2_in_f2);
        self.tail = (self.tail + len) % self.cap;
    }
}
...
unsafe fn copy_bytes_overshooting(
    ...
    if min_buffer_size >= COPY_AT_ONCE_SIZE && copy_at_least <= COPY_AT_ONCE_SIZE {
        dst.0.cast::<CopyType>().write_unaligned(src.0.cast::<CopyType>().read_unaligned())
    } else {
...
unsafe fn copy_without_checks(
    ...
    f1_ptr.add(m1_in_f1).copy_from_nonoverlapping(m2_ptr, m2_in_f1);

    f2_ptr.copy_from_nonoverlapping(m1_ptr.add(m1_in_f1), m1_in_f2);
    f2_ptr.add(m1_in_f2).copy_from_nonoverlapping(m2_ptr.add(m2_in_f1), m2_in_f2);
}
...
unsafe fn copy_with_nobranch_check(
    ...
    match case {
        ...
        12 => {
            f2_ptr.copy_from_nonoverlapping(m1_ptr, m1_in_f2);
            f2_ptr.add(m1_in_f2).copy_from_nonoverlapping(m2_ptr, m2_in_f2);
        }
