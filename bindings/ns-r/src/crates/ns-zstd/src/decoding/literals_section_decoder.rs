...
fn decompress_literals(
    ...
    // Prepare exact output space (initialized) and decode in-place.
    target.resize(start_len + regen, 0);
    let out_ptr = unsafe { target.as_mut_ptr().add(start_len) };
    let mut write_idx = 0;
    ...
    if num_streams == 4 {
        ...
        if source.len() < jump3 {
            target.truncate(start_len);
            return Err(err::MissingBytesForLiterals { got: source.len(), needed: jump3 });
        }

        let streams: [&[u8]; 4] =
            [&source[..jump1], &source[jump1..jump2], &source[jump2..jump3], &source[jump3..]];

        // Per-stream output sizes (zstd spec: first 3 = ceil(regen/4), 4th = remainder)
        ...
        for br_i in br.iter_mut() {
            if let Err(e) = skip_padding(br_i) {
                target.truncate(start_len);
                return Err(e);
            }
        }
