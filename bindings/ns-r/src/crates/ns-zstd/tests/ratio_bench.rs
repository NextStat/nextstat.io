...
fn bench_ratio_and_compress_throughput_vs_c() {
    ...
    // Measure two ns-zstd modes:
    // - cold: `compress_to_vec` (constructs a fresh compressor each call)
    // - reuse: reuse a `FrameCompressor` to avoid per-iteration allocations
    let mut ns_cold_best: f64 = 0.0;
    let mut ns_cold_total = 0.0;
    for _ in 0..iters {
        let t0 = Instant::now();
        ...
        ns_cold_total += tput;
        ns_cold_best = ns_cold_best.max(tput);
    }

    let mut ns_reuse_best: f64 = 0.0;
    let mut ns_reuse_total = 0.0;
    let mut ns_reuse: ns_zstd::encoding::FrameCompressor<
        std::io::Cursor<&[u8]>,
        Vec<u8>,
        ns_zstd::encoding::MatchGeneratorDriver,
    > = ns_zstd::encoding::FrameCompressor::new(ns_zstd::encoding::CompressionLevel::Default);
    ns_reuse.set_drain(Vec::with_capacity(bound.max(1)));
    for _ in 0..iters {
        ns_reuse.set_source(std::io::Cursor::new(&input));
        ns_reuse.drain_mut().unwrap().clear();
        let t0 = Instant::now();
        ns_reuse.compress();
        let dt = t0.elapsed().as_secs_f64();
        std::hint::black_box(ns_reuse.drain().unwrap().len());
        let tput = mbps(input.len(), dt);
        ns_reuse_total += tput;
        ns_reuse_best = ns_reuse_best.max(tput);
    }
    ...
    eprintln!(
        "compress throughput:\n  ns_default (cold):  best={:.1} MiB/s avg={:.1} MiB/s (iters={})\n  ns_default (reuse): best={:.1} MiB/s avg={:.1} MiB/s (iters={})\n  c_zstd_l3:          best={:.1} MiB/s avg={:.1} MiB/s (iters={})",
        ns_cold_best,
        ns_cold_total / iters as f64,
        iters,
        ns_reuse_best,
        ns_reuse_total / iters as f64,
        iters,
        c_best,
