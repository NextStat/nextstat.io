        ...
        D: Deserializer<'de>,
    {
        ...
        match type_str {
            "normfactor" => {
                ...
                Ok(Hs3Modifier::NormFactor { name: m.name, parameter: m.parameter })
            }
            "normsys" => {
...
impl Serialize for Hs3Modifier {
        ...
        S: Serializer,
    {
        ...
        match self {
            ...
            Hs3Modifier::NormSys { name, parameter, constraint_name, data } => {
            ...
            Hs3Modifier::HistoSys { name, parameter, constraint_name, data } => {
                let mut map = serializer.serialize_map(Some(5))?;
            ...
            Hs3Modifier::StatError { name, parameters, constraint_type } => {
            ...
            Hs3Modifier::ShapeSys { name, parameters, constraint_type, data } => {
                let mut map = serializer.serialize_map(Some(5))?;
            ...
            Hs3Modifier::ShapeFactor { name, parameters, parameter } => {
                let mut map = serializer.serialize_map(Some(4))?;
            ...
            Hs3Modifier::Lumi { name, parameter, constraint_name } => {
                let mut map = serializer.serialize_map(Some(4))?;
...
impl Hs3Modifier {
    ...
    pub fn type_tag(&self) -> &str {
        match self {
            ...
            Hs3Modifier::Unknown(v) => v.get("type").and_then(|t| t.as_str()).unwrap_or("unknown"),
        }
    }
}
...
impl Default for Hs3UnbinnedPdf {
    fn default() -> Self {
        Self {
            pdf_type: String::new(),
            observable: None,
