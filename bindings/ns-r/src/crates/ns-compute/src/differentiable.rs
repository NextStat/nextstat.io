...
fn launch_kernel(
    ...
    // SAFETY: All device pointers are valid CudaSlice allocations, scalar args match
    // the compiled kernel signature, launch config is within hardware limits.
    unsafe {
        builder
            .launch(config)
...
impl DifferentiableAccelerator {
    ...
    pub fn from_gpu_data(
        ...
        let d_nominal = stream.clone_htod(&data.nominal).map_err(cuda_err)?;
        let d_samples = stream.clone_htod(&data.samples).map_err(cuda_err)?;
        let d_modifier_descs = stream.clone_htod(&data.modifier_descs).map_err(cuda_err)?;
        let d_modifier_desc_offsets =
            stream.clone_htod(&data.modifier_desc_offsets).map_err(cuda_err)?;
        let d_per_bin_param_indices =
            stream.clone_htod(&data.per_bin_param_indices).map_err(cuda_err)?;
        let d_modifier_data = stream.clone_htod(&data.modifier_data).map_err(cuda_err)?;
        let d_aux_poisson = stream.clone_htod(&data.aux_poisson).map_err(cuda_err)?;
        let d_gauss_constr = stream.clone_htod(&data.gauss_constraints).map_err(cuda_err)?;

        // Dynamic buffers (single model, not batch)
        let d_params = stream.alloc_zeros::<f64>(data.n_params).map_err(cuda_err)?;
        let d_observed = stream.alloc_zeros::<f64>(data.n_main_bins).map_err(cuda_err)?;
        let d_ln_facts = stream.alloc_zeros::<f64>(data.n_main_bins).map_err(cuda_err)?;
        let d_obs_mask = stream.alloc_zeros::<f64>(data.n_main_bins).map_err(cuda_err)?;
        let d_nll_out = stream.alloc_zeros::<f64>(1).map_err(cuda_err)?;
        let d_grad_params_out = stream.alloc_zeros::<f64>(data.n_params).map_err(cuda_err)?;
        let d_grad_signal_buf = stream.alloc_zeros::<f64>(total_signal_bins).map_err(cuda_err)?;
        ...
        let d_signal_indices = stream.clone_htod(&indices).map_err(cuda_err)?;
        let d_signal_first_bins = stream.clone_htod(&first_bins).map_err(cuda_err)?;
        let d_signal_n_bins_arr = stream.clone_htod(&n_bins_arr).map_err(cuda_err)?;
    ...
    pub fn upload_observed(
        ...
        self.stream.memcpy_htod(observed, &mut self.d_observed).map_err(cuda_err)?;
        self.stream.memcpy_htod(ln_facts, &mut self.d_ln_facts).map_err(cuda_err)?;
        self.stream.memcpy_htod(obs_mask, &mut self.d_obs_mask).map_err(cuda_err)?;
        Ok(())
    }
    ...
    fn download_nll_and_grad_params(&self) -> ns_core::Result<(f64, Vec<f64>)> {
        ...
        self.stream.memcpy_dtoh(&self.d_grad_params_out, &mut grad_params).map_err(cuda_err)?;
    ...
    pub fn nll_grad_all(
        ...
        self.stream.memcpy_dtoh(&self.d_grad_signal_buf, &mut grad_signal).map_err(cuda_err)?;
        self.stream.synchronize().map_err(cuda_err)?;
