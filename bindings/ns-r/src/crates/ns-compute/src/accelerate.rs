...
pub fn poisson_nll_accelerate(
    ...
    POISSON_SCRATCH.with(|scratch| {
        ...
        // SAFETY: All pointers point into slices of length >= `n` (guaranteed by
        // `ensure_len` and caller contract). `ni` is the validated c_int length.
        unsafe {
...
pub fn batch_poisson_nll_accelerate(
    ...
    if per_toy_obs {
        ...
        // SAFETY: All flat buffers have length `total_bins`, validated by asserts above.
        // `ni` is the validated c_int cast of `total_bins`.
        unsafe {
        ...
        // SAFETY: `ln_exp` and `expected_flat` both have length `total_bins`; `ni` is valid.
        unsafe {
            vvlog(ln_exp.as_mut_ptr(), expected_flat.as_ptr(), &ni);
        }
        ...
        for toy_idx in 0..n_toys {
            ...
            // SAFETY: Slices are bounds-checked above; length `n_bins` is correct for vDSP.
            unsafe {
    ...
    for (toy_idx, result) in results.iter_mut().enumerate() {
        let offset = toy_idx * n_bins;
        // SAFETY: `offset + n_bins <= total_bins` by loop bounds; `result` is a valid &mut.
        unsafe {
...
pub fn clamp_expected_inplace(expected: &mut [f64], floor: f64) {
    ...
    let hi = f64::MAX;
    // SAFETY: `expected` is a valid mutable slice of length `n`; in-place aliasing
    // (src == dst) is permitted by vDSP_vclipD.
    unsafe {
