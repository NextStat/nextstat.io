...
impl MetalBatchAccelerator {
    ...
    fn copy_to_buffer<T>(buffer: &Buffer, data: &[T]) {
        ...
        // by the caller. `contents()` returns a valid mapped pointer for the buffer's
        // lifetime. `data` is a valid slice; no overlap with device memory.
        unsafe {
    ...
    fn read_buffer_f32_to_f64(buffer: &Buffer, count: usize) -> Vec<f64> {
        ...
        // `contents()` returns a valid mapped pointer; `count` matches the kernel output size.
