...
def _sample_color(name: str) -> str:
    ...
    idx = int(digest[:8], 16) % len(_SAMPLE_PALETTE)
    return _SAMPLE_PALETTE[idx]
...
def _plot_distributions_channel(channel: Mapping[str, Any]):
    ...
    if mc_band_postfit is not None:
    ...
    if labels:
        ...
        items = sorted(zip(handles, labels), key=lambda t: _priority(t[1]))
        ax.legend(
            [h for (h, _l) in items],
            [l for (_h, l) in items],
            ncol=2,
