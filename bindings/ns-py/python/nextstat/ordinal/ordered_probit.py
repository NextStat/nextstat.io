"""Ordered probit regression (latent Gaussian threshold) (Phase 9.C).
...
class OrderedProbitFit:
    ...
    def predict_proba(self, x: Sequence[Sequence[float]]) -> List[List[float]]:
        ...
        out: List[List[float]] = []
        for e in eta:
            cdf = [_normal_cdf(self.cutpoints[0] - float(e))]
            for ck in self.cutpoints[1:]:
                cdf.append(_normal_cdf(float(ck) - float(e)))
...
def fit(
    ...
    *,
    ...
    model = nextstat._core.OrderedProbitModel(x2, y2, n_levels=k)
