...
fn sample_laps_py<'py>(
    ...
    let laps_model = if let Some(m) = laps_model_from_raw {
        ...
        match name {
            ...
            other => {
                return Err(PyValueError::new_err(format!(
                    "model must be 'std_normal', 'eight_schools', 'neal_funnel', 'neal_funnel_ncp', 'neal_funnel_riemannian', 'glm_logistic', or a RawCudaModel, got '{other}'"
                )));
            }
        }
    } else {
    ...
    let result = sampler_result_to_py(
        py,
