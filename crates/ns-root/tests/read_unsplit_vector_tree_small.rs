...
fn load_expected() -> Expected {
    ...
    let text = std::fs::read_to_string(&path).expect("unsplit_vector_tree_small_expected.json not found");
...
fn read_unsplit_vector_branch_with_small_entry_offset_len() {
    ...
    assert_eq!(
        jet_info.entry_offset_len, 12,
        "expected small fEntryOffsetLen for 3-entry basket"
    );
