...
fn main() -> Result<(), io::Error> {
    ...
    println!(
        "\nSummary: {} ok, {} failed (keep_going={})",
        ok, failed, cfg.keep_going
    );

    if failed == 0 {
        Ok(())
    } else {
        Err(io::Error::new(io::ErrorKind::Other, "accuracy gate failed"))
    }
}
...
fn compare_file(cfg: &Config, path: &Path) -> Result<Stats, io::Error> {
    ...
    let zstd_stdout = child
        .stdout
        .take()
        .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "missing zstd stdout"))?;
    let zstd_stderr = child
        .stderr
        .take()
        .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "missing zstd stderr"))?;
    ...
    if !status.success() {
        ...
        return Err(io::Error::new(
            io::ErrorKind::Other,
            format!("reference zstd failed: status={status}, stderr={tail}"),
        ));
    }
        ...
        loop {
            while self.decoder.is_finished() && self.decoder.can_collect() == 0 {
                match self.decoder.init(&mut self.source) {
                    ...
                    Err(FrameDecoderError::ReadFrameHeaderError(ReadFrameHeaderError::MagicNumberReadError(e)))
