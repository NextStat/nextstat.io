...
enum Commands {
    ...
    UnbinnedFitToys {
        ...
        /// Use GPU for unbinned toy fitting. Values: "cuda" (NVIDIA, f64), "metal" (Apple Silicon, f32), or "auto" (policy-gated).
        #[arg(long)]
        gpu: Option<String>,
...
enum VizCommands {
    ...
    Render {
        ...
        /// VizConfig YAML file for native renderer (themes, colors, experiment label, etc.).
        #[arg(long)]
        config: Option<PathBuf>,

        /// Force Python matplotlib renderer instead of native Rust renderer.
        #[arg(long)]
        use_python: bool,
    },
}
...
enum ImportCommands {
    ...
    /// Cabinetry YAML config → pyhf JSON workspace (requires pre-built histograms).
    Cabinetry {
        /// Path to cabinetry config YAML.
        #[arg(long)]
        config: PathBuf,

        /// Directory containing pre-built histograms (JSON files: {region}_{sample}.json
        /// with keys "nominal", "stat_uncertainties", "systematics").
        #[arg(long)]
        histograms: PathBuf,
...
fn main() -> Result<()> {
    ...
    match command {
        ...
        Commands::Fit {
            ...
            if let Some(ref dev) = gpu {
                match dev.as_str() {
                    ...
                    other => anyhow::bail!(
                        "unknown --gpu device: {other}. Use 'cuda', 'metal', or 'auto'"
                    ),
                }
            }
            if parity {
        ...
        Commands::UnbinnedFitToys {
            ...
            if shard.is_some() && matches!(gpu.as_deref(), Some("cuda" | "metal")) {
...
fn cmd_import_cabinetry(
    config_path: &PathBuf,
    histograms_dir: &PathBuf,
    output: Option<&PathBuf>,
) -> Result<()> {
    use ns_translate::cabinetry;

    tracing::info!(config = %config_path.display(), histograms = %histograms_dir.display(), "importing cabinetry config");

    let yaml = std::fs::read_to_string(config_path)?;
    let config = cabinetry::parse_cabinetry_config(&yaml).map_err(|e| anyhow::anyhow!("{e}"))?;

    // Load histograms from JSON files: {region}_{sample}.json
    let mut histograms = Vec::new();
    let mut observations = Vec::new();

    for region in &config.regions {
        for sample in &config.samples {
            let filename = format!("{}_{}.json", region.name, sample.name);
            let path = histograms_dir.join(&filename);
            if !path.exists() {
                if sample.data {
                    // Data sample → observation.
                    let obs_path = histograms_dir.join(format!("{}_data.json", region.name));
                    if obs_path.exists() {
                        let obs_json: serde_json::Value =
                            serde_json::from_str(&std::fs::read_to_string(&obs_path)?)?;
                        if let Some(data) = obs_json.get("data").and_then(|d| d.as_array()) {
                            let vals: Vec<f64> = data.iter().filter_map(|v| v.as_f64()).collect();
                            observations.push((region.name.clone(), vals));
                        }
                    }
                    continue;
                }
                anyhow::bail!("missing histogram file: {}", path.display());
            }

            let histo_json: serde_json::Value =
                serde_json::from_str(&std::fs::read_to_string(&path)?)?;

            let nominal: Vec<f64> = histo_json
                .get("nominal")
                .and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
                .unwrap_or_default();

            let stat_unc: Vec<f64> = histo_json
                .get("stat_uncertainties")
                .and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
                .unwrap_or_default();

            let mut systs = Vec::new();
            if let Some(syst_obj) = histo_json.get("systematics").and_then(|v| v.as_object()) {
                for (name, val) in syst_obj {
                    if let (Some(up), Some(down)) = (val.get("up"), val.get("down")) {
                        let up_data: Vec<f64> = up
                            .as_array()
                            .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
                            .unwrap_or_default();
                        let down_data: Vec<f64> = down
                            .as_array()
                            .map(|a| a.iter().filter_map(|v| v.as_f64()).collect())
                            .unwrap_or_default();
                        systs.push((name.clone(), up_data, down_data));
                    }
                }
            }

            if sample.data {
                observations.push((region.name.clone(), nominal));
            } else {
                histograms.push(cabinetry::HistogramData {
                    region: region.name.clone(),
                    sample: sample.name.clone(),
                    nominal,
                    stat_uncertainties: stat_unc,
                    systematics: systs,
                });
            }
        }
    ...
    let workspace = cabinetry::to_workspace(&config, &histograms, &observations)
        .map_err(|e| anyhow::anyhow!("{e}"))?;

    let ws_json = serde_json::to_value(&workspace)?;
    write_json(output, &ws_json)?;

    tracing::info!(
        channels = workspace.channels.len(),
        measurements = workspace.measurements.len(),
        "cabinetry config imported successfully"
    );
...
fn cmd_unbinned_hypotest_toys(
    ...
    const AUTO_CUDA_MIN_EVENTS_PER_TOY: usize = 100_000;

    if n_toys == 0 {
        anyhow::bail!("n_toys must be > 0");
    }
    if gpu == Some("auto") {
        if !gpu_devices.is_empty() || gpu_shards.is_some() || gpu_sample_toys {
            anyhow::bail!(
                "--gpu auto must not be combined with --gpu-devices/--gpu-shards/--gpu-sample-toys; use --gpu cuda|metal for explicit control"
            );
        }
    } else if gpu_sample_toys && gpu != Some("cuda") && gpu != Some("metal") {
    ...
    // Apply `--gpu auto` policy (CPU vs CUDA).
    let (gpu, gpu_sample_toys) = if gpu == Some("auto") {
        if estimated_events_per_toy >= AUTO_CUDA_MIN_EVENTS_PER_TOY {
            #[cfg(feature = "cuda")]
            {
                tracing::info!(
                    estimated_events_per_toy,
                    "gpu=auto resolved to cuda (reason=estimated_events_per_toy_threshold)"
                );
                (Some("cuda"), true)
            }
            #[cfg(not(feature = "cuda"))]
            {
                anyhow::bail!(
                    "--gpu auto resolved to cuda, but this binary was built without --features cuda"
                );
            }
        } else {
            tracing::info!(
                estimated_events_per_toy,
                "gpu=auto resolved to cpu (reason=estimated_events_per_toy_threshold)"
            );
            (None, false)
        }
    } else {
    ...
    #[allow(unused_variables)]
    let _metal_max_toys_per_batch = if gpu == Some("metal") {
    ...
    let output_json = match gpu {
        Some("metal") => {
            #[cfg(feature = "metal")]
            {
                ...
                let metal_batches = contiguous_toy_batches(n_toys, _metal_max_toys_per_batch);
                ...
                if !expected_set {
                    ...
                    timing_breakdown.insert(
                        ...
                        serde_json::json!({
                            ...
                            "max_toys_per_batch": _metal_max_toys_per_batch,
...
fn cmd_unbinned_fit_toys(
    ...
    // `--gpu auto` is policy-gated and should be used without backend-specific knobs.
    if gpu == Some("auto") {
        if !gpu_devices.is_empty() || gpu_shards.is_some() || gpu_sample_toys || gpu_native {
            anyhow::bail!(
                "--gpu auto must not be combined with --gpu-devices/--gpu-shards/--gpu-sample-toys/--gpu-native; use --gpu cuda|metal for explicit control"
            );
        }
    } else {
    ...
    let is_simple_gauss_exp = {
        let mut ok = true;
        for ch in model.channels().iter().filter(|ch| ch.include_in_fit) {
            let mut tags: Vec<&str> = ch.processes.iter().map(|p| p.pdf.pdf_tag()).collect();
            if tags.len() != 2 {
                ok = false;
                break;
            }
            tags.sort_unstable();
            if tags[0] != "exponential" || tags[1] != "gaussian" {
                ok = false;
                break;
            }
        }
    ...
    // Apply `--gpu auto` policy (CPU vs CUDA).
    let (gpu, mut gpu_sample_toys, gpu_native) = if gpu == Some("auto") {
        if shard.is_some() {
            tracing::info!("gpu=auto resolved to cpu (reason=cpu_farm_shard)");
            (None, false, false)
        } else if has_flow_pdfs {
            tracing::info!("gpu=auto resolved to cpu (reason=flow_pdfs)");
            (None, false, false)
        } else if is_simple_gauss_exp {
            tracing::info!("gpu=auto resolved to cpu (reason=cpu_fused_gauss_exp_topology)");
            (None, false, false)
        } else {
            let mut approx_params: Vec<f64> = model.parameters().iter().map(|p| p.init).collect();
            for (name, value) in set {
                if let Some(idx) = model.parameters().iter().position(|p| p.name == *name) {
                    approx_params[idx] = *value;
                }
            }
            let est_events = estimate_unbinned_expected_events_per_toy(&model, &approx_params)?;
            if est_events >= AUTO_CUDA_MIN_EVENTS_PER_TOY {
                #[cfg(feature = "cuda")]
                {
                    tracing::info!(
                        est_events,
        ...
        (gpu, gpu_sample_toys, gpu_native)
    };

    // Resolve CUDA device ids after `--gpu auto` selection.
    let cuda_device_ids = normalize_cuda_device_ids(gpu, gpu_devices)?;

    // PF3.1-OPT4: auto-enable gpu_sample_toys when --gpu cuda
    // and the model uses only analytical PDFs (no flow PDFs).
    // `--gpu-native` remains explicit opt-in (no auto-enable).
    if !gpu_sample_toys && gpu == Some("cuda") && !has_flow_pdfs {
        tracing::info!("OPT4: auto-enabling --gpu-sample-toys (analytical PDFs, CUDA)");
        gpu_sample_toys = true;
    }
    #[cfg(not(feature = "cuda"))]
    let _ = gpu_native;
    ...
    // Policy warning for explicit CUDA selection in known CPU-faster regimes.
    if gpu == Some("cuda") && !has_flow_pdfs && is_simple_gauss_exp {
        tracing::warn!(
            estimated_events_per_toy,
            shard_n_toys,
            "policy: model matches Gauss+Exp topology; CPU fused path is typically faster. Consider omitting --gpu or using --gpu auto."
        );
    }
    ...
    let mut timing_breakdown = serde_json::Map::<String, serde_json::Value>::new();
    #[allow(unused_mut)]
    let mut optimizer_diagnostics = serde_json::Value::Null;
    ...
    match gpu {
        ...
        Some("metal") => {
            #[cfg(feature = "metal")]
            {
                ...
                let optimizer_diag = toy_fit_optimizer_diagnostics(
                    &fit_results_all,
                    ns_inference::OptimizerConfig::default().max_iter as usize,
                );
                optimizer_diagnostics = optimizer_diag.clone();
        ...
        Some("cuda") => {
            #[cfg(feature = "cuda")]
            {
                ...
                if !has_flows {
                    ...
                    let mut shard_progress_detail: Vec<serde_json::Value> = Vec::new();
                    ...
                    let (fit_results, mut accels_opt) = if gpu_sample_toys {
                        ...
                        if cuda_device_shard_plan.len() == 1 {
                            ...
                            let shard_dispatch_t0 = std::time::Instant::now();
                            let shard_progress = std::sync::Arc::new(std::sync::Mutex::new(Vec::<
                                serde_json::Value,
                            >::new(
                            )));
                            let shard_results: anyhow::Result<
                                Vec<anyhow::Result<Vec<ShardResult>>>,
                            > = std::thread::scope(|scope| {
                                let handles: Vec<_> = shards_by_device
                                        .into_iter()
                                        .map(|(device_id, device_shards)| {
                                            let cuda_opt_cfg = cuda_opt_cfg.clone();
                                            let shard_progress = std::sync::Arc::clone(&shard_progress);
                                            let shard_dispatch_t0 = shard_dispatch_t0;
                                            scope.spawn(move || {
                                            let checkpoint = |
                                                phase: &'static str,
                                                toy_start: usize,
                                                ...
                                                status: &'static str,
                                            | {
                                                if let Ok(mut v) = shard_progress.lock() {
                                                    v.push(serde_json::json!({
                                                        ...
                                                        "phase": phase,
                                                        "status": status,
                                                        "elapsed_s": shard_dispatch_t0.elapsed().as_secs_f64(),
                                                    }));
                                                }
                                            };

                                            checkpoint("sampler_init", 0, 0, "start");
                                            let mut samplers = Vec::<
                                                ns_compute::cuda_unbinned_toy::CudaUnbinnedToySampler,
                                            >::with_capacity(
                                                static_models.len()
                                            ...
                                            checkpoint("sampler_init", 0, 0, "done");
                                            ...
                                            for (toy_start, toy_end) in device_shards {
                                                ...
                                                checkpoint("sample", toy_start, toy_end, "start");
                                                let t_sample0 = std::time::Instant::now();
                                                let mut toy_offsets_by_channel =
                                                    Vec::<Vec<u32>>::with_capacity(samplers.len());
                                                let mut d_obs_flat_by_channel =
                                                ...
                                                for (ch_i, sampler) in samplers.iter_mut().enumerate() {
                                                    ...
                                                    let (offs, d_obs) = sampler.sample_toys_1d_device(
                                                        ...
                                                        anyhow::anyhow!(
                                                            "gpu {device_id} shard [{toy_start}..{toy_end}) sample ch{ch_i}: {e}"
                                                        )
                                                    })?;
                                                    toy_offsets_by_channel.push(offs);
                                                    d_obs_flat_by_channel.push(d_obs);
                                                ...
                                                let sample_s = t_sample0.elapsed().as_secs_f64();
                                                checkpoint("sample", toy_start, toy_end, "done");

                                                checkpoint("build", toy_start, toy_end, "start");
                                                let t_build0 = std::time::Instant::now();
                                                let mut accels = Vec::with_capacity(static_models.len());
                                                for (i, (m, (offs, d_obs))) in static_models
                                                    .iter()
                                                    ...
                                                    .enumerate()
                                                {
                                                    ...
                                                    .map_err(|e| {
                                                        anyhow::anyhow!(
                                                            "gpu {device_id} shard [{toy_start}..{toy_end}) build channel {i}: {e}"
                                                        )
                                                    })?;
                                                    accels.push(accel);
                                                }
                                                let build_s = t_build0.elapsed().as_secs_f64();
                                                checkpoint("build", toy_start, toy_end, "done");

                                                checkpoint("fit", toy_start, toy_end, "start");
                                                let t_fit0 = std::time::Instant::now();
                                                let fit_results_shard = if gpu_native {
                                                    if accels.len() == 1 {
                                                        accels[0].batch_fit_on_device(
                                                            ...
                                                            anyhow::anyhow!(
                                                                "gpu {device_id} shard [{toy_start}..{toy_end}) native fit single-channel: {e}"
                                                            )
                                                        })?
                                                    } else {
                                                        ns_compute::cuda_unbinned_batch::batch_fit_multi_channel_on_device(
                                                            ...
                                                            anyhow::anyhow!(
                                                                "gpu {device_id} shard [{toy_start}..{toy_end}) native fit multi-channel: {e}"
                                                            )
                                                        })?
                                                    }
                                                } else {
                                                    let (fit_results_shard, _accels) =
                                                        ns_inference::unbinned_gpu_batch::fit_unbinned_toys_batch_cuda_with_accels(
                                                            ...
                                                            anyhow::anyhow!(
                                                                "gpu {device_id} shard [{toy_start}..{toy_end}) fit_with_accels: {e}"
                                                            )
                                                        })?;
                                                    fit_results_shard
                                                };
                                                let fit_s = t_fit0.elapsed().as_secs_f64();
                                                checkpoint("fit", toy_start, toy_end, "done");
                                ...
                                for h in handles {
                                    match h.join() {
                                        ...
                                        Err(payload) => {
                                            ...
                                            let progress_snapshot = shard_progress
                                                .lock()
                                                .map(|v| v.clone())
                                                .unwrap_or_default();
                                            return Err(anyhow::anyhow!(
                                                "CUDA device-sharded worker panicked: {panic_msg}; progress checkpoints={}",
                                                serde_json::to_string(&progress_snapshot)
                            ...
                            shard_progress_detail =
                                shard_progress.lock().map(|v| v.clone()).unwrap_or_default();

                            for device_results in shard_results? {
                                let device_results = device_results.map_err(|e| {
                                    anyhow::anyhow!(
                                        "CUDA device-sharded worker failed: {e}; progress checkpoints={}",
                                        serde_json::to_string(&shard_progress_detail)
                                            .unwrap_or_else(|_| "[]".to_string())
                                    )
                                })?;
                                for sr in device_results {
                        ...
                        if cuda_device_ids.len() == 1 {
                            let t_build0 = std::time::Instant::now();
                            let device_id = cuda_device_ids[0];
                            let ctx = match std::panic::catch_unwind(|| {
                                ns_compute::cuda_driver::CudaContext::new(device_id)
                            }) {
                                Ok(Ok(ctx)) => ctx,
                                Ok(Err(e)) => {
                                    return Err(anyhow::anyhow!(
                                        "CUDA context init failed (device {device_id}): {e}"
                                    ));
                                }
                                Err(_) => {
                                    return Err(anyhow::anyhow!(
                                        "CUDA driver library not available (device {device_id})"
                                    ));
                                }
                            };
                            ...
                            for (i, ((m, offs), obs)) in static_models
                                ...
                                .enumerate()
                            {
                                let accel = ns_compute::cuda_unbinned_batch::CudaUnbinnedBatchAccelerator::from_unbinned_static_and_toys_in_ctx(
                                    ctx.clone(),
                                    stream.clone(),
                                    m,
                                    offs,
                    ...
                    let fit_max_iter = if gpu_native {
                        native_max_iter as usize
                    } else {
                        cuda_opt_cfg.max_iter as usize
                    };
                    let optimizer_diag = toy_fit_optimizer_diagnostics(&fit_results, fit_max_iter);
                    optimizer_diagnostics = optimizer_diag.clone();
                    ...
                    "toys".into(),
                    {
                        ...
                        if !shard_progress_detail.is_empty() {
                            t["shard_progress"] = serde_json::json!(shard_progress_detail);
                        }
                        t
                    },
                );
...
const CUDA_AUTO_USABLE_VRAM_FRAC: f64 = 0.70;
const CUDA_AUTO_MAX_TOYS_PER_DEVICE: usize = 4096;

fn min_shards_for_vram_budget(
    n_toys: usize,
    estimated_bytes_per_toy: usize,
    min_vram_bytes: usize,
) -> Option<(usize, usize)> {
    if n_toys == 0 || estimated_bytes_per_toy == 0 || min_vram_bytes == 0 {
        return None;
    }
    let usable = ((min_vram_bytes as f64) * CUDA_AUTO_USABLE_VRAM_FRAC) as usize;
    if usable == 0 {
        return None;
    }
    let max_toys_per_device =
        (usable / estimated_bytes_per_toy).max(1).min(CUDA_AUTO_MAX_TOYS_PER_DEVICE);
    let min_shards = n_toys.div_ceil(max_toys_per_device);
    Some((min_shards.max(1), max_toys_per_device.max(1)))
}
...
fn normalize_cuda_device_shard_plan(
    ...
    #[cfg(feature = "cuda")]
    let min_vram_bytes = cuda_device_ids
        .iter()
        .filter_map(|&d| ns_compute::cuda_device_total_mem(d).ok())
        .min()
        .unwrap_or(0);
    #[cfg(not(feature = "cuda"))]
    let min_vram_bytes = 0usize;

    let vram_guard = if gpu_sample_toys {
        min_shards_for_vram_budget(n_toys, estimated_bytes_per_toy, min_vram_bytes)
    } else {
        None
    };
    ...
    let mut n_shards = if let Some(v) = gpu_shards {
        ...
        let mut n = cuda_device_ids.len();
        if let Some((min_shards_vram, max_toys_per_device)) = vram_guard {
            let need = min_shards_vram.max(cuda_device_ids.len());
            if need > n {
                tracing::info!(
                    "OPT4 auto-shard: {min_vram_bytes} B VRAM, ~{estimated_bytes_per_toy} B/toy \
                     → max {max_toys_per_device} toys/device → {need} shards"
                );
                n = need;
            }
        }
        ...
        if gpu_sample_toys {
            tracing::warn!(
                "auto-shard: increasing shards from {n_shards} to {min_shards} \
                     for --gpu-sample-toys to satisfy 32-bit toy-offset budget \
                     (estimated_events_per_toy={estimated_events_per_toy}, max_toys_per_shard={max_toys_per_shard})"
            );
            n_shards = min_shards;
        } else {
    ...
    if let Some((min_shards_vram, max_toys_per_device)) = vram_guard {
        let required = min_shards_vram.max(cuda_device_ids.len());
        if n_shards < required {
            tracing::warn!(
                "auto-shard: increasing shards from {n_shards} to {required} \
                 for --gpu-sample-toys VRAM budget \
                 (min_vram={min_vram_bytes} B, ~{estimated_bytes_per_toy} B/toy, max_toys_per_device={max_toys_per_device})"
            );
            n_shards = required;
        }
    }
...
fn parse_warning_counter(warnings: &[String], prefix: &str) -> Option<u64> {
    warnings.iter().find_map(|w| w.strip_prefix(prefix).and_then(|v| v.parse::<u64>().ok()))
}

fn quantile_u64(sorted: &[u64], p: f64) -> Option<u64> {
    if sorted.is_empty() {
        return None;
    }
    if sorted.len() == 1 {
...
fn toy_fit_optimizer_diagnostics(
    fit_results: &[ns_core::Result<ns_core::FitResult>],
    max_iter: usize,
) -> serde_json::Value {
    let mut n_ok = 0usize;
    let mut n_validation_error = 0usize;
    let mut n_computation_error = 0usize;
    let mut n_retry_used = 0usize;
    let mut max_retry_attempts_used = 0u64;
    let mut n_line_search_exhausted = 0usize;
    let mut total_line_search_exhaustions = 0u64;
    let mut n_at_max_iter = 0usize;
    let mut iters = Vec::<u64>::new();
    let mut status_hist = std::collections::BTreeMap::<String, usize>::new();

    for r in fit_results {
        match r {
            Ok(fit) => {
                n_ok += 1;
                iters.push(fit.n_iter as u64);
                if max_iter > 0 && fit.n_iter >= max_iter {
                    n_at_max_iter += 1;
                }

                let retry_attempts =
                    parse_warning_counter(&fit.warnings, "retry_attempts_used=").unwrap_or(0);
                if retry_attempts > 0 {
                    n_retry_used += 1;
                    max_retry_attempts_used = max_retry_attempts_used.max(retry_attempts);
                }

                let ls_exhaust =
                    parse_warning_counter(&fit.warnings, "line_search_exhaustions=").unwrap_or(0);
                if ls_exhaust > 0 {
                    n_line_search_exhausted += 1;
                }
                total_line_search_exhaustions += ls_exhaust;

                let status = if fit.termination_reason.trim().is_empty() {
                    if fit.converged {
                        "Converged".to_string()
                    } else {
                        "MaxIterReached".to_string()
                    }
                } else {
                    fit.termination_reason.clone()
                };
                *status_hist.entry(status).or_insert(0) += 1;
            }
            Err(ns_core::Error::Validation(_)) => n_validation_error += 1,
            Err(_) => n_computation_error += 1,
        }
    }

    iters.sort_unstable();
    let frac =
        |num: usize, den: usize| -> f64 { if den == 0 { 0.0 } else { num as f64 / den as f64 } };

    serde_json::json!({
        "n_total": fit_results.len(),
        "n_ok": n_ok,
        "n_error": n_validation_error + n_computation_error,
        "error_histogram": {
            "validation": n_validation_error,
            "computation": n_computation_error,
        },
        "status_histogram": status_hist,
        "iter": {
            "median": quantile_u64(&iters, 0.50),
            "p95": quantile_u64(&iters, 0.95),
            "fraction_at_max_iter": frac(n_at_max_iter, n_ok),
            "max_iter": max_iter,
        },
        "retry": {
            "n_toys_used": n_retry_used,
            "fraction_toys_used": frac(n_retry_used, n_ok),
            "max_attempts_used": max_retry_attempts_used,
        },
        "line_search": {
            "n_toys_exhausted": n_line_search_exhausted,
            "fraction_toys_exhausted": frac(n_line_search_exhausted, n_ok),
            "total_exhaustions": total_line_search_exhaustions,
        },
    })
}
...
mod cli_parse_tests {
    ...
    fn interp_defaults_parses_and_defaults() {
        // Cli struct is large; run on a thread with an 8 MiB stack to avoid
        // stack overflow in debug builds on CI.
        std::thread::Builder::new()
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                let cli =
                    Cli::try_parse_from(["nextstat", "fit", "--input", "workspace.json"]).unwrap();
...
fn cmd_viz_render(
    ...
    // Native Rust renderer (default when feature is enabled and --use-python is not set)
    #[cfg(feature = "native-render")]
    if !use_python {
        let artifact_json = std::fs::read_to_string(input)?;
        let mut config = if let Some(yaml_path) = config_yaml {
        ...
        ns_viz_render::render_to_file(&artifact_json, kind.as_str(), output.as_ref(), &config)
            .map_err(|e| anyhow::anyhow!("{e}"))?;
        return Ok(());
    }

    #[cfg(not(feature = "native-render"))]
    let _ = (config_yaml, use_python);
...
mod tests {
    ...
    fn cuda_shard_plan_auto_increases_too_few_shards_for_u32_offsets_in_device_mode() {
        ...
        .expect("expected auto-shard to increase undersharded device plan");
        assert!(plan.len() >= 5, "expected >=5 shards after u32 guard, got {}", plan.len());
    }

    #[test]
    fn cuda_shard_plan_rejects_too_few_shards_for_u32_offsets_in_host_mode() {
        ...
        .expect_err("expected host mode to reject undersharded explicit plan");
        let msg = format!("{err:#}");
        assert!(
    ...
    #[test]
    fn vram_guard_computes_min_shards() {
        let guard =
            min_shards_for_vram_budget(1_000, 100, 1_000).expect("guard should be computed");
        assert_eq!(guard, (143, 7));
    }
    ...
    #[test]
    fn toy_fit_optimizer_diagnostics_reports_expected_fields() {
        let mut ok_retry = ns_core::FitResult::new(vec![0.0], vec![0.0], 1.0, true, 42, 50, 42)
            .with_diagnostics("ConvergedAfterRetry".to_string(), f64::NAN, f64::NAN, 0);
        ok_retry.warnings.push("retry_attempts_used=1".to_string());
        ok_retry.warnings.push("line_search_exhaustions=0".to_string());

        let mut ok_tail = ns_core::FitResult::new(vec![0.0], vec![0.0], 2.0, false, 100, 120, 100)
            .with_diagnostics("MaxIterReached".to_string(), f64::NAN, f64::NAN, 0);
        ok_tail.warnings.push("retry_attempts_used=0".to_string());
        ok_tail.warnings.push("line_search_exhaustions=2".to_string());

        let fit_results = vec![
            Ok(ok_retry),
            Ok(ok_tail),
            Err(ns_core::Error::Validation("bad".to_string())),
            Err(ns_core::Error::Computation("boom".to_string())),
        ];

        let diag = toy_fit_optimizer_diagnostics(&fit_results, 100);
        assert_eq!(diag["n_total"].as_u64(), Some(4));
        assert_eq!(diag["n_ok"].as_u64(), Some(2));
        assert_eq!(diag["n_error"].as_u64(), Some(2));
        assert_eq!(diag["error_histogram"]["validation"].as_u64(), Some(1));
        assert_eq!(diag["error_histogram"]["computation"].as_u64(), Some(1));
        assert_eq!(diag["retry"]["n_toys_used"].as_u64(), Some(1));
        assert_eq!(diag["line_search"]["n_toys_exhausted"].as_u64(), Some(1));
        assert_eq!(diag["line_search"]["total_exhaustions"].as_u64(), Some(2));
        assert_eq!(diag["iter"]["median"].as_u64(), Some(100));
        assert_eq!(diag["iter"]["p95"].as_u64(), Some(100));
        assert_eq!(diag["iter"]["fraction_at_max_iter"].as_f64(), Some(0.5));
        assert_eq!(diag["status_histogram"]["ConvergedAfterRetry"].as_u64(), Some(1));
        assert_eq!(diag["status_histogram"]["MaxIterReached"].as_u64(), Some(1));
    }

    #[test]
    fn toy_fit_optimizer_diagnostics_is_backward_compatible_without_warnings() {
        let ok = ns_core::FitResult::new(vec![0.0], vec![0.0], 1.0, true, 7, 8, 7);
        let diag = toy_fit_optimizer_diagnostics(&[Ok(ok)], 1000);
