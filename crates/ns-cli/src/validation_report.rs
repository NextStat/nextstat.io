...
fn suite_status(master: &Value, key: &str) -> Option<String> {
    master.get(key)?.get("status")?.as_str().map(|s| s.to_string())
}

fn map_push_highlight(out: &mut serde_json::Map<String, Value>, msg: String) {
    let v = out.entry("highlights".to_string()).or_insert_with(|| Value::Array(Vec::new()));
...
fn map_push_worst_case(
    ...
    let v = out.entry("worst_cases".to_string()).or_insert_with(|| Value::Array(Vec::new()));
    if let Value::Array(arr) = v {
...
fn suite_summary(master: &Value, key: &str) -> Option<Value> {
    ...
    match key {
        "pyhf" => {
                ...
                .and_then(|v| v.as_array())
            {
                ...
                for c in cases {
                    ...
                    let allowed =
                        c.get("parity").and_then(|v| v.get("nll_allowed")).and_then(|v| v.as_f64());
        ...
        "nuts_quality" => {
            if let Some(s) = master.get("nuts_quality") {
                let qs = s.get("quality_status").and_then(|v| v.as_str()).unwrap_or("unknown");
        ...
        "nuts_quality_report" => {
            ...
            {
                let cases =
                    rep.get("cases").and_then(|v| v.as_array()).cloned().unwrap_or_default();
                if !cases.is_empty() {
                    out.insert("n_cases".to_string(), Value::Number((cases.len() as u64).into()));
                    let n_ok = cases
                        .iter()
                    ...
                    for c in &cases {
                        ...
                        let div =
                            s.get("divergence_rate").and_then(|v| v.as_f64()).unwrap_or(f64::NAN);
                        let td = s
                            .get("max_treedepth_rate")
                            .and_then(|v| v.as_f64())
                            .unwrap_or(f64::NAN);
                        let ebfmi = s.get("min_ebfmi").and_then(|v| v.as_f64()).unwrap_or(f64::NAN);
                        let essb =
                            s.get("min_ess_bulk").and_then(|v| v.as_f64()).unwrap_or(f64::NAN);
                        let esst =
                            s.get("min_ess_tail").and_then(|v| v.as_f64()).unwrap_or(f64::NAN);
        ...
        "root" => {
            ...
            if let Some(rep) = suite.and_then(|v| v.get("report")).and_then(|v| v.as_object()) {
                let dq_atol = rep
                    .get("meta")
                ...
                if let Some(cases) = rep.get("cases").and_then(|v| v.as_array()) {
                    ...
                    if let Some((worst, worst_case)) = by_dq.first() {
                        map_push_highlight(
                            &mut out,
                            format!("worst max_abs_dq_mu = {:.3e} ({})", worst, worst_case),
                        );
                    }
                    if let Some((worst, _signed, worst_case)) = by_abs_mu.first() {
                        map_push_highlight(
                            &mut out,
                            format!("worst abs d_mu_hat = {:.3e} ({})", worst, worst_case),
                        );
                    }
                    ...
                    for (abs_v, signed, name) in by_abs_mu.into_iter().take(5) {
                        let notes =
                            mu_hat_atol.map(|a| format!("signed={:.3e} allowed={:.3e}", signed, a));
                        map_push_worst_case(&mut out, name, "abs_d_mu_hat", abs_v, notes);
                    }
                }

                if let Some(pr) =
                    rep.get("meta").and_then(|v| v.get("prereqs")).and_then(|v| v.as_object())
                {
                    ...
                    if !missing.is_empty() {
                        map_push_highlight(
                            &mut out,
                            format!("missing prereqs: {}", missing.join(", ")),
                        );
                    }
                }
            }
    ...
    if !out.contains_key("n_cases") {
        ...
        if let Some(cases) = cases_any
            && cases.iter().any(|c| c.get("ok").is_some())
        {
            out.insert("n_cases".to_string(), Value::Number((cases.len() as u64).into()));
            let n_ok = cases
                .iter()
    ...
    if let Some(r) = master.get(key).and_then(|v| v.get("reason")).and_then(|v| v.as_str()) {
        map_push_highlight(&mut out, format!("reason: {}", r));
    }
...
pub fn cmd_validation_report(
    ...
    for ch in 0..n_channels {
        let name = channel_names.get(ch).cloned().unwrap_or_else(|| format!("ch{ch}"));
    ...
    let poi = model.poi_index().and_then(|i| model.parameters().get(i)).map(|p| p.name.clone());
    ...
    if let Some(obj) = apex2_master.as_object() {
        for (k, v) in obj {
            ...
            if v.get("status").and_then(|s| s.as_str()).is_some()
                && let Some(sum) = suite_summary(&apex2_master, k)
            {
    ...
    let generated_at =
        if deterministic { Value::Null } else { Value::String(chrono::Utc::now().to_rfc3339()) };
    ...
    let out_json =
        if deterministic { crate::normalize_json_for_determinism(out_json) } else { out_json };
