...
fn tmp_file_path(suffix: &str) -> PathBuf {
    ...
    p.push(format!("nextstat_validation_report_{}_{}_{}.json", std::process::id(), nanos, suffix));
    p
}
...
fn validation_report_writes_json_artifact() {
    ...
    assert_eq!(v.pointer("/apex2_summary/overall").and_then(|x| x.as_str()), Some("pass"));
...
fn validation_report_includes_regulated_review_and_unknown_suite_counts() {
    ...
    assert_eq!(
        v.pointer("/regulated_review/contains_raw_data").and_then(|x| x.as_bool()),
        Some(false)
    );
    assert!(
    ...
    assert_eq!(
        v.pointer("/apex2_summary/suites/timeseries/n_cases").and_then(|x| x.as_u64()),
        Some(2)
    );
    assert_eq!(
        v.pointer("/apex2_summary/suites/timeseries/n_ok").and_then(|x| x.as_u64()),
        Some(2)
    );
    assert_eq!(
        v.pointer("/apex2_summary/suites/pharma_reference/n_cases").and_then(|x| x.as_u64()),
        Some(3)
    );
    assert_eq!(
        v.pointer("/apex2_summary/suites/pharma_reference/n_ok").and_then(|x| x.as_u64()),
        Some(3)
    );
    ...
    let evidence_all: Vec<String> = items
        ...
        .collect();
    assert!(
        evidence_all.iter().any(|s| s == "apex2:suite:timeseries:status=ok"),
        "expected evidence to include timeseries status"
    );
    assert!(
        evidence_all.iter().any(|s| s == "apex2:suite:pharma_reference:status=ok"),
