...
impl MetalProfiledDifferentiableSession {
    ...
    pub fn new(model: &HistFactoryModel, signal_sample_name: &str) -> Result<Self> {
        ...
        let mut accel =
            MetalDifferentiableAccelerator::from_metal_data(&metal_data, &signal_entries)?;
    ...
    fn signal_grad_at(&mut self, params: &[f64]) -> Result<Vec<f64>> {
        self.accel.nll_grad_all(params).map(|(_nll, _grad_params, grad_signal)| grad_signal)
    }
    ...
    pub fn batch_profiled_qmu(&mut self, mu_values: &[f64]) -> Result<Vec<(f64, Vec<f64>)>> {
        mu_values.iter().map(|&mu| self.profiled_qmu_and_grad(mu)).collect()
    }
}
