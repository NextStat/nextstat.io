...
impl GpuFlowSession {
    ...
    /// Compute NLL + gradient analytically from GPU-resident `float` log-prob and Jacobian.
    ///
    /// This is the fully device-resident analytical gradient path: the ONNX CUDA EP
    /// `log_prob_grad` model produces both `log_prob` and `d_log_prob_d_context` as
    /// device-resident `float` tensors, and we pass the raw pointers directly to the
    /// GPU NLL+grad reduction kernel. **1 forward pass** instead of `2×n_params+1`.
    ///
    /// # Arguments
    ///
    ...
    ///   Pass `0` if `n_context == 0`.
    /// - `params`: `[n_params]` current parameter values.
    pub fn nll_grad_analytical_device_f32(
        &mut self,
        d_logp_flat_ptr: u64,
        d_jac_flat_ptr: u64,
        params: &[f64],
        ...
        let (nll, sum_r, sum_jr) =
            self.accel.nll_grad_device_ptr_f32(d_logp_flat_ptr, d_jac_flat_ptr, &yields, params)?;
