...
use crate::ode_adaptive::{OdeOptions, OdeSystem, esdirk4, lsoda, rk45};
...
pub enum OdeSolverType {
    ...
    Esdirk4,
    /// LSODA-style automatic non-stiff/stiff switching.
    Lsoda,
}
...
impl OdePkSolver {
    ...
    pub fn solve(
        ...
        for dose in doses {
            match dose.route {
                ...
                DoseRoute::Infusion { duration } => {
                    ...
                    let info = InfusionInfo { rate, compartment: dose_compartment };
        ...
        for &tc in &critical_times {
            ...
            if tc > t_current + 1e-15 {
                ...
                if has_infusion {
                    let wrapper =
                        InfusionWrapper { inner: &*self.system, rates: &total_infusion_rate };
            ...
            while timeline_ptr < timeline.len() && timeline[timeline_ptr].time() <= tc + 1e-15 {
    ...
    fn integrate_dyn(
        ...
        match self.solver {
            ...
            OdeSolverType::Lsoda => lsoda(&wrapper, y0, t0, t1, opts),
        }
    }

    /// Compute total infusion rate per compartment from active infusions.
    fn compute_infusion_rates(&self, infusions: &[InfusionInfo], active: &[bool]) -> Vec<f64> {
        let dim = self.system.ndim();
...
mod tests {
    ...
    fn test_ode_pk_solver_iv_bolus() {
        ...
        assert!((result.cmax - dose).abs() < 1e-6, "Cmax should equal dose: got {}", result.cmax);
        assert!(result.tmax.abs() < 1e-6, "Tmax should be at t=0: got {}", result.tmax);
    }
    ...
    fn test_ode_pk_solver_multi_dose() {
        ...
        assert!(c_at_12 > c_at_0 * 0.5, "accumulation expected: c(12)={c_at_12}, c(0)={c_at_0}");
    }
    ...
    fn test_ode_pk_nll_minimum() {
        ...
        let nll_true =
            ode_pk_nll(&solver_true, &doses, &obs_times, &obs_values, 0, 0, sigma).unwrap();
        ...
        let expected_nll =
            0.5 * obs_values.len() as f64 * (2.0 * std::f64::consts::PI * sigma * sigma).ln();
        assert!(
    ...
    fn test_transit_delayed_absorption() {
        ...
        assert!(result.tmax > 0.5, "Tmax should be delayed: got {}", result.tmax);

        // Peak should be positive
        assert!(result.cmax > 0.0, "Cmax should be positive: got {}", result.cmax);
