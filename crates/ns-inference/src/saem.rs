...
use crate::foce::{
    CovariateSpec, FoceResult, IovSpec, OmegaMatrix, RandomEffectTransform,
    TimeVaryingCovariateSpec, apply_all_covariates, apply_all_time_varying_covariates,
    apply_random_effect_transform, build_iov_omega, compute_covariance_step_generic,
    iov_max_occasions, occasion_index_for_time, validate_iov_spec,
    validate_random_effect_transform, validate_time_varying_covariates,
};
...
impl SaemEstimator {
    ...
    /// Fit a 1-compartment IV model with configurable random-effect transforms.
    pub fn fit_1cpt_iv_with_transforms_correlated(
        ...
        if theta_init.len() != 2 {
            return Err(Error::Validation("theta_init must have 2 elements".into()));
        }
        if omega_init.dim() != 2 {
            return Err(Error::Validation("omega must be 2×2".into()));
        }
        if transforms.len() != 2 {
            return Err(Error::Validation("transforms must have 2 elements".into()));
        }
        for k in 0..2 {
    ...
    pub fn fit_2cpt_iv_correlated(
        ...
        if theta_init.len() != 4 {
            return Err(Error::Validation(
                "theta_init must have 4 elements [CL, V1, Q, V2]".into(),
            ));
        }
        if omega_init.dim() != 4 {
    ...
    /// Fit a 2-compartment IV model with configurable random-effect transforms.
    pub fn fit_2cpt_iv_with_transforms_correlated(
        ...
        if theta_init.len() != 4 {
            return Err(Error::Validation("theta_init must have 4 elements".into()));
        }
        if omega_init.dim() != 4 {
            return Err(Error::Validation("omega must be 4×4".into()));
        }
        if transforms.len() != 4 {
            return Err(Error::Validation("transforms must have 4 elements".into()));
        }
        for k in 0..4 {
            validate_random_effect_transform(theta_init[k], transforms[k])?;
        }
    ...
    /// Fit a 1-compartment oral model with per-subject dosing regimens using SAEM.
    ...
    /// Fit a 3-compartment IV model with per-subject dosing regimens using SAEM.
    ...
    /// Fit a 3-compartment oral model with per-subject dosing regimens using SAEM.
    ...
    pub fn fit_2cpt_oral_correlated(
        ...
        if theta_init.len() != 5 {
            return Err(Error::Validation(
                "theta_init must have 5 elements [CL, V1, Q, V2, Ka]".into(),
            ));
        }
        if omega_init.dim() != 5 {
    ...
    /// Fit a 2-compartment oral model with configurable random-effect transforms.
    pub fn fit_2cpt_oral_with_transforms_correlated(
        ...
        if theta_init.len() != 5 {
            return Err(Error::Validation("theta_init must have 5 elements".into()));
        }
        if omega_init.dim() != 5 {
            return Err(Error::Validation("omega must be 5×5".into()));
        }
        if transforms.len() != 5 {
            return Err(Error::Validation("transforms must have 5 elements".into()));
        }
        for k in 0..5 {
            validate_random_effect_transform(theta_init[k], transforms[k])?;
        }
    ...
    pub fn fit_3cpt_iv_correlated(
        ...
        if theta_init.len() != 6 {
            return Err(Error::Validation(
                "theta_init must have 6 elements [CL, V1, Q2, V2, Q3, V3]".into(),
            ));
        }
        if omega_init.dim() != 6 {
    ...
    /// Fit a 3-compartment IV model with configurable random-effect transforms.
    pub fn fit_3cpt_iv_with_transforms_correlated(
        ...
        if theta_init.len() != 6 {
            return Err(Error::Validation("theta_init must have 6 elements".into()));
        }
        if omega_init.dim() != 6 {
            return Err(Error::Validation("omega must be 6×6".into()));
        }
        if transforms.len() != 6 {
            return Err(Error::Validation("transforms must have 6 elements".into()));
        }
        for k in 0..6 {
            validate_random_effect_transform(theta_init[k], transforms[k])?;
        }
    ...
    pub fn fit_3cpt_oral_correlated(
        ...
        if theta_init.len() != 7 {
            return Err(Error::Validation(
                "theta_init must have 7 elements [CL, V1, Q2, V2, Q3, V3, Ka]".into(),
            ));
        }
        if omega_init.dim() != 7 {
    ...
    /// Fit a 3-compartment oral model with configurable random-effect transforms.
    pub fn fit_3cpt_oral_with_transforms_correlated(
        ...
        if theta_init.len() != 7 {
            return Err(Error::Validation("theta_init must have 7 elements".into()));
        }
        if omega_init.dim() != 7 {
            return Err(Error::Validation("omega must be 7×7".into()));
        }
        if transforms.len() != 7 {
            return Err(Error::Validation("transforms must have 7 elements".into()));
        }
        for k in 0..7 {
            validate_random_effect_transform(theta_init[k], transforms[k])?;
        }
    ...
    /// Fit a 1-compartment oral model with time-varying covariates using SAEM.
    ...
    /// Fit a 1-compartment oral model with IOV using SAEM.
    ...
    pub(crate) fn fit_generic_cov(
        ...
        if doses.len() != n_subjects {
            return Err(Error::Validation(format!(
                "doses.len()={} != n_subjects={n_subjects}",
                doses.len()
            )));
        }
        error_model.validate()?;
        ...
        {
            return Err(Error::Validation("config.lloq must be finite and >= 0".into()));
        }
        ...
        for iter in 0..total_iter {
            ...
            for s in 0..n_subjects {
                ...
                let theta_s = if has_cov {
                    apply_all_covariates(&theta, covariates, s)
                } else {
                    theta.clone()
                };
        ...
        let relative_change = if theta_trace.len() >= 2 {
            ...
            prev.iter()
                .zip(last.iter())
                .map(|(p, l)| if p.abs() > 1e-30 { ((l - p) / p).abs() } else { (l - p).abs() })
                .collect()
        } else {
        ...
        let sigma_val = crate::foce::sigma_from_error_model(&error_model);
        let cov_step = compute_covariance_step_generic(
            &subj_obs,
            &theta,
            &omega,
            &error_model,
    ...
    pub fn fit_1cpt_oral_with_transforms_correlated(
        ...
        if transforms.len() != 3 {
            return Err(Error::Validation("transforms must have 3 elements".into()));
        }
        for k in 0..3 {
            validate_random_effect_transform(theta_init[k], transforms[k])?;
        }
...
fn validate_covariates(
    ...
    for (i, cov) in covariates.iter().enumerate() {
        if cov.param_idx >= n_params {
            return Err(Error::Validation(format!(
                "covariate[{i}].param_idx={} >= n_params={n_params}",
                cov.param_idx
            )));
        }
        if cov.values.len() != n_subjects {
            return Err(Error::Validation(format!(
                "covariate[{i}].values.len()={} != n_subjects={n_subjects}",
                cov.values.len()
            )));
        }
    }
...
fn individual_log_posterior_generic(
    ...
    for &(t, yobs) in obs {
        ...
        if let Some(l) = lloq
            && yobs < l
        {
            log_lik -= em.nll_censored_below(l, c.max(1e-30));
        } else {
...
fn compute_marginal_ofv_generic(
    ...
    for (s, obs) in subj_obs.iter().enumerate() {
        ...
        let theta_s =
            if has_cov { apply_all_covariates(theta, covariates, s) } else { theta.to_vec() };
        ...
        for &(t, yobs) in obs {
            ...
            if let Some(l) = lloq
                && yobs < l
            {
                ofv += em.nll_censored_below(l, c.max(1e-30));
            } else {
...
pub fn bootstrap_saem(
    ...
    for b in 0..n_bootstrap {
        ...
        let bt_covariates: Vec<CovariateSpec> = covariates
            .iter()
            .map(|cov| CovariateSpec {
...
mod tests {
    ...
    fn make_synthetic_2cpt_iv_data(
        ...
        for s in 0..n_subjects {
            let eta: Vec<f64> =
                omega_sds.iter().map(|&sd| sd * rng.sample::<f64, _>(StandardNormal)).collect();
    ...
    fn saem_2cpt_iv_convergence() {
        ...
        let (times, y, subject_idx) = make_synthetic_2cpt_iv_data(
            n_subjects,
            n_obs,
            &theta_true,
            &omega_sds,
    ...
    fn saem_3cpt_iv_convergence() {
        ...
        let (times, y, subject_idx) = make_synthetic_3cpt_iv_data(
            n_subjects,
            n_obs,
            &theta_true,
            &omega_sds,
    ...
    fn saem_covariates_allometric() {
        ...
        let weights: Vec<f64> = (0..n_subjects)
            .map(|_| 50.0 + 40.0 * rng.sample::<f64, _>(rand_distr::StandardNormal).abs())
            .collect();
    ...
    fn saem_geweke_converged() {
        ...
        for (k, &z) in scores.iter().enumerate() {
            assert!(z.is_finite(), "Geweke z-score for param {k} should be finite, got {z}");
        }
    }
    ...
    fn saem_bootstrap_ci_contains_true() {
        ...
        assert!(
            result.n_successful >= 5,
            "at least 5 bootstrap reps should converge, got {}",
            result.n_successful
        );
...
12,24.22,1.17,0,0";
    ...
    fn saem_theophylline_nonmem_parity() {
        ...
        let doses: Vec<f64> = ds
            ...
            .iter()
            .map(|id| {
                ds.subject_records(id).iter().filter(|r| r.evid == 1).map(|r| r.amt).sum::<f64>()
            })
            .collect();
        ...
        assert!(
            ...
            "CL={:.4} outside expected range [0.02, 0.08]",
            result.theta[0]
        );
        // V should be in range [0.20, 0.80]
        assert!(
            result.theta[1] > 0.20 && result.theta[1] < 0.80,
            "V={:.4} outside expected range [0.20, 0.80]",
            result.theta[1]
        );
        // Ka should be in range [0.5, 5.0]
        assert!(
            result.theta[2] > 0.5 && result.theta[2] < 5.0,
            "Ka={:.4} outside expected range [0.5, 5.0]",
            result.theta[2]
        );
    }

    #[test]
    fn saem_lloq_censored_smoke() {
        let theta_true = [0.133, 8.0, 0.8];
        let omega_sds = [0.3, 0.25, 0.3];
        let sigma = 0.5;
        let dose = 100.0;
        ...
        let (times, y, subject_idx) =
            make_synthetic_data(n_subjects, n_obs, &theta_true, &omega_sds, sigma, dose, 9001);
        let doses = vec![dose; n_subjects];
        ...
        assert!(!diag.ofv_trace.is_empty());
    }

    #[test]
    fn saem_1cpt_iv_regimens_infusion_smoke() {
        let cl_pop = 1.1;
        let v_pop = 12.0;
        let sigma = 0.08;
        ...
        for sid in 0..n_subjects {
            let eta_cl: f64 = 0.2 * rng.sample::<f64, _>(StandardNormal);
            let eta_v: f64 = 0.2 * rng.sample::<f64, _>(StandardNormal);
            let cl_i = cl_pop * eta_cl.exp();
            let v_i = v_pop * eta_v.exp();

            for &t in &times_per {
                let c = regimens[sid].conc_1cpt(cl_i, v_i, 1.0, t);
                let eps: f64 = sigma * rng.sample::<f64, _>(StandardNormal);
                times.push(t);
                y.push((c + eps).max(0.01));
                subject_idx.push(sid);
            }
        }

        let config = SaemConfig { n_burn: 40, n_iter: 30, seed: 2026, ..Default::default() };
        let estimator = SaemEstimator::new(config);
        let omega_init = OmegaMatrix::from_diagonal(&[0.25, 0.25]).unwrap();
    ...
    #[test]
    fn saem_logit_normal_random_effects_smoke() {
        let cl_pop = 1.1;
        let v_pop = 13.0;
        let ka_pop = 1.6;
        let sigma = 0.08;
        let dose = 100.0;
        let n_subjects = 12usize;
        ...
        for sid in 0..n_subjects {
            let eta_cl: f64 = 0.2 * rng.sample::<f64, _>(StandardNormal);
            let eta_v: f64 = 0.2 * rng.sample::<f64, _>(StandardNormal);
            let eta_ka: f64 = 0.25 * rng.sample::<f64, _>(StandardNormal);
        ...
        let doses = vec![dose; n_subjects];
        let om = OmegaMatrix::from_diagonal(&[0.25, 0.25, 0.25]).unwrap();
        let cfg = SaemConfig { n_burn: 40, n_iter: 30, seed: 303, ..Default::default() };
