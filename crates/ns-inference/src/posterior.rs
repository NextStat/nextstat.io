...
impl<'a, M: LogDensityModel + ?Sized> Posterior<'a, M> {
    ...
    pub fn logpdf_grad_unconstrained(&self, z: &[f64]) -> Result<(f64, Vec<f64>)> {
        ...
        for g in &mut grad_theta_logpdf {
            *g = -*g;
        }
...
mod tests {
    ...
    fn test_identity_normal_prior_fused_grad_vs_finite_diff() {
        ...
        let x =
            vec![vec![1.0, 0.5], vec![-0.3, 1.2], vec![0.7, -0.8], vec![-1.1, 0.3], vec![0.2, 0.9]];
...
mod map_tests {
    ...
    fn test_map_se() {
        ...
        assert!((se[0] - expected_se).abs() < 1e-4, "SE should be {}, got {}", expected_se, se[0]);
    }
    ...
    fn test_map_shrinkage() {
        ...
        let config = MapConfig { compute_shrinkage: true, ..MapConfig::default() };
        let result = map_estimate(&posterior, &config).unwrap();
    ...
    fn test_map_no_prior_equals_mle() {
        ...
        let model =
            MultiGaussianModel { means: vec![1.0, -0.5, 3.0], variances: vec![1.0, 2.0, 0.5] };
        let posterior = Posterior::new(&model); // flat priors

        let config = MapConfig { compute_se: false, ..MapConfig::default() };
        let result = map_estimate(&posterior, &config).unwrap();

        assert!(result.converged, "MAP should converge");

        // MAP should equal MLE = data_means
        for (i, (&map_val, &expected)) in result.params.iter().zip(model.means.iter()).enumerate() {
            assert!(
