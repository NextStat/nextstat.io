//! Bijective transforms for unconstrained parameterization.
//!
//! NUTS samples in unconstrained space `z ∈ R^n`. These transforms map
//! between unconstrained `z` and constrained `theta` (the model parameters),
//! providing the Jacobian terms needed for correct posterior density.
...
mod tests {
    ...
    fn test_parameter_transform_from_bounds_selection() {
        // HEP typical bounds
        let bounds = vec![
            (0.0, 10.0),                        // POI: Sigmoid
            (1e-10, 10.0),                      // gamma: Sigmoid
            (-5.0, 5.0),                        // alpha: Sigmoid
            (f64::NEG_INFINITY, f64::INFINITY), // unbounded: Identity
        ];

        let t = ParameterTransform::from_bounds(&bounds);
        assert_eq!(t.dim(), 4);

        // Test roundtrip for the composite transform
