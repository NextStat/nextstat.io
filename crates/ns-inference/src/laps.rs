...
impl LapsModel {
    ...
    fn param_names(&self) -> Vec<String> {
        match self {
            ...
            | LapsModel::GlmPoisson { p, .. } => (0..*p).map(|i| format!("beta[{}]", i)).collect(),
...
fn should_skip_l_tuning(model: &LapsModel, config: &LapsConfig) -> bool {
    matches!(model, LapsModel::StdNormal { .. }) || (config.n_warmup <= 256 && model.is_glm())
}
...
fn should_use_fused_sampling<A: MamsAccelerator>(
    ...
    match model {
        ...
        | LapsModel::GlmNegBin { n, .. }
        | LapsModel::GlmComposedLogistic { n, .. }
            if *n >= 1024 && (accel.supports_warp() || accel.supports_warp_hi()) =>
        {
            false
        }
        _ => true,
