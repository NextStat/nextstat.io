...
impl DifferentiableSession {
    ...
    pub fn new(model: &HistFactoryModel, signal_sample_name: &str) -> Result<Self> {
        ...
        Ok(Self { accel, n_params, signal_n_bins: total_signal_bins, init_params })
    }
...
impl ProfiledDifferentiableSession {
    ...
    pub fn profiled_qmu_and_grad(
        ...
        let (nll_fixed, fixed_params) = self.profile_fit(signal_ptr, &bounds_mu, Some(&warm_mu))?;
    ...
    pub fn batch_profiled_qmu(
        ...
        mu_values.iter().map(|&mu| self.profiled_qmu_and_grad(mu, signal_ptr)).collect()
    }
}
