...
impl Default for BeConfig {
    fn default() -> Self {
        Self { alpha: 0.05, limits: (0.80, 1.25), design: BeDesign::Crossover2x2 }
    }
}
...
impl Default for RsabeConfig {
    fn default() -> Self {
        Self { alpha: 0.05, regulatory_constant: 0.8927593, swr_threshold: 0.294 }
    }
}
...
fn tost_power_integral(df: f64, tval: f64, a: f64, b: f64) -> f64 {
    ...
    let log_norm =
        (2.0_f64).ln() + half_df * half_df.ln() - statrs::function::gamma::ln_gamma(half_df);
...
fn validate_be_data(data: &BeData) -> Result<()> {
    ...
    {
        return Err(Error::Validation("All BeData vectors must have the same length".to_string()));
    }
    if data.log_value.iter().any(|v| !v.is_finite()) {
        return Err(Error::Validation("log_value must contain only finite values".to_string()));
    }
    // Check that treatments are 0 or 1.
    if data.treatment.iter().any(|&t| t > 1) {
        return Err(Error::Validation("treatment must be 0 (Reference) or 1 (Test)".to_string()));
    }
    Ok(())
}
...
pub fn average_be(data: &BeData, config: &BeConfig) -> Result<BeResult> {
    ...
    if config.limits.0 <= 0.0 || config.limits.1 <= config.limits.0 {
        return Err(Error::Validation("limits must satisfy 0 < lower < upper".to_string()));
    }
...
fn average_be_crossover_3x3(data: &BeData, config: &BeConfig) -> Result<BeResult> {
    ...
    for (&sid, test_vals) in &subject_test {
        let ref_vals = subject_ref.get(&sid).ok_or_else(|| {
            Error::Validation(format!("subject {} has Test but no Reference observations", sid))
        })?;
...
fn average_be_parallel(data: &BeData, config: &BeConfig) -> Result<BeResult> {
    ...
    let denom = (var_t / n_t).powi(2) / (n_t - 1.0) + (var_r / n_r).powi(2) / (n_r - 1.0);
    let df = num / denom;
...
pub fn reference_scaled_be(data: &BeData, config: &RsabeConfig) -> Result<RsabeResult> {
    ...
    if config.regulatory_constant <= 0.0 {
        return Err(Error::Validation("regulatory_constant must be positive".to_string()));
    }
    if config.swr_threshold <= 0.0 {
        return Err(Error::Validation("swr_threshold must be positive".to_string()));
    }
    ...
    let gmr = pe.exp();
    let pe_constraint_met = (0.80..=1.25).contains(&gmr);
    ...
    Ok(RsabeResult { scaled_criterion, upper_bound, swr, pe_constraint_met, conclusion })
}
...
pub fn be_power(n_total: usize, config: &BePowerConfig) -> Result<f64> {
    ...
    if n_total < 2 {
        return Err(Error::Validation("n_total must be at least 2".to_string()));
    }
...
pub fn be_sample_size(config: &BePowerConfig) -> Result<BePowerResult> {
    ...
    loop {
        ...
        if power >= config.target_power {
            let n_per_sequence = n_total.div_ceil(n_groups);
            ...
            return Ok(BePowerResult { n_per_sequence, n_total: n_total_balanced, achieved_power });
        }
...
fn validate_power_config(config: &BePowerConfig) -> Result<()> {
    ...
    if !(config.target_power > 0.0 && config.target_power < 1.0) {
        return Err(Error::Validation("target_power must be in (0, 1)".to_string()));
    }
    if !(config.cv > 0.0) {
...
fn design_se_df(n_total: usize, sigma_w: f64, design: BeDesign) -> Result<(f64, f64)> {
    match design {
        BeDesign::Crossover2x2 => {
            ...
            if n < 3.0 {
                return Err(Error::Validation("2x2 crossover requires n_total >= 3".to_string()));
            }
        ...
        BeDesign::Crossover3x3 => {
            ...
            let n = n_total as f64;
            if n < 4.0 {
                return Err(Error::Validation("3x3 crossover requires n_total >= 4".to_string()));
            }
        ...
        BeDesign::Parallel => {
            ...
            let n = n_total as f64;
            if n < 4.0 {
                return Err(Error::Validation("parallel design requires n_total >= 4".to_string()));
            }
...
mod tests {
    ...
    fn warfarin_auc_data() -> BeData {
        ...
        BeData { subject_id, sequence, period, treatment, log_value }
    }
    ...
    fn test_warfarin_auc_bioequivalent() {
        ...
        assert!(result.p_lower < 0.05, "p_lower = {} >= 0.05", result.p_lower);
        assert!(result.p_upper < 0.05, "p_upper = {} >= 0.05", result.p_upper);

        // df should be n - 1 = 11.
        assert!((result.df - 11.0).abs() < TOL, "df = {}, expected 11", result.df);
    }
    ...
    fn test_not_bioequivalent() {
        ...
        assert!(result.geometric_mean_ratio > 1.25, "GMR = {}", result.geometric_mean_ratio);
    ...
    fn test_exactly_on_boundary() {
        ...
        let data = BeData { subject_id, sequence, period, treatment, log_value };
        let config = BeConfig::default();
        let result = average_be(&data, &config).unwrap();
    ...
    fn test_parallel_design() {
        ...
        let data = BeData { subject_id, sequence, period, treatment, log_value };
        let config = BeConfig { design: BeDesign::Parallel, ..BeConfig::default() };
        let result = average_be(&data, &config).unwrap();
    ...
    fn test_3x3_crossover() {
        ...
        let config = BeConfig { design: BeDesign::Crossover3x3, ..BeConfig::default() };
        let result = average_be(&data, &config).unwrap();
    ...
    fn test_rsabe_high_variability() {
        ...
        let data = BeData {
            subject_id: vec![1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6],
    ...
    fn test_power_increases_with_n() {
        let config = BePowerConfig { cv: 0.30, gmr: 0.95, ..BePowerConfig::default() };

        let p10 = be_power(10, &config).unwrap();
        let p24 = be_power(24, &config).unwrap();
        let p50 = be_power(50, &config).unwrap();

        assert!(p24 > p10, "power should increase with n: p(10)={}, p(24)={}", p10, p24);
        assert!(p50 > p24, "power should increase with n: p(24)={}, p(50)={}", p24, p50);
    }
    ...
    fn test_power_gmr_1_cv30_n40() {
        ...
        let config = BePowerConfig { cv: 0.30, gmr: 1.00, ..BePowerConfig::default() };
        let power = be_power(40, &config).unwrap();
        assert!(power > 0.85, "power at N=40, GMR=1.0, CV=30% should be > 0.85, got {:.4}", power);
    }
    ...
    fn test_tost_power_debug() {
        ...
        assert!((power40 - 0.82).abs() < 0.05, "power at N=40 should be ~0.82, got {:.4}", power40);
    }
    ...
    fn test_power_gmr_1_is_highest() {
        let config_095 = BePowerConfig { cv: 0.25, gmr: 0.95, ..BePowerConfig::default() };
        let config_100 = BePowerConfig { cv: 0.25, gmr: 1.00, ..BePowerConfig::default() };
        let config_105 = BePowerConfig { cv: 0.25, gmr: 1.05, ..BePowerConfig::default() };
        ...
        assert!(p100 >= p095, "power(GMR=1.0) = {} should >= power(GMR=0.95) = {}", p100, p095);
        assert!(p100 >= p105, "power(GMR=1.0) = {} should >= power(GMR=1.05) = {}", p100, p105);
    }
    ...
    fn test_invalid_alpha_rejected() {
        let data = warfarin_auc_data();
        let config = BeConfig { alpha: 0.0, ..BeConfig::default() };
        assert!(average_be(&data, &config).is_err());

        let config2 = BeConfig { alpha: 1.0, ..BeConfig::default() };
    ...
    fn test_power_invalid_cv() {
        let config = BePowerConfig { cv: 0.0, ..BePowerConfig::default() };
    ...
    fn test_cv_to_sigma_w_known() {
        ...
        assert!((sigma - 0.2936).abs() < 0.001, "sigma_w = {}, expected ~0.2936", sigma);
    }
    ...
    fn test_ci_back_transform_consistency() {
        ...
        assert!((result.ci_lower - ci_log_lower.exp()).abs() < 1e-12, "CI lower mismatch");
        assert!((result.ci_upper - ci_log_upper.exp()).abs() < 1e-12, "CI upper mismatch");
    }
    ...
    fn test_sample_size_high_cv() {
        ...
        let config = BePowerConfig { cv: 0.40, gmr: 1.00, ..BePowerConfig::default() };
        let result = be_sample_size(&config).unwrap();
        ...
        let config_30 = BePowerConfig { cv: 0.30, gmr: 1.00, ..BePowerConfig::default() };
