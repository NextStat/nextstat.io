...
mod tests {
    ...
    fn scm_deterministic_forward_selection() {
        ...
        let (times, y, subject_idx) = generate_pk_data(
            n_subjects, &times_per, 1.2,   // cl_pop
            15.0,  // v_pop
            2.0,   // ka_pop
            100.0, // dose
            1.0,   // bioav
            ...
            &weights, wt_center, 0.75, // true wt exponent
    ...
    fn scm_deterministic_backward_elimination() {
        ...
        let (times, y, subject_idx) = generate_pk_data(
            n_subjects, &times_per, 1.2, 15.0, 2.0, 100.0, 1.0, 0.05, 0.20, &weights, 70.0,
    ...
    fn scm_no_covariates_selected() {
        ...
        let noise_cov1: Vec<f64> =
            (0..n_subjects).map(|i| 60.0 + 20.0 * (i as f64 / n_subjects as f64)).collect();
        let noise_cov2: Vec<f64> = (0..n_subjects)
            .map(|i| 30.0 + 10.0 * ((n_subjects - i) as f64 / n_subjects as f64))
            .collect();
    ...
    fn scm_multiple_covariates() {
        ...
        assert!(!result.forward_trace.is_empty(), "forward trace should not be empty");
        assert_eq!(
            result.forward_trace[0].name, "WT_on_CL",
