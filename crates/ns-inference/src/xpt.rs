...
pub fn ibm_to_ieee(bytes: [u8; 8]) -> Option<f64> {
    ...
    if ieee_exp >= 2047 {
        // Overflow
        let val = if sign == 1 { f64::NEG_INFINITY } else { f64::INFINITY };
        return Some(val);
    }
    ...
    let ieee_bits = ((sign as u64) << 63) | ((ieee_exp as u64) << 52) | mantissa;
...
pub fn read_xpt_bytes(data: &[u8]) -> Result<Vec<XptDataset>> {
    ...
    loop {
        ...
        let ds_name = member_desc.get(8..16).unwrap_or("").trim().to_string();

        // Member header continued (80 bytes): dataset label + type
        let member_cont = read_record(data, &mut pos)?;
        let ds_label = member_cont.get(0..40).unwrap_or("").trim().to_string();
        ...
        let nvar_str = namestr_hdr.get(54..58).unwrap_or("0").trim();
            ...
            for var in &variables {
                ...
                match var.var_type {
                    ...
                    XptVarType::Character => {
                        let s = String::from_utf8_lossy(cell_bytes).trim_end().to_string();
        ...
        datasets.push(XptDataset { name: ds_name, label: ds_label, variables, data: rows });
    }
...
pub fn xpt_to_nonmem(dataset: &XptDataset) -> Result<NonmemDataset> {
    let col_names: Vec<String> = dataset.variables.iter().map(|v| v.name.to_uppercase()).collect();
    ...
    for row in &dataset.data {
        ...
        let amt = amt_col.and_then(|c| value_to_f64(&row[c])).unwrap_or(0.0);
        let evid = evid_col
        ...
        let cmt = cmt_col.and_then(|c| value_to_f64(&row[c])).map(|v| v as u8).unwrap_or(1);
        let rate = rate_col.and_then(|c| value_to_f64(&row[c])).unwrap_or(0.0);
...
fn parse_namestr(ns: &[u8]) -> Result<XptVariable> {
    ...
    let var_type = match ntype {
        ...
        _ => {
            return Err(Error::Validation(format!("unknown variable type {} in namestr", ntype)));
        }
    };
...
mod tests {
    ...
    fn test_ibm_to_ieee_pi() {
        ...
        let val = ibm_to_ieee(ibm).unwrap();
        assert!((val - std::f64::consts::PI).abs() < 1e-7, "expected pi, got {val}");
    }
    ...
    fn test_ibm_to_ieee_large() {
        ...
        let val = ibm_to_ieee(ibm).unwrap();
        assert!((val - 1e10).abs() / 1e10 < 1e-7, "expected 1e10, got {val}");
    }
    ...
    fn test_ieee_to_ibm_roundtrip() {
        ...
        for &original in &test_values {
            ...
            match roundtripped {
                Some(val) => {
                    if original == 0.0 {
                        assert!(val.abs() < 1e-30, "roundtrip of 0.0 gave {val}");
                    } else {
    ...
    fn test_ieee_to_ibm_infinity_is_missing() {
        ...
        assert_eq!(ibm_to_ieee(ibm), None, "Infinity should encode as SAS missing");
    }
    ...
    fn test_write_read_roundtrip_tempfile() {
        let ds = XptDataset {
            ...
            data: vec![
                vec![XptValue::Character("Headache".to_string()), XptValue::Numeric(1.0)],
                vec![XptValue::Character("Nausea".to_string()), XptValue::Numeric(2.0)],
            ],
        };
    ...
    fn test_multiple_datasets() {
        ...
        let ds2 = XptDataset {
            ...
            data: vec![vec![XptValue::Numeric(1.0)], vec![XptValue::Numeric(3.0)]],
        };
