...
pub fn generate_virtual_population(
    ...
    for _ in 0..n_subjects {
        // Draw z ~ N(0, I).
        let z: Vec<f64> = (0..n_params).map(|_| rng.sample::<f64, _>(StandardNormal)).collect();
        ...
        let eta = match &chol {
            ...
            None => {
                ...
                z.iter().zip(&population.omega).map(|(&zi, oi)| oi * zi).collect()
            }
        };
...
pub fn simulate_trial(config: &TrialConfig) -> Result<TrialResult> {
    ...
    if config.obs_times.is_empty() {
        return Err(Error::Validation("obs_times must be non-empty".to_string()));
    }
    if config.n_subjects == 0 {
        return Err(Error::Validation("n_subjects must be > 0".to_string()));
    }
    if config.dosing.is_empty() {
        return Err(Error::Validation("dosing must contain at least one dose event".to_string()));
    }
    ...
    Ok(TrialResult { concentrations, individual_params, endpoints })
}
...
fn compute_endpoints(concentrations: &[Vec<f64>], obs_times: &[f64]) -> TrialEndpoints {
    ...
    TrialEndpoints { auc, cmax, tmax, ctrough }
}
...
pub fn simulate_trials(config: &MonteCarloConfig) -> Result<MonteCarloResult> {
    ...
    let pta = config.targets.as_ref().map(|targets| compute_pta(&results, targets));

    let wall_s = start.elapsed().as_secs_f64();

    Ok(MonteCarloResult { n_trials: config.n_trials, trial_summaries, pta, wall_s })
}
...
fn trial_summary(result: &TrialResult) -> TrialSummary {
    ...
    TrialSummary { mean_auc, median_auc, mean_cmax, median_cmax, n_subjects: n }
}
...
fn compute_pta(results: &[TrialResult], targets: &PtaTargets) -> PtaResult {
    ...
    for result in results {
        ...
        if let Some(cmax_max) = targets.cmax_max {
            cmax_hits += result.endpoints.cmax.iter().filter(|&&c| c < cmax_max).count();
        }
        if let Some(ctrough_min) = targets.ctrough_min {
            ctrough_hits += result.endpoints.ctrough.iter().filter(|&&c| c > ctrough_min).count();
        }
    }

    let frac = |hits: usize| -> f64 {
        if total_subjects > 0 { hits as f64 / total_subjects as f64 } else { 0.0 }
    };
...
pub fn find_optimal_dose(
    ...
    if !(0.0..=1.0).contains(&target_pta) {
        return Err(Error::Validation(format!("target_pta must be in [0, 1], got {}", target_pta)));
    }

    // Generate evenly spaced dose levels.
    let step = (dose_range.1 - dose_range.0) / (n_dose_levels - 1) as f64;
    let dose_levels: Vec<f64> =
        (0..n_dose_levels).map(|i| dose_range.0 + step * i as f64).collect();
    ...
    Ok(DoseOptResult { optimal_dose, achieved_pta, dose_pta_curve })
}
...
fn median(values: &[f64]) -> f64 {
    ...
    let n = sorted.len();
    if n.is_multiple_of(2) { 0.5 * (sorted[n / 2 - 1] + sorted[n / 2]) } else { sorted[n / 2] }
}
...
mod tests {
    ...
    fn test_simulate_single_trial() {
        ...
        for &t in &result.endpoints.tmax {
            assert!((0.5..=24.0).contains(&t), "Tmax out of range: {t}");
        }
    }
    ...
    fn test_monte_carlo_deterministic() {
        let mc_config = MonteCarloConfig {
            ...
            targets: Some(PtaTargets { auc_min: Some(100.0), cmax_max: None, ctrough_min: None }),
            n_threads: 0,
        };
    ...
    fn test_correlated_random_effects() {
        ...
        assert!(corr > 0.3, "CL-V correlation should be positive (~0.5), got {corr}");
        ...
        assert!(corr_cl_ka.abs() < 0.15, "CL-Ka correlation should be ~0, got {corr_cl_ka}");
    }
    ...
    fn test_2cpt_iv_trial() {
        let config = TrialConfig {
            n_subjects: 30,
            dosing: vec![DoseEvent { time: 0.0, amount: 100.0, route: DoseRoute::IvBolus }],
    ...
    fn test_2cpt_oral_trial() {
        let config = TrialConfig {
            ...
            dosing: vec![DoseEvent {
                time: 0.0,
                amount: 200.0,
                route: DoseRoute::Oral { bioavailability: 0.7 },
            }],
            obs_times: vec![0.5, 1.0, 2.0, 4.0, 6.0, 8.0, 12.0, 24.0],
            ...
            population: PopulationPkParams {
                ...
                sigma: 0.08,
                error_model: ErrorModelType::Combined { prop_fraction: 0.6 },
                omega_correlation: None,
            },
            seed: 42,
        };
