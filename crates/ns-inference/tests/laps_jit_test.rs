...
fn test_laps_multi_gpu_consistency() {
    ...
    println!("{:<6} {:<12} {:<12} {:<12} {:<12}", "Param", "Mean(1GPU)", "Mean(MGPU)", "Var(1GPU)", "Var(MGPU)");
    println!("{}", "-".repeat(56));
    for p in 0..dim {
...
fn bench_laps_4x_gpu() {
    ...
    for &(n_chains, dev_ids) in configs {
        ...
        let speedup = baseline_1gpu
            .get(&n_chains)
            .map(|&base| base / elapsed)
            .unwrap_or(1.0);

        let speedup_str = if n_gpus == 1 {
            "1.00x".to_string()
        } else {
            format!("{:.2}x", speedup)
        };
