...
use ns_inference::{
    ...
    pk::{ErrorModel, conc_iv_2cpt_macro},
...
fn parse_theophylline() -> (Vec<f64>, Vec<f64>, Vec<usize>, usize, f64) {
    ...
    let doses: Vec<f64> = ds
        .subject_ids()
        .iter()
        .map(|id| ds.subject_records(id).iter().filter(|r| r.evid == 1).map(|r| r.amt).sum::<f64>())
        .collect();
...
fn generate_warfarin_synthetic(n_subjects: usize, seed: u64) -> (Vec<f64>, Vec<f64>, Vec<usize>) {
    let cl_pop = 0.134;
    let v_pop = 8.0;
    let ka_pop = 1.0;
    let omega_cl = 0.20;
...
fn parity_warfarin_saem_vs_foce() {
    ...
    for (i, &label) in labels.iter().enumerate() {
        let diff =
            (saem_result.theta[i] - foce_result.theta[i]).abs() / foce_result.theta[i].max(1e-10);
        println!(
            "  {:<9} | {:.4}   | {:.4}   | {:.1}%",
            label,
            foce_result.theta[i],
...
fn generate_phase1_iv_2cpt(n_subjects: usize, seed: u64) -> (Vec<f64>, Vec<f64>, Vec<usize>) {
    let cl_pop = 5.0;
    let v1_pop = 10.0;
    let q_pop = 15.0;
    let v2_pop = 20.0;
...
fn parity_phase1_iv_2cpt_saem() {
    ...
    let true_vals = [phase1_iv_ref::CL, phase1_iv_ref::V1, phase1_iv_ref::Q, phase1_iv_ref::V2];
    for (i, &label) in labels.iter().enumerate() {
        let rel_diff = (result.theta[i] - true_vals[i]).abs() / true_vals[i];
        println!(
            "  {:<9} | {:.4}   | {:.4}   | {:.1}%",
...
fn parity_combined_summary() {
    ...
    let (theo_saem, _) = saem
        .fit_1cpt_oral(
            ...
            &theo_doses,
            1.0,
            ErrorModel::Proportional(0.7),
            &[0.04, 0.45, 1.5],
            &[0.30, 0.25, 0.50],
        )
        .expect("Theophylline SAEM");
    let theo_saem_ok = theo_saem.converged
        && theo_saem.theta[0] > 0.02
        && theo_saem.theta[0] < 0.08
        && theo_saem.theta[1] > 0.20
        && theo_saem.theta[1] < 0.80
        && theo_saem.theta[2] > 0.5
        && theo_saem.theta[2] < 5.0;
    println!(
    ...
    let theo_foce_ok = theo_foce.ofv.is_finite()
        && theo_foce.theta[0] > 0.02
        && theo_foce.theta[0] < 0.08
        && theo_foce.theta[1] > 0.20
        && theo_foce.theta[1] < 0.80
        && theo_foce.theta[2] > 0.5
        && theo_foce.theta[2] < 5.0;
    println!(
    ...
    let warf_foce_ok = warf_foce.ofv.is_finite()
        && warf_foce.theta[0] > 0.0
        && warf_foce.theta[0].is_finite()
    ...
    let iv_saem_ok =
        iv_saem.converged && (iv_saem.theta[0] - 5.0).abs() / 5.0 < 0.60 && iv_saem.theta[0] > 0.0;
    println!(
