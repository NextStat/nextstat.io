...
impl CudaMamsAccelerator {
    ...
    pub fn new_on_device(
        ...
        let (kernel, kernel_fused, kernel_warp, kernel_warp_hi) =
            match load_precompiled_mams_kernels(&ctx) {
                Ok(kernels) => kernels,
                Err(pre_err) => {
                    log::warn!(
    ...
    pub fn transition_warp(
        ...
        let kernel =
            self.kernel_warp.as_ref().ok_or_else(|| cuda_err("warp kernel not available"))?.clone();
