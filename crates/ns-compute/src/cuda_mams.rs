...
impl CudaMamsAccelerator {
    ...
    /// - `n_steps`: leapfrog steps per transition
    ...
    pub fn transition(
        ...
        let n_steps_arg = n_steps as i32;
    ...
    ///
    /// The kernel accumulates positions and diagnostics for the first `n_report`
    /// chains into GPU-side buffers. After all launches, one bulk download.
    ///
