...
impl MetalUnbinnedToySampler {
    ...
    pub fn from_unbinned_static(data: &UnbinnedGpuModelData) -> ns_core::Result<Self> {
        ...
        let buf_params =
            device.new_buffer((data.n_params * mem::size_of::<f32>()).max(4) as u64, opts);
        let buf_counts_cache =
            device.new_buffer((DEFAULT_TOY_CAPACITY * mem::size_of::<u32>()).max(4) as u64, opts);
        let toy_offsets_len = DEFAULT_TOY_CAPACITY + 1;
        let buf_toy_offsets_cache =
            device.new_buffer((toy_offsets_len * mem::size_of::<u32>()).max(4) as u64, opts);
    ...
    fn prepare_params_and_counts<'a>(
        ...
        if n_toys <= self.cached_n_toys {
            ...
            let opts = MTLResourceOptions::StorageModeShared;
            let fresh =
                self.device.new_buffer((n_toys * mem::size_of::<u32>()).max(4) as u64, opts);
    ...
    fn sample_toys_1d_fused(
        ...
        let mut timing = MetalToySampleTiming { fused: true, ..Default::default() };
        let opts = MTLResourceOptions::StorageModeShared;
        ...
        if offsets_len <= self.cached_toy_offsets_len {
            ...
            let fresh =
                self.device.new_buffer((offsets_len * mem::size_of::<u32>()).max(4) as u64, opts);
        ...
        let buf_obs_out =
            self.device.new_buffer((capacity * mem::size_of::<f32>()).max(4) as u64, opts);
        ...
        let scalar_sample = ToyScalarArgs { obs_capacity: capacity as u32, ..scalar_counts };
