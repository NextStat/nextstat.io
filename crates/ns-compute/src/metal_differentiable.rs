...
impl MetalDifferentiableAccelerator {
    ...
    pub fn from_metal_data(
        ...
        let buf_observed = device.new_buffer((data.n_main_bins * mem::size_of::<f32>()) as u64, opts);
        let buf_ln_facts = device.new_buffer((data.n_main_bins * mem::size_of::<f32>()) as u64, opts);
        let buf_obs_mask = device.new_buffer((data.n_main_bins * mem::size_of::<f32>()) as u64, opts);
        let buf_nll_out = device.new_buffer(mem::size_of::<f32>() as u64, opts);
        let buf_grad_params_out = device.new_buffer((data.n_params * mem::size_of::<f32>()) as u64, opts);
    ...
    pub fn nll_grad_all(&mut self, params: &[f64]) -> ns_core::Result<(f64, Vec<f64>, Vec<f64>)> {
        ...
        let grad_signal = Self::read_buffer_f32_to_f64(&self.buf_grad_signal_out, self.total_signal_bins);
