...
impl CudaBatchAccelerator {
    ...
    pub fn from_gpu_data(data: &GpuModelData, max_batch: usize) -> ns_core::Result<Self> {
        ...
        let d_params = stream.alloc_zeros::<f64>(max_batch * data.n_params).map_err(cuda_err)?;
        let d_observed =
            stream.alloc_zeros::<f64>(max_batch * data.n_main_bins).map_err(cuda_err)?;
        let d_ln_facts =
            stream.alloc_zeros::<f64>(max_batch * data.n_main_bins).map_err(cuda_err)?;
        let d_obs_mask =
            stream.alloc_zeros::<f64>(max_batch * data.n_main_bins).map_err(cuda_err)?;
        let d_nll_out = stream.alloc_zeros::<f64>(max_batch).map_err(cuda_err)?;
        let d_grad_out = stream.alloc_zeros::<f64>(max_batch * data.n_params).map_err(cuda_err)?;
    ...
    pub fn upload_observed(
        ...
        self.stream.memcpy_htod(&observed_flat[..n], &mut self.d_observed).map_err(cuda_err)?;
        self.stream.memcpy_htod(&ln_facts[..n], &mut self.d_ln_facts).map_err(cuda_err)?;
        self.stream.memcpy_htod(&obs_mask[..n], &mut self.d_obs_mask).map_err(cuda_err)?;
        Ok(())
    }
    ...
    pub fn batch_nll_grad(
        ...
        self.stream.memcpy_htod(params_flat, &mut self.d_params).map_err(cuda_err)?;

        // Zero gradient output by re-uploading zeros
        let zeros = vec![0.0f64; n_active * self.n_params];
        self.stream.memcpy_htod(&zeros, &mut self.d_grad_out).map_err(cuda_err)?;
