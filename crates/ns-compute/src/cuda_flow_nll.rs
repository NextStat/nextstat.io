...
impl CudaFlowNllAccelerator {
    ...
    /// Compute NLL + analytical gradient intermediates from GPU-resident `float` buffers.
    ///
    /// This is the zero-copy analytical gradient path for CUDA EP: the `log_prob_grad`
    /// ONNX model outputs both `log_prob` and `d_log_prob_d_context` as device-resident
    /// `float` tensors, and this method reads them directly without any H2D copy.
    ///
    /// Returns `(nll, sum_r, sum_jr)` — same semantics as [`nll_grad`].
    ///
    /// # Arguments
    ///
    /// - `d_logp_flat_ptr`: raw CUDA device pointer to `float[n_procs × n_events]`
    /// - `d_jac_flat_ptr`: raw CUDA device pointer to `float[n_procs × n_events × n_context]`
    ///   Pass `0` (null) if `n_context == 0`.
    /// - `yields`: `[n_procs]` per-process yields (host).
    /// - `params`: `[n_params]` current parameter values (host, for constraints).
    pub fn nll_grad_device_ptr_f32(
        &mut self,
        d_logp_flat_ptr: u64,
        d_jac_flat_ptr: u64,
        yields: &[f64],
        ...
        // H2D: yields + params (small buffers).
        self.stream.memcpy_htod(yields, &mut self.d_yields).map_err(cuda_err)?;
        if self.n_params > 0 {
            self.stream.memcpy_htod(params, &mut self.d_params).map_err(cuda_err)?;
        }
        ...
        let mut builder = self.stream.launch_builder(&self.kernel_grad_reduce_f32);
        builder.arg(&d_logp_flat_ptr);
        builder.arg(&d_jac_flat_ptr);
        builder.arg(&self.d_yields);
        builder.arg(&self.d_gauss);
        builder.arg(&self.d_params);
        builder.arg(&mut self.d_nll_out);
        ...
        // SAFETY: We pass raw CUDA device pointers (as u64) for logp and jac args;
        // the kernel expects `const float*` and the ABI is the same (64-bit pointers).
        // Other args are owned CudaSlice allocations or scalars matching the signature.
        unsafe {
            builder
                .launch(config)
                .map_err(|e| cuda_err(format!("launch flow_nll_grad_reduce_f32: {e}")))?;
        }
