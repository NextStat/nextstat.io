...
/**
 * Same as flow_batch_nll_reduce, but consumes log-prob as f32.
 *
 * Preferred for CUDA EP I/O binding path where ONNX Runtime produces float
 * outputs that stay device-resident (zero H2D copy).
 */
extern "C" __global__ void flow_batch_nll_reduce_f32(
    const float* __restrict__ g_logp_flat,      /* [n_procs × total_events] */
    const unsigned int* __restrict__ g_toy_offsets, /* [n_toys + 1] */
    const double* __restrict__ g_yields_flat,   /* [n_toys × n_procs] */
    const struct FlowGaussConstraintEntry* __restrict__ g_gauss, /* [n_gauss] */
    const double* __restrict__ g_params_flat,   /* [n_toys × n_params] */
    ...
    for (unsigned int i = tid; i < n_events; i += block_size) {
        ...
        for (unsigned int p = 0; p < n_procs; p++) {
            ...
            double logp = (double)g_logp_flat[(size_t)p * (ev_end - g_toy_offsets[0]) + global_ev - g_toy_offsets[0]];
            if (!isfinite(logp)) {
                continue;
            }
