...
__device__ double user_nll_warp(
    ...
    int /*n_obs*/, int /*n_feat*/, int /*lane_id*/)
{
...
extern "C" __global__ void mams_transition_warp(
    ...
    // --- Cooperative shared memory load: transpose X to column-major ---
    ...
    for (int s = 0; s < n_steps; s++) {
        ...
        user_grad_warp(x, grad_reg, dim, model_data, s_X_col, s_y, n_obs, n_feat, lane_id);
        potential = user_nll_warp(x, dim, model_data, s_X_col, s_y, n_obs, n_feat, lane_id);
        // b_step again
        total_delta_k += b_step(u, grad_reg, inv_mass, eps * 0.5, dim, dm1);
