...
fn metal_toy_sampler_multiproc_deterministic() {
    ...
    for (i, (&a, &b)) in xs1.iter().zip(xs2.iter()).enumerate() {
        assert_eq!(a.to_bits(), b.to_bits(), "sample {i} not bit-exact: {a} vs {b}");
    }
...
fn metal_toy_sampler_fused_parity() {
    ...
    let (offs_two, xs_two) =
        sampler.sample_toys_1d_two_pass_for_test(&params, n_toys, seed).unwrap();

    assert_eq!(offs_fused, offs_two, "toy offsets differ between fused and two-pass");
    assert_eq!(
        xs_fused.len(),
        xs_two.len(),
    ...
    for (i, (&a, &b)) in xs_fused.iter().zip(xs_two.iter()).enumerate() {
        assert_eq!(a.to_bits(), b.to_bits(), "sample {i} not bit-exact: fused={a} vs two_pass={b}");
    }

    println!("fused parity: {} toys, {} events, bit-exact OK", n_toys, xs_fused.len());
}
...
fn metal_toy_sampler_fused_timing() {
    ...
    for i in 0..n_iters {
        let (_offs, _xs, timing) =
            sampler.sample_toys_1d_timed(&params, n_toys, i as u64 + 1000).unwrap();
        let wall = timing.counts_kernel_s
            + timing.counts_readback_s
            + timing.prefix_sum_s
