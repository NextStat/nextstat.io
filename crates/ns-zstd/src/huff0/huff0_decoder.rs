...
impl HuffmanTable {
    ...
    fn read_weights(&mut self, source: &[u8]) -> Result<u32, HuffmanTableError> {
            ...
            0..=127 => {
                ...
                loop {
                    ...
                    if self.weights.len() > 255 {
                        return Err(err::TooManyWeights { got: self.weights.len() });
                    }
                }
            }
        ...
        let bytes_read = if bits_read % 8 == 0 { bits_read / 8 } else { (bits_read / 8) + 1 };
    ...
    fn build_table_from_weights(&mut self) -> Result<(), HuffmanTableError> {
        ...
        for symbol in 0..self.weights.len() {
            let bits =
                if self.weights[symbol] > 0 { max_bits + 1 - self.weights[symbol] } else { 0 };
