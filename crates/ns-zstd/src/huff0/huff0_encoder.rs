...
impl HuffmanTable {
    ...
    pub fn build_from_weights(weights: &[usize]) -> Self {
        ...
        for (symbol, weight) in weights.iter().copied().enumerate() {
            if weight > 0 {
                sorted.push(SortEntry { symbol: symbol as u8, weight });
            }
        }
        ...
        let mut table = HuffmanTable { codes: Vec::with_capacity(weights.len()) };
    ...
    #[inline]
    pub fn can_encode_data(&self, data: &[u8]) -> bool {
        // NOTE: `HuffmanTable` is currently sized to `max_symbol + 1` for the block it was built from.
        // Treeless literals are only valid if all symbols in `data` have a non-zero code in this table.
        for &b in data {
            let idx = b as usize;
            if idx >= self.codes.len() {
                return false;
            }
            if self.codes[idx].1 == 0 {
                return false;
            }
        }
        true
    ...
    // NOTE: `can_encode` used to be a heuristic helper comparing 2 tables. It was removed because it
    // ended up unused after introducing `can_encode_data` (treeless fast-path) and produced warnings
    // under `-D warnings`.
}
...
fn redistribute_weights(weights: &mut [usize], max_num_bits: usize) {
    let weight_sum_log = weights.iter().copied().map(|x| 1 << x).sum::<usize>().ilog2() as usize;
