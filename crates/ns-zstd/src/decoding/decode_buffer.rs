...
impl DecodeBuffer {
    ...
    pub fn repeat(&mut self, offset: usize, match_length: usize) -> Result<(), DecodeBufferError> {
        if offset > self.buffer.len() {
            ...
            if end_idx > buf_len {
                ...
                unsafe { self.buffer.extend_from_within_unchecked(start_idx, match_length) };
            }
    ...
    fn repeat_in_chunks(&mut self, offset: usize, match_length: usize, start_idx: usize) {
        // Exponential growth: after each copy, the pattern is doubled in the buffer,
        // so we can copy twice as much next time. This reduces iterations from
        // match_length/offset to log2(match_length/offset).
        let mut remaining = match_length;
        let mut available = offset;

        while remaining > 0 {
            let copy = usize::min(available, remaining);

            // SAFETY: Requirements checked:
            // 1. start_idx + copy <= self.buffer.len()
            //      Initially start_idx = buffer.len() - offset and copy <= offset, so
            //      start_idx + copy <= buffer.len(). After each extend, buffer.len()
            //      grows by the same amount as `available`, keeping the invariant.
            // 2. Enough memory was reserved for the whole match_length upfront.
            unsafe { self.buffer.extend_from_within_unchecked(start_idx, copy) };
            remaining -= copy;
            available += copy;
        }
    }
    ...
    fn repeat_from_dict(
        ...
        if self.total_output_counter <= self.window_size as u64 {
            ...
            Err(DecodeBufferError::OffsetTooBig { offset, buf_len: self.buffer.len() })
        }
    }
    ...
    fn drain_to(
        ...
        let mut drain_guard = DrainGuard { buffer: &mut self.buffer, amount: 0 };
...
mod tests {
    ...
    fn short_writer() {
        ...
        let mut short_writer = ShortWriter { buf: vec![], write_len: 10 };
        ...
        for _ in 0..repeats {
            ...
            decode_buf.drain_to_window_size_writer(&mut short_writer).unwrap();
            assert_eq!(decode_buf.len(), 100);
        }
    ...
    fn wouldblock_writer() {
        ...
        let mut short_writer = WouldblockWriter { buf: vec![], last_blocked: 0, block_every: 5 };
