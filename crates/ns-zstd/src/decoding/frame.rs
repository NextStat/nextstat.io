...
pub fn read_frame_header(mut r: impl Read) -> Result<(FrameHeader, u8), ReadFrameHeaderError> {
    ...
    if (0x184D2A50..=0x184D2A5F).contains(&magic_num) {
        r.read_exact(&mut buf).map_err(err::FrameDescriptorReadError)?;
        let skip_size = u32::from_le_bytes(buf);
        return Err(ReadFrameHeaderError::SkipFrame { magic_number: magic_num, length: skip_size });
    }
    ...
    r.read_exact(&mut buf[0..1]).map_err(err::FrameDescriptorReadError)?;
    ...
    if !desc.single_segment_flag() {
        r.read_exact(&mut buf[0..1]).map_err(err::WindowDescriptorReadError)?;
    ...
    if fcs_len != 0 {
        ...
        r.read_exact(fcs_buf).map_err(err::FrameContentSizeReadError)?;
        bytes_read += fcs_len;
        let mut fcs = 0u64;
