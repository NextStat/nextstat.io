...
const DEFAULT_MAX_CHAIN_DEPTH: usize = 2;
...
impl Matcher for MatchGeneratorDriver {
    fn reset(&mut self, level: CompressionLevel) {
        ...
        self.hc_generator.reset(|mut data, mut store| {
    ...
    fn commit_space(&mut self, space: Vec<u8>) {
        if self.use_hash_chain {
            let vec_pool = &mut self.vec_pool;
            let mut store = self.hc_pool.pop().unwrap_or_else(|| {
                HashChainStore::new(space.len(), DEFAULT_HASH_LOG, DEFAULT_MAX_CHAIN_DEPTH)
            });
            store.reset_for_len(space.len());
            let hc_pool = &mut self.hc_pool;
            self.hc_generator.add_data(space, store, |mut data, mut store| {
                #[allow(clippy::uninit_vec)]
                unsafe {
                    data.set_len(data.capacity());
                }
    ...
    fn start_matching(&mut self, mut handle_sequence: impl for<'a> FnMut(Sequence<'a>)) {
        if self.use_hash_chain {
            self.hc_generator.start_matching(&mut handle_sequence);
        } else {
