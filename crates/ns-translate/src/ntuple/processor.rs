        ...
        for modifier in &sample.modifiers {
            match modifier {
                ...
                NtupleModifier::HistoSys {
                    ...
                    let file_up_path = file_up
                        .as_ref()
                        .map(|f| self.base_dir.join(f))
                        .unwrap_or_else(|| self.base_dir.join(&sample.file));
                    let file_down_path = file_down
                        .as_ref()
                        .map(|f| self.base_dir.join(f))
                        .unwrap_or_else(|| self.base_dir.join(&sample.file));

                    let open_or_cached = |path: &std::path::Path,
                                          cache: &mut HashMap<PathBuf, RootFile>|
                     -> Result<()> {
                        let canonical = path.to_path_buf();
                        if let std::collections::hash_map::Entry::Vacant(e) = cache.entry(canonical)
                        {
                            let rf = RootFile::open(path).map_err(|e| {
                                Error::RootFile(format!("opening {}: {}", path.display(), e))
                            })?;
                            e.insert(rf);
                        }
                        Ok(())
                    ...
                    let hist_up = root_cache
                        ...
                        .map_err(|e| {
                            Error::RootFile(format!(
                                ...
                                histo_name_up,
                                file_up_path.display(),
                                e
                            ))
                        })?;
                    let hist_down = root_cache
                        .get(&file_down_path)
                        .unwrap()
