...
fn parse_region_block(b: &RawBlock, hist_filter_only: bool) -> Result<TrexRegion> {
    ...
    fn parse_variable_spec(value: &str) -> Option<(String, Vec<f64>)> {
        ...
        let nums: Vec<f64> =
            toks[1..].iter().filter_map(|s| s.trim().parse::<f64>().ok()).collect();
    ...
    let (variable, binning) = if let Some(binning_str) =
        last_attr_value(&b.attrs, "Binning").or_else(|| last_attr_value(&b.attrs, "BinEdges"))
    {
    ...
    for b in blocks {
        ...
        if b.kind == BlockKind::Systematic && syst_kind == Some(SystKind::Weight) {
            ...
            if !(has_explicit_up || has_explicit_down) {
                let base =
                    last_attr(&b.attrs, "WeightBase").or_else(|| last_attr(&b.attrs, "Weight"));
...
fn parse_sample_block(b: &RawBlock, hist_filter_only: bool) -> Result<TrexSample> {
    ...
    let weight =
        last_attr_value(&b.attrs, "Weight").or_else(|| last_attr_value(&b.attrs, "MCweight"));
...
fn parse_systematic_block(b: &RawBlock) -> Result<TrexSystematic> {
    ...
    let kind = if let Some(type_str) = last_attr_value(&b.attrs, "Type") {
        ...
        let has_norm = ["OverallUp", "OverallDown", "Lo", "Hi", "Up", "Down"]
            ...
            .any(|k| has_attr(&b.attrs, k));
        let has_tree =
            ["FileUp", "UpFile", "FileDown", "DownFile"].iter().any(|k| has_attr(&b.attrs, k));
...
fn sys_to_modifier(
    ...
    match sys.kind {
        ...
        SystKind::Weight => {
            ...
            let up = expr_mul(
                [external_weight.map(|s| s.to_string()), Some(up_raw)].into_iter().flatten(),
            )
            .ok_or_else(|| {
                Error::Validation(format!("Systematic '{}' empty WeightUp", sys.name))
            })?;
            let down = expr_mul(
                [external_weight.map(|s| s.to_string()), Some(down_raw)].into_iter().flatten(),
            )
            .ok_or_else(|| {
...
pub fn workspace_from_config(cfg: &TrexConfig, base_dir: &Path) -> Result<Workspace> {
    ...
    for region in &cfg.regions {
        let variable = region.variable.as_deref().ok_or_else(|| {
            Error::Validation(format!(
                "Region '{}' missing Variable (required in NTUP mode)",
                region.name
            ))
        })?;
        let binning = region.binning.as_ref().ok_or_else(|| {
            Error::Validation(format!(
                "Region '{}' missing Binning (required in NTUP mode)",
                region.name
            ))
        })?;
        ...
        for s in &cfg.samples {
            ...
            let file = s.file.clone().ok_or_else(|| {
                Error::Validation(format!(
                    "Sample '{}' missing File (required in NTUP mode)",
                    s.name
                ))
            })?;
...
pub fn workspace_from_str(text: &str, base_dir: &Path) -> Result<Workspace> {
    ...
    for region in &cfg.regions {
        ...
        let mut ch =
            ChannelConfig::new(&region.name).variable(region_variable).binning(region_binning);
        ...
        for s in &cfg.samples {
            ...
            let nominal_weight =
                expr_mul([external_weight.clone(), s.weight.clone()].into_iter().flatten());
            ...
            for sys in &cfg.systematics {
                if sys_applies(sys, &region.name, &s.name) {
                    sc.modifiers.push(sys_to_modifier(
                        sys,
                        external_weight.as_deref(),
                        s.weight.as_deref(),
                    )?);
                }
            }
    ...
    if !cfg.samples.is_empty() {
        ...
        for mut ch in ws.channels.drain(..) {
            ...
            if candidates.is_empty() {
                candidates =
                    non_data.iter().copied().filter(|s| sample_applies(s, &ch.name)).collect();
            }
...
"#;
        ...
        assert!(
            (b - 2.0 * a).abs() < 1e-8,
            "expected region Weight to scale weight sys: a={a} b={b}"
        );
    }
...
"#;
        ...
        fn canonicalize_ws_value(v: &mut serde_json::Value) {
            ...
            if let Some(Value::Array(chs)) = root.get_mut("channels") {
                chs.sort_by_key(|c| {
                    c.get("name").and_then(Value::as_str).unwrap_or("").to_string()
                });
            ...
            if let Some(Value::Array(meas)) = root.get_mut("measurements") {
                ...
                for m in meas.iter_mut() {
                    if let Some(cfg) = m.get_mut("config").and_then(Value::as_object_mut)
                        && let Some(Value::Array(params)) = cfg.get_mut("parameters")
                    {
...
"#;
        ...
        let ws =
            workspace_from_str(cfg, &repo_root()).expect("HIST mode workspace (region filters)");
        assert_eq!(ws.channels.len(), 2);
        assert_eq!(ws.channels[0].name, "channel1");
        assert_eq!(ws.channels[0].samples.len(), 1);
...
Regions: SR_AllBDT
"#;
        let ws = workspace_from_str(cfg, &repo_root())
            .expect("HIST mode workspace (tttt-prod filtered)");
        assert_eq!(ws.channels.len(), 1);
    ...
    fn trex_import_hist_mode_filters_realistic_trex_export_dir_tttt_prod_from_config_file() {
        ...
        let cfg =
            std::fs::read_to_string(&cfg_path).expect("missing hist_tttt_prod_masking.config");

        let ws =
            workspace_from_str(&cfg, &root).expect("HIST mode workspace (tttt-prod from file)");
        assert_eq!(ws.channels.len(), 1);
        assert_eq!(ws.channels[0].name, "SR_AllBDT");
        let names: Vec<&str> = ws.channels[0].samples.iter().map(|s| s.name.as_str()).collect();
        assert_eq!(names, vec!["tttt", "ttW"]);
...
"#;
        ...
        let bad = rep
            ...
            .iter()
            .find(|x| {
                x.block_kind.as_deref() == Some("Sample") && x.block_name.as_deref() == Some("bad")
            })
            .expect("bad sample item");
        assert!(!bad.ok, "expected bad expression to fail");

        let derived_up =
            rep.items.iter().find(|x| x.derived && x.key == "WeightUp").expect("derived WeightUp");
...
"#;
        ...
        assert_eq!(
            parsed.samples[0].file.as_deref(),
...
"#;
        ...
        assert_eq!(parsed.samples.len(), 1);
        assert!(
            parsed.samples[0].modifiers.iter().any(
                |m| matches!(m, NtupleModifier::NormFactor { name } if name == "SigXsecOverSM")
            ),
