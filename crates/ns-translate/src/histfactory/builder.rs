...
fn build_modifier(
    ...
    match m {
        ...
        ModifierXml::StatError { histo_name, histo_path, input_file } => {
            // StatError:
            // - If a StatError histogram is provided, ROOT/HistFactory exports store **absolute**
            //   per-bin uncertainties (sigma_abs). Do NOT multiply by nominal.
            ...
            let data = if let Some(hn) = histo_name {
                ...
                if sigma_abs.len() != nominal.bin_content.len() {
                    return Err(Error::Xml(format!(
