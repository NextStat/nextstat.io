...
fn resolve_input_file_path(base_dir: &Path, input_file: &str) -> PathBuf {
    ...
    // TREx/HistFactory exports sometimes embed machine-local relative prefixes like
    // "tmp/trex_exports/<case>_<timestamp>/data.root". When the referenced path doesn't exist in
    // this repo, fall back to the export-dir copy (`base_dir/<basename>`).
    if let Some(name) = p.file_name() {
...
pub fn from_xml_with_basedir(
    ...
    for ch_xml in &channels_xml {
        ...
        // HistFactory/ROOT uses Poisson/Gamma-constrained per-bin staterror parameters
        // (gamma_stat_<ch>_bin_<i>) when `ConstraintType=Poisson`.
        //
        ...
        let staterror_constraint_type = ch_xml
            .stat_error_config
            .as_ref()
            .map(|c| c.constraint_type.as_str())
            .unwrap_or("Poisson");
        if staterror_constraint_type.eq_ignore_ascii_case("Poisson") {
...
fn modifier_sort_key(m: &Modifier) -> (&'static str, &str) {
    match m {
        ...
        Modifier::TemplateMorphing { name, .. } => ("template_morphing", name.as_str()),
...
fn build_channel(
    ...
    // Apply HistFactory StatErrorConfig RelErrorThreshold at the *channel* level.
    //
    // ROOT/HistFactory prunes small relative stat errors by disabling the shared per-bin
    // staterror nuisance parameter (gamma_stat fixed at 1.0). This is a channel-level decision
    // based on the combined nominal yield and combined stat uncertainty across all samples that
    // participate in the `staterror_<channel>` modifier.
    let staterror_rel_threshold =
        ch.stat_error_config.as_ref().and_then(|c| c.rel_error_threshold).unwrap_or(0.05);
    if staterror_rel_threshold > 0.0 {
        #[derive(Debug, Clone)]
        struct StatAccum {
            sum_nominal: Vec<f64>,
            sum_sigma2: Vec<f64>,
        }
        let mut acc: HashMap<String, StatAccum> = HashMap::new();

        for s in &samples {
            for m in &s.modifiers {
                let Modifier::StatError { name, data } = m else { continue };
                let entry = acc.entry(name.clone()).or_insert_with(|| StatAccum {
                    sum_nominal: vec![0.0; s.data.len()],
                    sum_sigma2: vec![0.0; s.data.len()],
                });
                for (i, (&sigma_abs, &nom)) in data.iter().zip(s.data.iter()).enumerate() {
                    entry.sum_nominal[i] += nom;
                    entry.sum_sigma2[i] += sigma_abs * sigma_abs;
                }
            }
        }

        let mut disabled_by_name: HashMap<String, Vec<bool>> = HashMap::new();
        for (name, a) in &acc {
            let mut disabled = vec![false; a.sum_nominal.len()];
            for i in 0..a.sum_nominal.len() {
                let denom = a.sum_nominal[i];
                let rel = if denom > 0.0 { a.sum_sigma2[i].sqrt() / denom } else { 0.0 };
                disabled[i] = rel.is_finite() && rel > 0.0 && rel < staterror_rel_threshold;
            }
            disabled_by_name.insert(name.clone(), disabled);
        }

        for s in &mut samples {
            for m in &mut s.modifiers {
                let Modifier::StatError { name, data } = m else { continue };
                let Some(disabled) = disabled_by_name.get(name) else { continue };
                for (v, &is_disabled) in data.iter_mut().zip(disabled.iter()) {
                    if is_disabled {
                        *v = 0.0;
                    }
                }
...
fn build_sample(
    ...
    for m in &s.modifiers {
        // ROOT/HistFactory defaults when `<StatErrorConfig>` is omitted:
        // - ConstraintType = Poisson
        // - RelErrorThreshold = 0.05
        //
        // See `RooStats::HistFactory::StatErrorConfig()` defaults in ROOT.
        let staterror_constraint_type =
            ch.stat_error_config.as_ref().map(|c| c.constraint_type.as_str()).unwrap_or("Poisson");
        let staterror_rel_threshold =
            ch.stat_error_config.as_ref().and_then(|c| c.rel_error_threshold).unwrap_or(0.05);
        let mods = build_modifier(
            m,
            histo_path,
            input_file,
            base_dir,
...
fn build_modifier(
    ...
    channel_name: &str,
    _sample_name: &str,
    staterror_constraint_type: &str,
    _staterror_rel_threshold: f64,
) -> Result<Vec<Modifier>> {
    match m {
        ...
        ModifierXml::StatError { histo_name, histo_path, input_file } => {
                ...
                if let Some(sw2) = nominal.sumw2.as_ref() {
                    sw2.iter()
                        ...
                        .map(|(v, n)| {
                            ...
                            let _rel = if *n > 0.0 { sigma_abs / *n } else { 0.0 };
                            sigma_abs
                        })
                        .collect()
                } else {
                    ...
                    nominal
                        ...
                        .map(|v| {
                            ...
                            let _rel = if *v > 0.0 { sigma_abs / *v } else { 0.0 };
                            sigma_abs
                        })
                        .collect()
                }
            ...
            //
            // Note: ROOT/HistFactory uses `ConstraintType=Poisson` by default (gamma constraints),
            // but the modifier surface remains `StatError` with a shared per-bin gamma parameter.
            // We encode the constraint-term semantics via measurement-level ConstraintSpec metadata
            // (see `staterror_gamma_rels` handling), not by re-labelling the modifier as `ShapeSys`.
            let _constraint_type = staterror_constraint_type;
...
fn build_measurements(
    ...
    config
        ...
        .map(|m| {
            ...
            // Non-standard extension: represent HistFactory `ConstraintType=Poisson` StatError
            // semantics as Gamma constraints on the staterror per-bin parameters.
            //
