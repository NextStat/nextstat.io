...
pub enum ResolvedModifier {
    ...
    StatError {
        ...
        constraint_type: StatConstraintType,
    },
    ShapeSys {
        param_names: Vec<String>,
...
pub enum Hs3ResolveError {
    ...
    #[error("bin count mismatch in channel {channel}: observed has {obs_bins} bins but sample {sample} has {sample_bins}")]
    BinCountMismatch {
        channel: String,
        obs_bins: usize,
        sample: String,
...
pub fn resolve(
    ...
    for d in &ws.distributions {
        if let Hs3Distribution::Gaussian(g) = d {
            if let Some(center_val) = global_obs_values.get(&g.mean) {
                if let Some(ci) = constraint_by_param.get_mut(&g.x) {
    ...
    for (dist_name, data_name) in likelihood
        .distributions
        .iter()
        .zip(likelihood.data.iter())
    {
        // Look up the histfactory_dist
        let dist = dist_map
        ...
        let hf = match dist {
            ...
            _ => return Err(Hs3ResolveError::NotHistFactoryDist(dist_name.clone())),
        };
...
fn resolve_modifier(
    m: &Hs3Modifier,
) -> Result<Option<ResolvedModifier>, Hs3ResolveError> {
    match m {
        Hs3Modifier::NormFactor { parameter, .. } => Ok(Some(ResolvedModifier::NormFactor {
            param_name: parameter.clone(),
        })),
        Hs3Modifier::NormSys {
            parameter, data, ..
        } => Ok(Some(ResolvedModifier::NormSys {
            param_name: parameter.clone(),
            hi: data.hi,
            lo: data.lo,
        })),
        Hs3Modifier::HistoSys {
            parameter, data, ..
        } => Ok(Some(ResolvedModifier::HistoSys {
            param_name: parameter.clone(),
        ...
        Hs3Modifier::ShapeSys {
            ...
            let uncertainties = data
                .as_ref()
                .map(|d| d.vals.clone())
                .unwrap_or_default();
            Ok(Some(ResolvedModifier::ShapeSys {
                param_names: parameters.clone(),
                uncertainties,
            }))
        }
        ...
        Hs3Modifier::Lumi { parameter, .. } => Ok(Some(ResolvedModifier::Lumi {
            param_name: parameter.clone(),
        })),
