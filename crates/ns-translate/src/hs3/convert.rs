...
pub fn from_hs3(
    ...
    let resolved =
        resolve(&ws, analysis_name, param_point_set).map_err(Hs3ConvertError::Resolve)?;
...
pub fn from_hs3_default(json: &str) -> Result<HistFactoryModel, Hs3ConvertError> {
    from_hs3(json, None, None, NormSysInterpCode::Code1, HistoSysInterpCode::Code0)
}
...
pub fn convert(
    ...
    for poi_name in &resolved.pois {
        let bounds = resolved.bounds.get(poi_name).copied().unwrap_or((NORM_LO, NORM_HI));
            ...
            for m in &sample.modifiers {
                match m {
                    ...
                    ResolvedModifier::StatError { param_names, .. } => {
                        ...
                        let staterror_key = (
                            ch_idx,
                            param_names.first().map(|s| s.as_str()).unwrap_or("").to_string(),
                        );
                        let errors = sample.errors.as_deref().unwrap_or(&[]);
                        let accum = staterror_accum.entry(staterror_key).or_insert_with(|| {
                            StatErrorAccum {
                        ...
                        for bin in 0..ch.n_bins {
                            accum.sum_nominal[bin] +=
                                sample.nominal.get(bin).copied().unwrap_or(0.0);
    ...
    for ((ch_idx, _), accum) in &staterror_accum {
        ...
        for sample in &ch.samples {
            for m in &sample.modifiers {
                if let ResolvedModifier::StatError { param_names, constraint_type: _ } = m {
    ...
    for ch in resolved.channels.iter() {
        ...
        for (sample_idx, sample) in ch.samples.iter().enumerate() {
            ...
            for (mod_idx, m) in sample.modifiers.iter().enumerate() {
                match m {
                    ...
                    ResolvedModifier::NormSys { param_name, hi, lo } => {
                        let param_idx = param_map[param_name];
                    ...
                    ResolvedModifier::HistoSys { param_name, hi_template, lo_template } => {
                        let param_idx = param_map[param_name];
                    ...
                    ResolvedModifier::StatError { param_names, .. } => {
                        let param_indices: Vec<usize> =
                            param_names.iter().map(|pn| param_map[pn]).collect();
                        let uncertainties = sample.errors.as_deref().unwrap_or(&[]).to_vec();
                        model_modifiers
                            .push(ModelModifier::StatError { param_indices, uncertainties });
                    }
                    ResolvedModifier::ShapeSys { param_names, uncertainties } => {
                        let param_indices: Vec<usize> =
                            param_names.iter().map(|pn| param_map[pn]).collect();
