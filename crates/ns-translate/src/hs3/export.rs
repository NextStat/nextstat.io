...
pub fn export_hs3(
    ...
    for ch_idx in 0..model.n_channels() {
        ...
        for (sample_idx, sample_name) in model.sample_names(ch_idx).iter().enumerate() {
            let nominal = model
                .sample_nominal(ch_idx, sample_idx)
                .unwrap_or(&[])
                .to_vec();
                ...
                match m {
                    ...
                    ModelModifier::HistoSys {
                        ...
                        modifiers.push(Hs3Modifier::HistoSys {
                            ...
                            data: Hs3HistoSysData {
                                hi: Hs3HistoTemplate {
                                    contents: hi_template.clone(),
                                },
                                lo: Hs3HistoTemplate {
                                    contents: lo_template.clone(),
                                },
                            },
                        });
                    }
                    ModelModifier::StatError {
                        param_indices,
                        uncertainties,
                    } => {
                        let param_names: Vec<String> = param_indices
                            .iter()
                            .map(|&idx| params[idx].name.clone())
                            .collect();
                    ...
                    ModelModifier::ShapeSys {
                        ...
                        modifiers.push(Hs3Modifier::ShapeSys {
                            ...
                            data: Some(Hs3ShapeSysData {
                                vals: uncertainties.clone(),
                            }),
                        });
                    }
    ...
    if let Some((name, values)) = bestfit_params {
        let pp: Vec<Hs3ParameterValue> = params
            .iter()
            .zip(values.iter())
            .map(|(p, &v)| Hs3ParameterValue {
                name: p.name.clone(),
                value: v,
            })
            .collect();
        parameter_points.push(Hs3ParameterPointSet {
            name: name.to_string(),
            parameters: pp,
        });
    }
    ...
    // -----------------------------------------------------------------------
    let dist_names: Vec<String> = model
        .channel_names()
        .iter()
        .cloned()
        .collect();

    let data_names: Vec<String> = dist_names
        .iter()
        .map(|n| format!("obsData_{n}"))
        .collect();
    ...
    let pois: Vec<String> = if let Some(poi_idx) = poi_index {
        vec![params[poi_idx].name.clone()]
    } else {
        vec![]
    };
    ...
    // Metadata
    // -----------------------------------------------------------------------
    let metadata = original_hs3
        .map(|ws| ws.metadata.clone())
        .unwrap_or(Hs3Metadata {
