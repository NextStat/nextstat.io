...
use super::ingest::{from_arrow_ipc, from_record_batches, ArrowIngestConfig};
use super::parquet::{write_parquet_bytes, read_parquet_bytes};
...
fn test_ingest_staterror_modifier() {
    ...
    let se = signal.modifiers.iter().find(|m| matches!(m,
        crate::pyhf::schema::Modifier::StatError { .. }
    ));
    assert!(se.is_some());
}
...
fn test_parquet_roundtrip() {
    ...
    let pq_bytes = write_parquet_bytes(&[batch.clone()]).unwrap();
