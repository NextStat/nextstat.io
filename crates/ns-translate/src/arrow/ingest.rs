...
pub enum ArrowIngestError {
    ...
    #[error("inconsistent bin count in channel '{channel}': expected {expected}, got {actual} (sample '{sample}')")]
    InconsistentBins {
        channel: String,
        sample: String,
        expected: usize,
...
fn extract_rows(batch: &RecordBatch, rows: &mut Vec<RowData>) -> Result<(), ArrowIngestError> {
    ...
    let sample_idx = schema
        .index_of("sample")
        .map_err(|_| ArrowIngestError::MissingColumn("sample".into()))?;
    let yields_idx = schema
        .index_of("yields")
        .map_err(|_| ArrowIngestError::MissingColumn("yields".into()))?;
    ...
    for i in 0..batch.num_rows() {
        ...
        let yields_f64 = yields_values
            .as_any()
            .downcast_ref::<Float64Array>()
            .ok_or_else(|| ArrowIngestError::WrongType {
                col: "yields".into(),
