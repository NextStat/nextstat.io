...
pub fn workspace_audit(json: &serde_json::Value) -> WorkspaceAudit {
    ...
    if let Some(channels) = json.get("channels").and_then(|v| v.as_array()) {
        ...
        for channel in channels {
                ...
                for sample in samples {
                    ...
                    if let Some(modifiers) = sample.get("modifiers").and_then(|v| v.as_array()) {
                        for modifier in modifiers {
                            ...
                            let mod_type = modifier
                                .get("type")
                                .and_then(|v| v.as_str())
                                .unwrap_or("unknown");
                            let mod_name = modifier
                                .get("name")
                                .and_then(|v| v.as_str())
                                .unwrap_or("unnamed");

                            *audit.modifier_types_used.entry(mod_type.to_string()).or_insert(0) += 1;

                            if !KNOWN_MODIFIER_TYPES.contains(&mod_type) {
                                let msg =
                                    format!("unsupported modifier type: \"{}\" (name: \"{}\")", mod_type, mod_name);
            ...
            if let Some(first_sample) = channel
                .get("samples")
                .and_then(|v| v.as_array())
                .and_then(|a| a.first())
            {
                if let Some(data) = first_sample.get("data").and_then(|v| v.as_array()) {
    ...
    if let Some(measurements) = json.get("measurements").and_then(|v| v.as_array()) {
        for meas in measurements {
            ...
            let n_fixed = meas
                ...
                .and_then(|v| v.as_array())
                .map(|params| params.iter().filter(|p| p.get("fixed") == Some(&serde_json::json!(true))).count())
                .unwrap_or(0);

            audit.measurements.push(MeasurementInfo {
                name,
                poi,
...
mod tests {
    ...
    fn test_audit_simple_workspace() {
        ...
        assert!(audit.unsupported_features.is_empty(), "simple workspace should have no unsupported features: {:?}", audit.unsupported_features);
