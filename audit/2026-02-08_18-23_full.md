...
### [Expectations] Audit item outdated: `ns-compute` does not expose stub `CudaBackend` / `MetalBackend` implementations
- File: crates/ns-compute/src/lib.rs:1
- Evidence: Current `cuda.rs` / `metal.rs` are accelerator re-export modules and explicitly note they do **not** implement the generic `ComputeBackend` trait; there are no `CudaBackend`/`MetalBackend` types in the crate.
- Impact: This item no longer applies; GPU acceleration is provided via dedicated accelerator types (`cuda_batch`/`metal_batch` and differentiable sessions), not via `ComputeBackend`.
- Fix: Keep docs consistent: `crates/ns-compute/src/lib.rs` should not imply Metal/CUDA `ComputeBackend` implementations exist unless they actually do.
- Confidence: high
