...
As of Git commit `a253f2525ad16d603d96d55a354a9e2f7e8979c6`, the items in this report are **resolved** and the corresponding epics are now **done** in BMCP.

- HistFactory XML Import Parity: `ConstraintTerm` support implemented + CLI `--interp-defaults {root|pyhf}` added (pyhf: code1/code0; root: code4/code4p).
- Performance & Robustness: Metal f32 single-model fit/scan path implemented and wired through CLI.
- Metal GPU Full Backend: `GpuAccelerator` trait + generic `GpuSession` implemented; Metal single-fit + scan wired and tested.
- CUDA backend (Phase 2C): already completed (no remaining tasks).
- TREx Replacement: remaining tasks completed; SIMD task marked cancelled (duplicate/backlog).

Verification: `cargo test -p ns-cli` and `cargo test -p ns-cli --features metal` passed.
...
### [GPU/Phase 2C] Audit item outdated: no stub `ComputeBackend` CUDA/Metal backends in `ns-compute`
- File: crates/ns-compute/src/lib.rs:1
- Evidence: `ns_compute::cuda` / `ns_compute::metal` currently re-export accelerator types and explicitly state they do **not** implement `ComputeBackend`; there are no `CudaBackend`/`MetalBackend` types in the crate.
- Impact: This is no longer a stub-backend risk; however it remains important that docs and public API do not imply a generic GPU `ComputeBackend` exists when it does not.
- Fix: Keep crate docs aligned with reality (CPU is the only `ComputeBackend`; GPU uses accelerator/session APIs).
- Confidence: high
