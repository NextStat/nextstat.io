...
# Reference parameter estimates from NONMEM literature (Holford 2001).
# theta: population typical values (CL L/h, V L, Ka 1/h)
# omega: BSV variances on CL and V (diagonal)
# sigma: proportional residual error variance
# ofv: objective function value (-2LL)
REFERENCE = {
    "tool": "NONMEM 7.5",
    "method": "FOCE INTERACTION",
    "theta": {"CL": 0.134, "V": 8.05, "Ka": 1.49},
    "omega": {"CL": 0.0707, "V": 0.0267},
    "sigma": 0.0164,
    "ofv": -37.3,
}
...
def bench_nextstat(seed=42, n_repeats=3):
    ...
    # Initial estimates
    theta_init = [0.15, 8.0, 1.5]  # CL, V, Ka
    omega_init = [0.05, 0.05]       # omega_CL, omega_V (diagonal)
