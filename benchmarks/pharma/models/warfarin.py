...
# Synthetic generator ranges in this fixture: CL ~ 0.055 L/h, V ~ 1.0 L, Ka ~ 0.5 1/h.
...
def bench_nextstat(seed=42, n_repeats=3):
    ...
    # NS 1cpt_oral currently requires 3 random effects; keep Ka BSV near fixed.
    omega_init = [0.04, 0.04, 0.001]   # omega_CL, omega_V, omega_Ka≈0
...
one.cmt <- function() {{
  ini({{
    # Align initials to synthetic-data reference used by NextStat harness.
    tka <- log(0.50)
    tcl <- log(0.055)
    tv  <- log(1.00)
    eta.cl ~ 0.04
...
invisible(nlmixr2(one.cmt, d, est="focei", control=foceiControl(print=0)))
...
def run(seed=42, n_repeats=3, skip_competitors=False) -> dict[str, Any]:
    ...
    if skip_competitors:
        print("  [Warfarin] Competitors skipped by --skip-competitors")
    else:
