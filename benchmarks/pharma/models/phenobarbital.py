...
def _flatten_pheno_data():
    ...
    # Use mean dose for the scalar dose parameter
    mean_dose = sum(s["dose"] for s in PHENO_DATA) / n_subjects
...
def bench_nextstat(seed=42, n_repeats=3):
    ...
    times, y, subject_idx, n_subjects, dose = _flatten_pheno_data()

    # For IV bolus approximation: Ka=100 (fixed effectively),
    # CL and V are the real parameters of interest.
    theta_init = [0.005, 0.65, 100.0]  # CL, V, Ka (Ka fixed high for IV)
    omega_init = [0.03, 0.03]           # omega_CL, omega_V
