...
# Note: nlme_foce now supports model="1cpt_iv" natively.
REFERENCE = {
    "tool": "NONMEM 7.5",
    "method": "FOCE",
...
def bench_nextstat(seed=42, n_repeats=3):
    ...
    theta_init = [0.005, 0.65]      # CL (L/h/kg), V (L/kg)
    omega_init = [0.025, 0.02]      # omega_CL, omega_V
    ...
    return {
        ...
        "theta": dict(zip(["CL", "V"], result["theta"])),
        "omega": result["omega"],
...
def bench_nlmixr2(seed=42, n_repeats=3):
    ...
    # Use mg/kg dose to match NS harness parameterization (CL, V in per-kg units).
    rows = []
    for subj in PHENO_DATA:
        dose_mg_per_kg = subj["dose"] / subj["wt"]
        # IV bolus dosing record (CMT=1 for central compartment)
        rows.append(f'{subj["id"]},0,0,{dose_mg_per_kg:.4f},1')
        for t, c in zip(subj["times"], subj["conc"]):
...
invisible(nlmixr2(one.cmt.iv, d, est="foce", control=foceiControl(print=0)))
...
def run(seed=42, n_repeats=3, skip_competitors=False) -> dict[str, Any]:
    ...
    print(f"\n  [Phenobarbital] NextStat FOCE INTERACTION ({n_repeats} repeats)...")
    ...
    if skip_competitors:
        print("  [Phenobarbital] Competitors skipped by --skip-competitors")
    else:
