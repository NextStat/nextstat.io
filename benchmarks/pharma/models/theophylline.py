...
#
# NOTE: The NONMEM reference uses 2-IIV (CL, V only) with sigma estimated.
# Our harness uses 3-IIV (Ka near-fixed) with sigma fixed at sqrt(0.0164)=0.128.
# Both NS and nlmixr2 agree on CL~0.040, which differs from the NONMEM reference
# CL=0.031 due to the model specification difference (IIV structure, sigma handling).
# For parity comparisons, use nlmixr2 as the primary comparator.
REFERENCE = {
    "tool": "NONMEM 7.5",
    "method": "FOCE",
...
def _flatten_theo_data():
    ...
    for i, subj in enumerate(THEO_DATA):
        for t, c in zip(subj["times"], subj["conc"]):
            # Single-dose oral model predicts C(t=0)=0; all pre-dose baseline
            # records must be excluded, regardless of measured DV.
            if t <= 0.0:
                continue
            times.append(t)
            y.append(c)
    ...
    # Per-subject dose in mg/kg — matches NONMEM reference parameterization
    # (CL_F in L/h/kg, V_F in L/kg).
    doses = [s["dose"] for s in THEO_DATA]
...
def bench_nextstat(seed=42, n_repeats=3):
    ...
    times, y, subject_idx, n_subjects, doses = _flatten_theo_data()

    # Initial estimates (close to NONMEM reference)
    theta_init = [0.035, 0.5, 1.5]     # CL_F (L/h/kg), V_F (L/kg), Ka (1/h)
    # NONMEM ref has BSV on CL and V only — Ka fixed via omega_fixed.
    omega_init = [0.03, 0.02, 1e-8]     # omega_CL, omega_V, omega_Ka≈0 (near-fixed)
    # NONMEM reports σ² (variance): 0.0164 → σ (SD) = √0.0164 ≈ 0.128.
    # Fixed via estimate_sigma=False to match NONMEM reference.
    sigma = 0.128
    ...
    # Untimed warmup for stable timing across fresh processes.
    _ = nextstat.nlme_foce(
    ...
    return {
        ...
        "method": "FOCE",
        "nextstat_module": str(getattr(nextstat, "__file__", "unknown")),
        ...
        "seed": seed,
        "harness": {
            "dose_mode": "per_subject_mg_per_kg",
            "doses_len": len(doses),
...
def bench_nlmixr2(seed=42, n_repeats=3):
    ...
    # Use mg/kg dose to match NS harness parameterization (CL_F, V_F in per-kg units).
    rows = []
    for subj in THEO_DATA:
        # Dosing record
        rows.append(f'{subj["id"]},0,0,{subj["dose"]:.4f},1')
        for t, c in zip(subj["times"], subj["conc"]):
            if t <= 0.0:
                continue
...
result <- NULL
# Untimed warmup to avoid first-call setup noise in timed repeats.
invisible(nlmixr2(one.cmt, d, est="foce", control=foceiControl(print=0)))
...
out <- list(
  ...
  method = "FOCE",
  theta = list(
    CL_F = exp(as.numeric(fixef(result)["tcl"])),
    V_F  = exp(as.numeric(fixef(result)["tv"])),
...
'''
    ...
    try:
        ...
        # Fallback: recover JSON object from noisy stdout.
        json_start = stdout.rfind("{")
        if json_start >= 0:
            return json.loads(stdout[json_start:])
        print("SKIP: nlmixr2 no JSON payload in stdout")
        return None
...
def run(seed=42, n_repeats=3, skip_competitors=False) -> dict[str, Any]:
    ...
    if skip_competitors:
        print("  [Theophylline] Competitors skipped by --skip-competitors")
    else:
