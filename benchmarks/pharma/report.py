...
Parity criteria:
- |theta_NS - theta_ref| / |theta_ref| < 0.001 (0.1% relative for each param)
- RSE proxy (cross-seed sd/|mean|) <= 10% for each parameter
...
def check_theta_rse_proxy(
    theta_rse_proxy: dict[str, float],
    rse_max: float = RSE_PROXY_MAX,
) -> tuple[bool, dict[str, float]]:
    """Check cross-seed RSE proxy gates.

    RSE proxy is computed by the harness as sd(theta)/|mean(theta)| across seeds.
    """
    if not theta_rse_proxy:
        return False, {}
    ok = True
    bad: dict[str, float] = {}
    for name, value in theta_rse_proxy.items():
        if value > rse_max:
            ok = False
            bad[name] = value
    return ok, bad
...
def _status(ns_faster: bool, parity_ok: bool, converged_ok: bool) -> str:
    """Determine status string."""
    if not converged_ok:
        return "fail:convergence"
...
def _extract_engine_summary(result: dict[str, Any], key: str) -> Optional[dict[str, Any]]:
    """Get summary entry for an engine from aggregated or legacy result format."""
...
def print_summary_table(results: list[dict[str, Any]]) -> None:
    ...
    for r in results:
        ...
        ns = _extract_engine_summary(r, "nextstat")
        ref = r.get("reference", {})
        ...
        ref_ofv = ref.get("ofv")
        if ns_ofv is not None and ref_ofv is not None:
            check_ofv_parity(ns_ofv, ref_ofv)
        converged_ok = bool(ns.get("converged_all", ns.get("converged", False)))
        n_seeds = int(ns.get("n_seeds", 1))
        rse_ok, _ = check_theta_rse_proxy(ns.get("theta_rse_proxy", {}))
        if n_seeds < 3:
            # Regulatory-grade requires multi-seed reproducibility.
            rse_ok = False

        parity_ok = theta_ok and rse_ok
        max_theta_diff = max(theta_diffs.values()) if theta_diffs else 0.0
        max_rse = max(ns.get("theta_rse_proxy", {}).values()) if ns.get("theta_rse_proxy") else float("inf")
        max_rse_str = f"{max_rse:.4f}" if max_rse != float("inf") else "N/A"
        parity_str = (
            f"{'ok' if parity_ok else 'fail'} (dtheta<{max_theta_diff:.4f}, "
            f"rse<{max_rse_str}, conv={'ok' if converged_ok else 'FAIL'})"
        )

        # Try to find a competitor result
        comp_name = None
        comp_wall = None
        for key in ["nlmixr2", "torsten", "saemix", "mas", "pharmpy"]:
            comp = _extract_engine_summary(r, key)
            if comp is not None:
                comp_name = key
                comp_wall = comp.get("wall_s")
                break
        ...
        if comp_name and comp_wall is not None and comp_wall > 0:
            ...
            status = _status(ns_faster, parity_ok, converged_ok)
        else:
            comp_time_str = "N/A (not installed)"
            speedup = "N/A"
            if not converged_ok:
                status = "fail:convergence"
            else:
                status = f"{'pass:ns_only' if parity_ok else 'fail:parity'}"

        print(
            f"{model_name:<28} | {ns_time_str:<14} | {comp_time_str:<18} | {speedup:<8} | "
            f"{parity_str:<24} | {status:<14}"
        )
...
def print_detailed_report(results: list[dict[str, Any]]) -> None:
    ...
    for r in results:
        ...
        print(f"  NextStat: wall={_fmt_time(ns.get('wall_s', 0))}"
              ...
              f"  converged={ns.get('converged_all', ns.get('converged', 'N/A'))}"
              f"  converged_rate={ns.get('converged_rate', 'N/A')}"
              f"  n_seeds={ns.get('n_seeds', 'N/A')}"
              f"  n_iter={ns.get('n_iter', 'N/A')}")
        if ns.get("theta_rse_proxy"):
            print(f"  RSE proxy (sd/|mean| across seeds): {ns['theta_rse_proxy']}")
