...
def main():
    ...
    print(f"{'='*110}")
    header = f"{'Model':<18} | {'Engine':<16} | {'Cold(s)':<9} | {'Warm(s)':<9} | {'min_ESS':<11} | {'ESS/s(warm)':<12} | {'R-hat':<8} | {'Chains':>6}"
    print(header)
    print("-" * len(header))

    for r in results:
        if "error" in r:
            print(f"{r['model']:<18} | {r['engine']:<16} | {'FAIL':<9} | {'':<9} | {'':<11} | {'':<12} | {'':<8} | {'':<6}")
            continue
        ...
        print(f"{r['model']:<18} | {r['engine']:<16} | {wall_cold:<9.2f} | {wall_warm:<9.2f} | {fmt(min_ess):<11} | {fmt(ess_per_s):<12} | {fmt(max_rhat,4):<8} | {n_ch:>6}")
