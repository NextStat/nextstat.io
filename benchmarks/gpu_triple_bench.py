...
def bench_ns_laps(
    ...
    wall_warm = phase_times.get("sampling_s") if isinstance(phase_times, dict) else None
    if not isinstance(wall_warm, (int, float)) or not math.isfinite(float(wall_warm)) or wall_warm <= 0:
        wall_warm = wall_s
...
def main():
    ...
    for model in models:
        ...
        if not args.skip_laps:
            ...
            try:
                ...
                print(
                    f"    wall_total={r['wall_s']:.2f}s  wall_sampling={r['wall_warm']:.2f}s  "
        ...
        if not args.skip_cpu:
            ...
            try:
                ...
                ess_s_total = r['min_ess'] / r['wall_s'] if r['min_ess'] and r['wall_s'] else None
                print(
                    f"    wall_total={r['wall_s']:.2f}s  min_ESS={fmt(r['min_ess'])}  "
                    f"ESS/s(total)={fmt(ess_s_total)}  "
    ...
    for r in results:
        ...
        wall_warm_raw = r.get("wall_warm", wall_cold)
        wall_warm = (
            float(wall_warm_raw)
            if isinstance(wall_warm_raw, (int, float)) and math.isfinite(float(wall_warm_raw))
            else None
        )
