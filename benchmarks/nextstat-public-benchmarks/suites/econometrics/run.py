...
- did_twfe_wild_bootstrap: TWFE ATT with wild cluster bootstrap inference
- did_staggered: staggered-adoption DiD (group-time ATT)
- event_study_twfe: TWFE baseline with lead/lag dummies
- iv_2sls: baseline IV/2SLS (optional parity via linearmodels)
- iv_2sls_hac: baseline IV/2SLS with Newey-West HAC covariance
...
def _maybe_import(name: str, dist_name: Optional[str] = None) -> tuple[bool, Optional[str]]:
    try:
        mod = importlib.import_module(name)
        mod_v = getattr(mod, "__version__", None)
        pkg_v: Optional[str] = None
        try:
            pkg_v = str(importlib.metadata.version(dist_name or name))
        except Exception:
            pkg_v = None
        v = pkg_v or (str(mod_v) if mod_v is not None else None) or "unknown"
        return True, str(v)
    except Exception:
        return False, None
...
def _gen_did_staggered(n_entities: int, n_times: int, *, seed: int) -> dict[str, Any]:
    rng = np.random.default_rng(int(seed))
    entity = [i for i in range(int(n_entities)) for _ in range(int(n_times))]
    time_id = [t for _ in range(int(n_entities)) for t in range(int(n_times))]
    n = len(entity)

    adoption_probs = rng.random(int(n_entities))
    # 30% never-treated, others adopt in [2, n_times-2].
    cohort_by_entity = np.full(int(n_entities), fill_value=-1, dtype=int)
    for i in range(int(n_entities)):
        if adoption_probs[i] < 0.30:
            cohort_by_entity[i] = -1
        else:
            lo = 2
            hi = max(lo + 1, int(n_times) - 1)
            cohort_by_entity[i] = int(rng.integers(lo, hi))
    ...
    cohort_obs: list[float | None] = []
    for i, (e, t) in enumerate(zip(entity, time_id)):
        g = int(cohort_by_entity[e])
        if g >= 0 and t >= g:
            treat[i] = 1
            rel = int(t - g)
            eff = 0.5 + 0.35 * min(rel, 4)
        else:
            eff = 0.0
        y[i] = alpha_i[e] + gamma_t[t] + eff + eps[i]
        cohort_obs.append(None if g < 0 else float(g))
...
def _fit_did_twfe_wild_bootstrap(data: dict[str, Any], *, n_boot: int) -> tuple[list[float], list[float], dict[str, Any]]:
    fit = nextstat.econometrics.did_twfe_wild_cluster_bootstrap(
        data["x"],
        data["y"],
        treat=data["treat"],
        post=data["post"],
    ...
    return [float(fit.att)], [float(fit.att_se_bootstrap)], {
        "n_obs": int(len(data["y"])),
        "n_entities": int(len(set(data["entity"]))),
        "n_times": int(len(set(data["time"]))),
        "n_boot": int(fit.n_boot),
        "p_value": float(fit.p_value),
    }


def _fit_did_staggered(data: dict[str, Any]) -> tuple[list[float], list[float], dict[str, Any]]:
    ...
    return [float(fit.att)], [float(fit.att_se)], {
        "n_obs": int(fit.n_obs),
        "n_entities": int(fit.n_entities),
        "n_times": int(fit.n_times),
        "n_cells": int(len(fit.cells)),
    }
...
def _fit_iv_2sls_hac(data: dict[str, Any]) -> tuple[list[float], list[float], dict[str, Any]]:
    fit = nextstat.econometrics.iv_2sls_fit(
        data["y"],
        endog=data["endog"],
        instruments=data["instruments"],
    ...
    return list(fit.coef), list(fit.standard_errors), {"n_obs": int(fit.n_obs), "df_resid": int(fit.df_resid), "max_lag": 4}
...
def _baseline_linearmodels_iv(data: dict[str, Any]) -> tuple[list[float], list[float]]:
    ...
    p = res.params
    s = res.std_errors
    names = [str(nm) for nm in p.index.tolist()]
    order = [i for i, nm in enumerate(names) if nm.startswith("endog")] + [i for i, nm in enumerate(names) if nm.startswith("exog")]
    if not order:
        order = list(range(len(names)))
    coef = [float(np.asarray(p).ravel()[i]) for i in order]
    se = [float(np.asarray(s).ravel()[i]) for i in order]
    return coef, se


def _baseline_linearmodels_iv_hac(data: dict[str, Any], *, bandwidth: int) -> tuple[list[float], list[float]]:
    from linearmodels.iv import IV2SLS  # type: ignore[import-not-found]
    ...
    endog = np.asarray(data["endog"], dtype=float)
    instr = np.asarray(data["instruments"], dtype=float)
    res = IV2SLS(Y, exog, endog, instr).fit(cov_type="kernel", kernel="bartlett", bandwidth=int(bandwidth))
    p = res.params
    s = res.std_errors
    names = [str(nm) for nm in p.index.tolist()]
    order = [i for i, nm in enumerate(names) if nm.startswith("endog")] + [i for i, nm in enumerate(names) if nm.startswith("exog")]
...
def _baseline_pyfixest_staggered_att(data: dict[str, Any]) -> tuple[list[float], list[float]]:
    import pandas as pd  # type: ignore[import-not-found]
    import pyfixest as pf  # type: ignore[import-not-found]

    cohort = [0 if g is None else int(g) for g in data["cohort"]]
    df = pd.DataFrame(
        {
            "y": data["y"],
            "entity": data["entity"],
    ...
    td = res.tidy()
    est = float(td.iloc[0]["Estimate"])
    se = float(td.iloc[0]["Std. Error"])
    return [est], [se]


def _baseline_pyfixest_did_wild_bootstrap(
    data: dict[str, Any],
    *,
    n_boot: int,
    ...
    fit = pf.feols(
        "y ~ treat:post | entity + time",
        data=df,
        vcov={"CRV1": "entity"},
    )
    coef_series = fit.coef()
    se_series = fit.se()
    param = str(coef_series.index[0])
    wb = fit.wildboottest(param=param, reps=int(n_boot), weights_type="webb", seed=0)
    pval = float(wb.loc["Pr(>|t|)"]) if hasattr(wb, "loc") else float("nan")
    return [float(coef_series.iloc[0])], [float(se_series.iloc[0])], pval
...
def main() -> int:
    ...
    if kind == "panel_fe":
    ...
    elif kind == "did_twfe_wild_bootstrap":
        spec = {
            "kind": kind,
            "n_entities": int(args.n_entities),
            "n_times": int(args.n_times),
        ...
        data = _gen_did(int(args.n_entities), int(args.n_times), n_x, seed=seed, treat_frac=float(args.treat_frac))
        run_fit = lambda d: _fit_did_twfe_wild_bootstrap(d, n_boot=int(args.n_boot))
    elif kind == "did_staggered":
        spec = {"kind": kind, "n_entities": int(args.n_entities), "n_times": int(args.n_times), "seed": seed}
        data = _gen_did_staggered(int(args.n_entities), int(args.n_times), seed=seed)
        run_fit = _fit_did_staggered
    ...
    elif kind == "iv_2sls_hac":
        spec = {"kind": kind, "n_obs": int(args.n_obs), "n_x": n_x, "seed": seed, "max_lag": 4}
        data = _gen_iv(int(args.n_obs), n_x, seed=seed)
        run_fit = _fit_iv_2sls_hac
    else:
        spec = {"kind": kind, "n_obs": int(args.n_obs), "n_x": n_x, "seed": seed}
    ...
    has_pf, pf_v = _maybe_import("pyfixest")
    _has_wb, wb_v = _maybe_import("wildboottest")
    ...
    try:
        ...
        obj = {
            ...
            "meta": {
                ...
                "pyfixest_version": pf_v,
                "wildboottest_version": wb_v,
            },
            "dataset": dataset,
            "config": {"kind": kind},
    ...
    try:
        if kind in ("panel_fe", "did_twfe", "event_study_twfe"):
            ...
            if not has_lm:
                raise RuntimeError("missing_dependency:linearmodels")
            coef_b, se_b = _baseline_linearmodels_iv_hac(data, bandwidth=4)
            runs_bl: list[float] = []
            for _ in range(repeat):
                t0 = time.perf_counter()
                _baseline_linearmodels_iv_hac(data, bandwidth=4)
                runs_bl.append(float(time.perf_counter() - t0))
            timing_baseline = {"name": "linearmodels", "wall_time_s": _summary(runs_bl), "raw": {"repeat": repeat, "runs_s": runs_bl}}
            baseline_obj = {"coef": coef_b, "se": se_b, "extra": {"backend": "linearmodels", "cov_type": "kernel_bartlett", "bandwidth": 4}}
            coef_abs, coef_rel = _max_abs_rel_diff(coef_ns, coef_b)
            se_abs, se_rel = _max_abs_rel_diff(se_ns, se_b)
            parity = {
                "status": "ok" if coef_abs is not None else "warn",
            ...
            if not has_pf:
                raise RuntimeError("missing_dependency:pyfixest")
            coef_b, se_b = _baseline_pyfixest_staggered_att(data)
            runs_bl: list[float] = []
            for _ in range(repeat):
                t0 = time.perf_counter()
                _baseline_pyfixest_staggered_att(data)
                runs_bl.append(float(time.perf_counter() - t0))
            timing_baseline = {"name": "pyfixest", "wall_time_s": _summary(runs_bl), "raw": {"repeat": repeat, "runs_s": runs_bl}}
            baseline_obj = {"coef": coef_b, "se": se_b, "extra": {"backend": "pyfixest", "estimator": "lpdid_att"}}
            coef_abs, coef_rel = _max_abs_rel_diff(coef_ns, coef_b)
            se_abs, se_rel = _max_abs_rel_diff(se_ns, se_b)
            parity = {
                "status": "ok" if coef_abs is not None else "warn",
                "reference": {"name": "pyfixest", "version": str(pf_v or "")},
                "metrics": {
                    "coef_max_abs_diff": coef_abs,
                    "coef_max_rel_diff": coef_rel,
                    "se_max_abs_diff": se_abs,
            ...
            if not has_pf:
                raise RuntimeError("missing_dependency:pyfixest")
            coef_b, se_b, p_b = _baseline_pyfixest_did_wild_bootstrap(data, n_boot=int(args.n_boot))
            runs_bl: list[float] = []
            for _ in range(repeat):
                t0 = time.perf_counter()
                _baseline_pyfixest_did_wild_bootstrap(data, n_boot=int(args.n_boot))
                runs_bl.append(float(time.perf_counter() - t0))
            timing_baseline = {"name": "pyfixest", "wall_time_s": _summary(runs_bl), "raw": {"repeat": repeat, "runs_s": runs_bl}}
            baseline_obj = {"coef": coef_b, "se": se_b, "extra": {"backend": "pyfixest", "estimator": "feols+wildboottest", "p_value": p_b}}
            coef_abs, coef_rel = _max_abs_rel_diff(coef_ns, coef_b)
            se_abs, se_rel = _max_abs_rel_diff(se_ns, se_b)
            p_ns = None
            try:
                p_ns = float(extra_ns.get("p_value")) if isinstance(extra_ns, dict) and ("p_value" in extra_ns) else None
            except Exception:
                p_ns = None
            p_abs = None if (p_ns is None or not math.isfinite(float(p_ns)) or not math.isfinite(float(p_b))) else abs(float(p_ns) - float(p_b))
            parity = {
                "status": "ok" if coef_abs is not None else "warn",
                "reference": {"name": "pyfixest", "version": str(pf_v or "")},
                "metrics": {
    ...
    if kind in ("panel_fe", "did_twfe", "event_study_twfe"):
        cfg = {"kind": kind, "cov": "cluster", "cluster": "entity"}
    elif kind == "did_twfe_wild_bootstrap":
        cfg = {
            "kind": kind,
            "cov": "wild_cluster_bootstrap",
            "cluster": "entity",
            "n_boot": int(args.n_boot),
            "weight_dist": "webb6",
        }
    elif kind == "did_staggered":
        cfg = {"kind": kind, "cov": "group_time", "control_group": "not_yet_treated"}
    elif kind == "iv_2sls_hac":
        cfg = {"kind": kind, "cov": "hac", "max_lag": 4}
    else:
        cfg = {"kind": kind, "cov": "hc1", "cluster": None}
