...
def _align_coef_for_parity(family: str, lhs: list[float], rhs: list[float]) -> tuple[list[float], list[float], Optional[str]]:
    """Align coefficient vectors for parity comparison when model parameterizations differ."""
    if family == "negbin" and len(lhs) == len(rhs) + 1:
        # NextStat NegativeBinomial includes an extra dispersion parameter.
        # statsmodels GLM(NegativeBinomial) here uses fixed alpha and returns only beta terms.
        return lhs[: len(rhs)], rhs, "trimmed_nextstat_dispersion"
    return lhs, rhs, None


def _sum_log_factorial(y: list[int]) -> Optional[float]:
    """Return sum(log(y!)) for integer y, or None if unavailable."""
    if not y:
        return 0.0
    try:
        # Prefer vectorized SciPy if present.
        from scipy.special import gammaln  # type: ignore[import-not-found]

        arr = np.asarray(y, dtype=float)
        return float(np.sum(gammaln(arr + 1.0)))
    except Exception:
        # Fallback: stdlib only.
        try:
            return float(sum(math.lgamma(int(v) + 1) for v in y))
        except Exception:
            return None


def _llf_abs_diff(family: str, nll_ns: float, llf_sm: Optional[float], y: list[int]) -> Optional[float]:
    """Compare NextStat NLL to statsmodels LLF with matched constant conventions where possible."""
    if llf_sm is None:
        return None
    # NextStat reports an objective (NLL) that is sometimes unnormalized w.r.t. discrete constants.
    # For parity, convert to an unnormalized LL and adjust statsmodels accordingly.
    if family in ("poisson", "negbin"):
        s = _sum_log_factorial(y)
        if s is None:
            return None
        # statsmodels llf includes -log(y!) term; NextStat omits it, so add it back.
        llf_sm_adj = llf_sm + float(s)
        return float(abs((-nll_ns) - llf_sm_adj))
    if family == "linear":
        # Scale/constant conventions differ (sigma/scale), keep LLF parity informationally disabled.
        return None
    # Default: treat NLL as true -LL.
    return float(abs((-nll_ns) - llf_sm))
...
def main() -> int:
    ...
    try:
        if has_sm:
            ...
            coef_ns_cmp, coef_sm_cmp, align_note = _align_coef_for_parity(family, coef_ns, coef_sm)
            coef_abs, coef_rel = _max_abs_rel_diff(coef_ns_cmp, coef_sm_cmp)
            llf_diff = _llf_abs_diff(family, nll_ns, llf_sm, [int(v) for v in data["y"]])
            ...
            if align_note is not None:
                metrics["coef_alignment"] = align_note
            parity["statsmodels"] = {
