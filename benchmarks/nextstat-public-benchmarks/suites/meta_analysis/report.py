#!/usr/bin/env python3
"""Small report generator for the meta-analysis suite results (human-facing)."""
...
def main() -> int:
    ...
    ap.add_argument("--suite", required=True, help="Path to meta_analysis_suite.json")
    ...
    lines: list[str] = []
    lines.append("# Meta-Analysis Suite (Snapshot)\n")
    ...
    lines.append("## Cases\n")
    lines.append("| Case | Kind | Studies | Status | Parity | Median (s) |")
    lines.append("|---|---|---:|---|---|---:|")
    for c in cases:
        lines.append(
            f"| `{c.get('case','')}` | {c.get('kind','')} | {int(c.get('n_studies', 0) or 0)} | {c.get('status','')} | "
            f"{c.get('parity_status','')} | {float(c.get('wall_time_median_s', 0.0) or 0.0):.6f} |"
        )
