...
def _competitor_versions() -> dict[str, str | None]:
    ...
    for pkg in packages:
        ...
        # Prefer package metadata because it's robust to modules with heavy import-time side effects.
        try:
            version = importlib_metadata.version(dist_name.get(pkg, pkg))
        except importlib_metadata.PackageNotFoundError:
            # Not installed. We'll try import as fallback for editable/dev installs.
            version = None
        except Exception as e:
            # Keep going, but preserve why metadata lookup failed for debugging.
            versions[pkg] = f"error:metadata:{type(e).__name__}"
        ...
        if version is None:
            try:
                ...
                versions[pkg] = f"error:import:{type(e).__name__}"
                continue
