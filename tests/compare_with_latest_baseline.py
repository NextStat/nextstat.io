...
def compare_root_suite_perf(
    *,
    ...
    for name, cur in sorted(cur_idx.items(), key=lambda kv: kv[0]):
        ...
        if base_status != "ok":
            # Baseline case was already failing (e.g. ROOT optimizer divergence or known mismatch).
            # Treat as expected: do not gate on correctness or perf.
            row.update(
                {
                    "ok": True,
                    "reason": f"expected_failure(baseline:{base_status},current:{cur_status})",
                }
            )
            cases_out.append(row)
            continue
...
def main() -> int:
    ...
    ap.add_argument("--pyhf-min-baseline-s", type=float, default=1e-5)

    # P6 compare thresholds (passed through)
    ap.add_argument("--p6-max-slowdown", type=float, default=1.30)
    ap.add_argument("--p6-min-baseline-fit-s", type=float, default=1e-2)
    ap.add_argument("--p6-min-baseline-predict-s", type=float, default=1e-2)
    ap.add_argument(
        "--p6-attempts",
        type=int,
        default=1,
    ...
    if (
        ...
        if not baseline_root_suite.exists() or not baseline_root_cases.exists():
            ...
            if rc_pr != 0:
                ...
                rc, stdout = _run(cmd, cwd=repo, env=env_dict)
                if (not out_cur.exists()) or out_cur.stat().st_size == 0:
                    report["root_suite"] = {
                        "status": "error",
                        "reason": "runner_failed_or_no_output",
                        "returncode": int(rc),
                        "stdout_tail": stdout[-4000:],
                        "baseline_suite_path": str(baseline_root_suite),
                        "baseline_cases_path": str(baseline_root_cases),
                    ...
                    # Correctness gating for ROOT suite is baseline-based:
                    # - baseline failing cases are treated as expected failures (do not gate)
                    # - baseline passing cases must keep passing
                    status = "ok" if (cmp.get("status") == "ok") else "fail"
                    if status != "ok":
                        any_failed = True
                    report["root_suite"] = {
                        "status": status,
                        "runner_returncode": int(rc),
                        "baseline_suite_path": str(baseline_root_suite),
                        "baseline_cases_path": str(baseline_root_cases),
                        "current_path": str(out_cur),
                        "compare": cmp,
