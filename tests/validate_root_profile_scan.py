...
def _stage_histfactory_export(
    *,
    ...
    # Many TREx/HistFactory export dirs embed absolute or build-machine-relative paths
    # in `InputFile="..."` attributes. Since we copy the export dir into `run_dir`,
    # rewrite InputFile references to local basenames when possible.
    #
    # Example: InputFile="tmp/trex_exports/.../data.root" -> InputFile="data.root"
    import re

    local_root_files = {p.name for p in run_dir.glob("*.root")}
    if local_root_files:
        for xml in run_dir.rglob("*.xml"):
            txt = xml.read_text()

            def _repl(m: re.Match[str]) -> str:
                val = m.group(1)
                base = Path(val).name
                if base in local_root_files and val != base:
                    return f'InputFile="{base}"'
                return m.group(0)

            new_txt = re.sub(r'InputFile="([^"]+)"', _repl, txt)
            if new_txt != txt:
                xml.write_text(new_txt)
...
def _write_root_macro_profile_scan(
  ...
  std::vector<int> params_hat_const;
  params_hat.reserve(params.size());
  params_hat_const.reserve(params.size());
  for (auto* p : params) params_hat.push_back(p->getVal());
  for (auto* p : params) params_hat_const.push_back(p->isConstant() ? 1 : 0);
"""
        params_mu_decl_code = r"""
  std::vector<std::vector<double>> params_mu;
  std::vector<std::vector<int>> params_mu_const;
  params_mu.reserve(mu_values.size());
  params_mu_const.reserve(mu_values.size());
"""
        params_mu_push_code = r"""
    std::vector<double> pv;
    std::vector<int> pc;
    pv.reserve(params.size());
    pc.reserve(params.size());
    for (auto* p : params) pv.push_back(p->getVal());
    for (auto* p : params) pc.push_back(p->isConstant() ? 1 : 0);
    params_mu.push_back(pv);
    params_mu_const.push_back(pc);
"""
        json_header_code = r"""
  out << "  \"parameter_names\": [";
  ...
  out << "],\n";
  out << "  \"parameter_const\": [";
  for (size_t i = 0; i < params_hat_const.size(); ++i) {
    out << params_hat_const[i];
    if (i + 1 != params_hat_const.size()) out << ", ";
  }
  out << "],\n";
  out << "  \"params_hat\": [";
...
"""
    ...
    out << "]";
    out << ", \"params_const\": [";
    for (size_t j = 0; j < params_mu_const[i].size(); ++j) {
      out << params_mu_const[i][j];
      if (j + 1 != params_mu_const[i].size()) out << ", ";
    }
    out << "]";
"""
    macro = f"""
