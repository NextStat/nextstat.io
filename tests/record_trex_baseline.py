...
def _normalize_combination_xml_in_place(*, stage_dir: Path, combination_xml: Path) -> None:
    """Make a staged HistFactory export relocatable.

    Some generators (including our pyhf->HistFactory export harness) emit absolute paths in:
    - <Combination OutputFilePrefix="...">
    - <Input>...</Input>

    This breaks tools that stage/copy the export dir because `hist2workspace` then writes the
    RooWorkspace outside the staged tree (and may even refer to inputs outside it).

    We rewrite those fields to point inside the staged export dir:
    - OutputFilePrefix -> "spec/<basename>"
    - Input -> "spec/<basename.xml>"
    """
    stage_dir = stage_dir.resolve()
    combination_xml = combination_xml.resolve()

    text = combination_xml.read_text()
    lines = text.splitlines(True)
    out: List[str] = []

    def _rewrite_output_prefix(line: str) -> str:
        key = "OutputFilePrefix="
        if key not in line:
            return line
        before, rest = line.split(key, 1)
        if not rest:
            return line
        quote = rest[0]
        if quote not in ("\"", "'"):
            return line
        parts = rest.split(quote, 2)
        if len(parts) < 3:
            return line
        old = parts[1]
        ...
        if not old_name:
            old_name = "ws"
        new = f"spec/{old_name}"
        return f"{before}{key}{quote}{new}{quote}{parts[2]}"

    def _rewrite_input(line: str) -> str:
        if "<Input>" not in line or "</Input>" not in line:
            return line
        try:
        ...
        if not base:
            return line
        # Prefer spec/ if it exists; otherwise keep basename relative to stage root.
        spec_candidate = stage_dir / "spec" / base
        rel = f"spec/{base}" if spec_candidate.exists() else base
        return line[:start] + rel + line[end:]

    for ln in lines:
        if "<Combination" in ln and "OutputFilePrefix=" in ln:
            ln = _rewrite_output_prefix(ln)
        if "<Input>" in ln and "</Input>" in ln:
            ln = _rewrite_input(ln)
        out.append(ln)

    combination_xml.write_text("".join(out))
...
def _write_root_macro_fit(macro_path: Path, *, root_workspace_file: Path, out_json: Path) -> None:
    ...
    lines = [
        ...
        '',
        f'  const char* root_path = "{root_path}";',
        f'  const char* out_path = "{out_path}";',
        '',
        ...
        r'  out << "{\n";',
        r'  out << "  \"status\": " << status << ",\n";',
        r'  out << "  \"nll_hat\": " << nll_hat << ",\n";',
        r'  out << "  \"twice_nll\": " << (2.0*nll_hat) << ",\n";',
        r'  out << "  \"parameters\": [\n";',
        '  for (size_t i = 0; i < ps.size(); i++) {',
        r'    out << "    {\"name\": \"" << escape_json(ps[i].name) << "\", \"value\": " << ps[i].val << ", \"uncertainty\": " << ps[i].err << "}";',
        '    if (i+1 < ps.size()) out << ",";',
        r'    out << "\n";',
        '  }',
        r'  out << "  ]\n";',
...
def _stage_histfactory_export_dir(*, export_dir: Path, stage_dir: Path) -> ExportInput:
    ...
    for p in sorted(export_dir.iterdir(), key=lambda x: x.name):
        ...
        if p.is_dir():
            shutil.copytree(
                ...
                dest,
                dirs_exist_ok=True,
                ignore=shutil.ignore_patterns("*_model.root"),
            )
        elif p.is_file():
    ...
    staged = _resolve_export_input(export_dir=stage_dir)
    # Make staged export relocatable to avoid absolute-path output/input issues.
    try:
        _normalize_combination_xml_in_place(stage_dir=stage_dir, combination_xml=staged.combination_xml)
    except Exception:
        # Best-effort: if normalization fails, keep original. The downstream error will surface.
        pass
    return staged
