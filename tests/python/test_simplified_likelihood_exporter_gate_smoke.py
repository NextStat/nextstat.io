...
def test_exporter_gate_validates_stable_source_semantics_schema_before_timestamp_normalization() -> None:
    gate = (
        _repo_root() / "scripts" / "benchmarks" / "simplified_likelihood_exporter_surface_gate.sh"
    ).read_text(encoding="utf-8")
    start = gate.index(
    ...
    committed_validate = section.index("jsonschema.validate(instance=committed, schema=schema)")
    generated_validate = section.index("jsonschema.validate(instance=generated, schema=schema)")
    committed_pop = section.index('committed.pop("generated_at_utc", None)')
    generated_pop = section.index('generated.pop("generated_at_utc", None)')

    assert committed_validate < committed_pop
    assert generated_validate < generated_pop
