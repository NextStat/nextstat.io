from __future__ import annotations

import importlib.util
import json
from pathlib import Path

import nextstat.bayes_design as ns_bayes_design
import pytest


def _design_spec() -> dict[str, object]:
    return {
        "schema_version": "nextstat_beta_binomial_design_v0",
        "design_id": "binary_superiority_demo",
        "control_prior": {"alpha": 1.0, "beta": 1.0},
        "treatment_prior": {"alpha": 1.0, "beta": 1.0},
        "looks": [
            {"id": "interim", "n_control": 20, "n_treatment": 20},
            {"id": "final", "n_control": 40, "n_treatment": 40},
        ],
        "decision_rules": {
            "success": {
                "posterior_probability_threshold": 0.95,
                "treatment_effect_margin": 0.0,
            },
            "futility": {
                "posterior_probability_threshold": 0.20,
                "treatment_effect_margin": 0.0,
            },
        },
        "analysis": {"credible_interval_level": 0.95},
        "simulation": {
            "n_replicates": 32,
            "seed": 123,
            "scenarios": [
                {"id": "null", "p_control": 0.40, "p_treatment": 0.40},
                {"id": "alt", "p_control": 0.40, "p_treatment": 0.60},
            ],
        },
    }


def _normal_design_spec() -> dict[str, object]:
    return {
        "schema_version": "nextstat_normal_normal_design_v0",
        "design_id": "continuous_superiority_demo",
        "control_prior": {"mean": 0.0, "sd": 10.0},
        "treatment_prior": {"mean": 0.0, "sd": 10.0},
        "likelihood": {"known_sd_control": 1.0, "known_sd_treatment": 1.0},
        "looks": [
            {"id": "interim", "n_control": 10, "n_treatment": 10},
            {"id": "final", "n_control": 20, "n_treatment": 20},
        ],
        "decision_rules": {
            "success": {
                "posterior_probability_threshold": 0.975,
                "treatment_effect_margin": 0.0,
            },
            "futility": {
                "posterior_probability_threshold": 0.10,
                "treatment_effect_margin": 0.0,
            },
        },
        "analysis": {"credible_interval_level": 0.95},
        "simulation": {
            "n_replicates": 32,
            "seed": 456,
            "scenarios": [
                {"id": "null", "mean_control": 0.0, "mean_treatment": 0.0},
                {"id": "alt", "mean_control": 0.0, "mean_treatment": 0.75},
            ],
        },
    }


def _observed_payload() -> dict[str, object]:
    return {
        "look_id": "interim",
        "control_successes": 8,
        "treatment_successes": 14,
    }


def _beta_prior_sensitivity_campaign() -> dict[str, object]:
    return {
        "schema_version": "nextstat_beta_binomial_prior_sensitivity_campaign_v0",
        "variants": [
            {
                "id": "skeptical",
                "control_prior": {"alpha": 1.0, "beta": 1.0},
                "treatment_prior": {"alpha": 1.0, "beta": 8.0},
            },
            {
                "id": "enthusiastic",
                "control_prior": {"alpha": 1.0, "beta": 1.0},
                "treatment_prior": {"alpha": 8.0, "beta": 1.0},
            },
        ],
    }


def _beta_extension_prior_sensitivity_campaign() -> dict[str, object]:
    return {
        "schema_version": "nextstat_beta_binomial_prior_sensitivity_campaign_v0",
        "variants": [
            {
                "id": "skeptical",
                "control_prior": {"alpha": 1.0, "beta": 1.0},
                "treatment_prior": {"alpha": 1.0, "beta": 8.0},
            }
        ],
    }


def _normal_prior_sensitivity_campaign() -> dict[str, object]:
    return {
        "schema_version": "nextstat_normal_normal_prior_sensitivity_campaign_v0",
        "variants": [
            {
                "id": "skeptical",
                "control_prior": {"mean": 0.0, "sd": 10.0},
                "treatment_prior": {"mean": -1.0, "sd": 0.2},
            },
            {
                "id": "enthusiastic",
                "control_prior": {"mean": 0.0, "sd": 10.0},
                "treatment_prior": {"mean": 1.0, "sd": 0.2},
            },
        ],
    }


def _normal_extension_prior_sensitivity_campaign() -> dict[str, object]:
    return {
        "schema_version": "nextstat_normal_normal_prior_sensitivity_campaign_v0",
        "variants": [
            {
                "id": "skeptical",
                "control_prior": {"mean": 0.0, "sd": 10.0},
                "treatment_prior": {"mean": -1.0, "sd": 0.2},
            }
        ],
    }


def _extension_design_spec() -> dict[str, object]:
    spec = json.loads(json.dumps(_design_spec()))
    spec["simulation"] = {
        "n_replicates": 1,
        "seed": 321,
        "scenarios": [
            {"id": "alt", "p_control": 0.40, "p_treatment": 0.60},
        ],
    }
    return spec


def _extension_normal_design_spec() -> dict[str, object]:
    spec = json.loads(json.dumps(_normal_design_spec()))
    spec["simulation"] = {
        "n_replicates": 1,
        "seed": 654,
        "scenarios": [
            {"id": "alt", "mean_control": 0.0, "mean_treatment": 0.75},
        ],
    }
    return spec


def _beta_historical_borrowing_policy() -> dict[str, object]:
    return {
        "schema_version": "nextstat_bayesian_historical_control_borrowing_policy_v0",
        "stability": "research-grade",
        "policy_id": "binary_superiority_demo_historical_control_policy_v0",
        "design_family": "beta_binomial",
        "borrowing_model": "power_prior",
        "historical_sources": [
            {
                "source_id": "study_2019_control",
                "source_role": "external_control_arm",
                "planned_control_sample_size": 120,
                "exchangeability_assessment": "moderate",
                "data_cut_label": "csr-final",
            }
        ],
        "eligibility": {
            "minimum_current_control_sample_size": 20,
            "minimum_control_information_fraction": 0.5,
            "disallow_recommended_action_flip": True,
        },
        "borrowing_strength": {
            "full_borrowing_fraction": 0.35,
            "tapered_borrowing_fraction": 0.15,
            "suspended_borrowing_fraction": 0.0,
        },
        "conflict_response": {
            "full_borrowing_max_conflict_severity": "low",
            "tapered_borrowing_max_conflict_severity": "moderate",
            "max_eventual_success_probability_range_for_full_borrowing": 0.15,
            "max_expected_total_sample_size_range_fraction_for_full_borrowing": 0.10,
        },
    }


def _normal_historical_borrowing_policy() -> dict[str, object]:
    return {
        "schema_version": "nextstat_bayesian_historical_control_borrowing_policy_v0",
        "stability": "research-grade",
        "policy_id": "continuous_superiority_demo_historical_control_policy_v0",
        "design_family": "normal_normal",
        "borrowing_model": "commensurate",
        "historical_sources": [
            {
                "source_id": "study_2022_control",
                "source_role": "external_control_arm",
                "planned_control_sample_size": 80,
                "exchangeability_assessment": "moderate",
                "data_cut_label": "db-lock",
            }
        ],
        "eligibility": {
            "minimum_current_control_sample_size": 10,
            "minimum_control_information_fraction": 0.5,
            "disallow_recommended_action_flip": True,
        },
        "borrowing_strength": {
            "full_borrowing_fraction": 0.30,
            "tapered_borrowing_fraction": 0.10,
            "suspended_borrowing_fraction": 0.0,
        },
        "conflict_response": {
            "full_borrowing_max_conflict_severity": "low",
            "tapered_borrowing_max_conflict_severity": "moderate",
            "max_eventual_success_probability_range_for_full_borrowing": 0.12,
            "max_expected_total_sample_size_range_fraction_for_full_borrowing": 0.10,
        },
    }


def _beta_robust_mixture_policy() -> dict[str, object]:
    return {
        "schema_version": "nextstat_bayesian_robust_mixture_prior_policy_v0",
        "stability": "research-grade",
        "policy_id": "binary_superiority_demo_robust_mixture_policy_v0",
        "design_family": "beta_binomial",
        "mixture_model": "robust_mixture_beta",
        "prior_target": "control_prior",
        "mixture_components": [
            {
                "component_id": "historical_informative",
                "component_role": "informative",
                "base_weight": 0.70,
                "alpha": 18.0,
                "beta": 42.0,
            },
            {
                "component_id": "weak_reference",
                "component_role": "weak_reference",
                "base_weight": 0.30,
                "alpha": 1.0,
                "beta": 1.0,
            },
        ],
        "eligibility": {
            "minimum_information_fraction": 0.5,
            "disallow_recommended_action_flip": True,
        },
        "weight_schedule": {
            "retain_informative_weight": 0.70,
            "tapered_informative_weight": 0.35,
            "fallback_informative_weight": 0.0,
        },
        "conflict_response": {
            "retain_max_conflict_severity": "low",
            "tapered_max_conflict_severity": "moderate",
            "max_eventual_success_probability_range_for_retain": 0.15,
            "max_expected_total_sample_size_range_fraction_for_retain": 0.10,
        },
    }


def _normal_robust_mixture_policy() -> dict[str, object]:
    return {
        "schema_version": "nextstat_bayesian_robust_mixture_prior_policy_v0",
        "stability": "research-grade",
        "policy_id": "continuous_superiority_demo_robust_mixture_policy_v0",
        "design_family": "normal_normal",
        "mixture_model": "robust_mixture_normal",
        "prior_target": "treatment_prior",
        "mixture_components": [
            {
                "component_id": "historical_informative",
                "component_role": "informative",
                "base_weight": 0.65,
                "mean": 0.60,
                "sd": 0.30,
            },
            {
                "component_id": "weak_reference",
                "component_role": "weak_reference",
                "base_weight": 0.35,
                "mean": 0.0,
                "sd": 10.0,
            },
        ],
        "eligibility": {
            "minimum_information_fraction": 0.5,
            "disallow_recommended_action_flip": True,
        },
        "weight_schedule": {
            "retain_informative_weight": 0.65,
            "tapered_informative_weight": 0.25,
            "fallback_informative_weight": 0.0,
        },
        "conflict_response": {
            "retain_max_conflict_severity": "low",
            "tapered_max_conflict_severity": "moderate",
            "max_eventual_success_probability_range_for_retain": 0.12,
            "max_expected_total_sample_size_range_fraction_for_retain": 0.10,
        },
    }


def _beta_report() -> dict[str, object]:
    return {
        "schema_version": "nextstat_beta_binomial_design_report_v0",
        "stability": "research-grade",
        "design_family": "beta_binomial",
        "design_spec": _design_spec(),
        "current_analysis": {
            "schema_version": "nextstat_beta_binomial_design_analysis_v0",
            "stability": "research-grade",
            "design_id": "binary_superiority_demo",
            "look": {"id": "interim", "n_control": 20, "n_treatment": 20},
            "observed": _observed_payload(),
            "posterior": {
                "control": {
                    "alpha": 1.0,
                    "beta": 1.0,
                    "mean": 0.5,
                    "ci_lower": 0.1,
                    "ci_upper": 0.9,
                },
                "treatment": {
                    "alpha": 1.0,
                    "beta": 1.0,
                    "mean": 0.5,
                    "ci_lower": 0.1,
                    "ci_upper": 0.9,
                },
                "effect_difference": {
                    "margin": 0.0,
                    "posterior_mean": 0.0,
                    "posterior_probability_gt_margin": 0.5,
                },
            },
            "decision": {
                "success": False,
                "futility": False,
                "recommended_action": "continue",
                "posterior_probability_gt_margin": 0.5,
                "success_threshold": 0.95,
                "futility_threshold": 0.2,
                "margin": 0.0,
            },
        },
        "operating_characteristics": {
            "schema_version": "nextstat_beta_binomial_operating_characteristics_v0",
            "stability": "research-grade",
            "design_id": "binary_superiority_demo",
            "n_replicates": 32,
            "seed": 123,
            "scenarios": [],
        },
        "posterior_predictive": {
            "schema_version": "nextstat_beta_binomial_posterior_predictive_v0",
            "stability": "research-grade",
            "design_id": "binary_superiority_demo",
            "current_analysis": {
                "schema_version": "nextstat_beta_binomial_design_analysis_v0",
                "decision": {"recommended_action": "continue"},
            },
            "n_replicates": 32,
            "seed": 123,
            "current_total_sample_size": 40.0,
            "expected_total_sample_size": 60.0,
            "expected_remaining_sample_size": 20.0,
            "eventual_success_probability": 0.4,
            "eventual_futility_probability": 0.2,
            "eventual_no_decision_probability": 0.4,
            "future_look_summaries": [],
        },
        "prior_sensitivity": {
            "schema_version": "nextstat_beta_binomial_prior_sensitivity_report_v0",
            "stability": "research-grade",
            "design_id": "binary_superiority_demo",
            "look": {"id": "interim", "n_control": 20, "n_treatment": 20},
            "observed": _observed_payload(),
            "n_replicates": 32,
            "seed": 123,
            "variants": [
                {
                    "variant_id": "baseline",
                    "is_baseline": True,
                    "control_prior": {"alpha": 1.0, "beta": 1.0},
                    "treatment_prior": {"alpha": 1.0, "beta": 1.0},
                    "posterior_mean": 0.0,
                    "posterior_probability_gt_margin": 0.5,
                    "recommended_action": "continue",
                    "eventual_success_probability": 0.4,
                    "eventual_futility_probability": 0.2,
                    "eventual_no_decision_probability": 0.4,
                    "expected_total_sample_size": 60.0,
                    "expected_remaining_sample_size": 20.0,
                    "future_look_summaries": [],
                    "posterior_probability_delta_vs_baseline": 0.0,
                    "eventual_success_probability_delta_vs_baseline": 0.0,
                    "expected_total_sample_size_delta_vs_baseline": 0.0,
                },
                {
                    "variant_id": "skeptical",
                    "is_baseline": False,
                    "control_prior": {"alpha": 1.0, "beta": 1.0},
                    "treatment_prior": {"alpha": 1.0, "beta": 3.0},
                    "posterior_mean": -0.02,
                    "posterior_probability_gt_margin": 0.46,
                    "recommended_action": "continue",
                    "eventual_success_probability": 0.34,
                    "eventual_futility_probability": 0.26,
                    "eventual_no_decision_probability": 0.40,
                    "expected_total_sample_size": 64.0,
                    "expected_remaining_sample_size": 24.0,
                    "future_look_summaries": [],
                    "posterior_probability_delta_vs_baseline": -0.04,
                    "eventual_success_probability_delta_vs_baseline": -0.06,
                    "expected_total_sample_size_delta_vs_baseline": 4.0,
                },
            ],
        },
        "provenance": {
            "software_name": "nextstat",
            "software_version": "0.0.0",
            "design_schema_version": "nextstat_beta_binomial_design_v0",
            "analysis_schema_version": "nextstat_beta_binomial_design_analysis_v0",
            "operating_characteristics_schema_version": "nextstat_beta_binomial_operating_characteristics_v0",
            "posterior_predictive_schema_version": "nextstat_beta_binomial_posterior_predictive_v0",
            "prior_sensitivity_campaign_schema_version": "nextstat_beta_binomial_prior_sensitivity_campaign_v0",
            "prior_sensitivity_report_schema_version": "nextstat_beta_binomial_prior_sensitivity_report_v0",
            "simulation_seed": 123,
            "n_replicates": 32,
        },
    }


def _normal_report() -> dict[str, object]:
    return {
        "schema_version": "nextstat_normal_normal_design_report_v0",
        "stability": "research-grade",
        "design_family": "normal_normal",
        "design_spec": _normal_design_spec(),
        "current_analysis": {
            "schema_version": "nextstat_normal_normal_design_analysis_v0",
            "stability": "research-grade",
            "design_id": "continuous_superiority_demo",
            "look": {"id": "interim", "n_control": 10, "n_treatment": 10},
            "observed": {
                "look_id": "interim",
                "control_sample_mean": 0.1,
                "treatment_sample_mean": 1.0,
            },
            "posterior": {
                "control": {
                    "posterior_mean": 0.1,
                    "posterior_sd": 0.2,
                    "ci_lower": -0.1,
                    "ci_upper": 0.3,
                },
                "treatment": {
                    "posterior_mean": 1.0,
                    "posterior_sd": 0.2,
                    "ci_lower": 0.8,
                    "ci_upper": 1.2,
                },
                "effect_difference": {
                    "margin": 0.0,
                    "posterior_mean": 0.9,
                    "posterior_sd": 0.3,
                    "ci_lower": 0.1,
                    "ci_upper": 1.7,
                    "posterior_probability_gt_margin": 0.99,
                },
            },
            "decision": {
                "success": True,
                "futility": False,
                "recommended_action": "stop_for_success",
                "posterior_probability_gt_margin": 0.99,
                "success_threshold": 0.975,
                "futility_threshold": 0.1,
                "margin": 0.0,
            },
        },
        "operating_characteristics": {
            "schema_version": "nextstat_normal_normal_operating_characteristics_v0",
            "stability": "research-grade",
            "design_id": "continuous_superiority_demo",
            "n_replicates": 32,
            "seed": 456,
            "scenarios": [],
        },
        "posterior_predictive": {
            "schema_version": "nextstat_normal_normal_posterior_predictive_v0",
            "stability": "research-grade",
            "design_id": "continuous_superiority_demo",
            "current_analysis": {
                "schema_version": "nextstat_normal_normal_design_analysis_v0",
                "decision": {"recommended_action": "stop_for_success"},
            },
            "n_replicates": 32,
            "seed": 456,
            "current_total_sample_size": 20.0,
            "expected_total_sample_size": 20.0,
            "expected_remaining_sample_size": 0.0,
            "eventual_success_probability": 1.0,
            "eventual_futility_probability": 0.0,
            "eventual_no_decision_probability": 0.0,
            "future_look_summaries": [],
        },
        "prior_sensitivity": {
            "schema_version": "nextstat_normal_normal_prior_sensitivity_report_v0",
            "stability": "research-grade",
            "design_id": "continuous_superiority_demo",
            "look": {"id": "interim", "n_control": 10, "n_treatment": 10},
            "observed": {
                "look_id": "interim",
                "control_sample_mean": 0.1,
                "treatment_sample_mean": 1.0,
            },
            "n_replicates": 32,
            "seed": 456,
            "variants": [
                {
                    "variant_id": "baseline",
                    "is_baseline": True,
                    "control_prior": {"mean": 0.0, "sd": 10.0},
                    "treatment_prior": {"mean": 0.0, "sd": 10.0},
                    "posterior_mean": 0.9,
                    "posterior_probability_gt_margin": 0.99,
                    "recommended_action": "stop_for_success",
                    "eventual_success_probability": 1.0,
                    "eventual_futility_probability": 0.0,
                    "eventual_no_decision_probability": 0.0,
                    "expected_total_sample_size": 20.0,
                    "expected_remaining_sample_size": 0.0,
                    "future_look_summaries": [],
                    "posterior_probability_delta_vs_baseline": 0.0,
                    "eventual_success_probability_delta_vs_baseline": 0.0,
                    "expected_total_sample_size_delta_vs_baseline": 0.0,
                },
                {
                    "variant_id": "skeptical",
                    "is_baseline": False,
                    "control_prior": {"mean": 0.0, "sd": 10.0},
                    "treatment_prior": {"mean": -0.3, "sd": 0.5},
                    "posterior_mean": 0.45,
                    "posterior_probability_gt_margin": 0.88,
                    "recommended_action": "continue",
                    "eventual_success_probability": 0.78,
                    "eventual_futility_probability": 0.0,
                    "eventual_no_decision_probability": 0.22,
                    "expected_total_sample_size": 24.0,
                    "expected_remaining_sample_size": 4.0,
                    "future_look_summaries": [],
                    "posterior_probability_delta_vs_baseline": -0.11,
                    "eventual_success_probability_delta_vs_baseline": -0.22,
                    "expected_total_sample_size_delta_vs_baseline": 4.0,
                },
            ],
        },
        "provenance": {
            "software_name": "nextstat",
            "software_version": "0.0.0",
            "design_schema_version": "nextstat_normal_normal_design_v0",
            "analysis_schema_version": "nextstat_normal_normal_design_analysis_v0",
            "operating_characteristics_schema_version": "nextstat_normal_normal_operating_characteristics_v0",
            "posterior_predictive_schema_version": "nextstat_normal_normal_posterior_predictive_v0",
            "prior_sensitivity_campaign_schema_version": "nextstat_normal_normal_prior_sensitivity_campaign_v0",
            "prior_sensitivity_report_schema_version": "nextstat_normal_normal_prior_sensitivity_report_v0",
            "simulation_seed": 456,
            "n_replicates": 32,
        },
    }


def test_analyze_beta_binomial_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        return json.dumps(
            {
                "schema_version": "nextstat_beta_binomial_design_analysis_v0",
                "decision": {"recommended_action": "continue"},
            }
        )

    monkeypatch.setattr(ns_bayes_design, "_beta_binomial_design_analyze_json", _fake)

    out = ns_bayes_design.analyze_beta_binomial_design(
        _design_spec(), _observed_payload()
    )

    assert out["decision"]["recommended_action"] == "continue"
    assert (
        captured["spec"] and captured["spec"]["design_id"] == "binary_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"


def test_analyze_beta_binomial_design_accepts_paths(
    monkeypatch, tmp_path: Path
) -> None:
    spec = _design_spec()
    observed = _observed_payload()
    spec_path = tmp_path / "design.json"
    observed_path = tmp_path / "observed.json"
    spec_path.write_text(json.dumps(spec), encoding="utf-8")
    observed_path.write_text(json.dumps(observed), encoding="utf-8")

    def _fake(spec_json: str, observed_json: str) -> str:
        assert json.loads(spec_json) == spec
        assert json.loads(observed_json) == observed
        return json.dumps({"ok": True})

    monkeypatch.setattr(ns_bayes_design, "_beta_binomial_design_analyze_json", _fake)

    assert ns_bayes_design.analyze_beta_binomial_design(spec_path, observed_path) == {
        "ok": True
    }


def test_simulate_beta_binomial_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        return json.dumps(
            {
                "schema_version": "nextstat_beta_binomial_operating_characteristics_v0",
                "scenarios": [],
            }
        )

    monkeypatch.setattr(
        ns_bayes_design,
        "_beta_binomial_design_operating_characteristics_json",
        _fake,
    )

    out = ns_bayes_design.simulate_beta_binomial_design(_design_spec())

    assert (
        out["schema_version"] == "nextstat_beta_binomial_operating_characteristics_v0"
    )
    assert captured["spec"] and captured["spec"]["simulation"]["seed"] == 123


def test_forecast_beta_binomial_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        return json.dumps(
            {
                "schema_version": "nextstat_beta_binomial_posterior_predictive_v0",
                "eventual_success_probability": 0.5,
            }
        )

    monkeypatch.setattr(
        ns_bayes_design, "_beta_binomial_design_posterior_predictive_json", _fake
    )

    out = ns_bayes_design.forecast_beta_binomial_design(
        _design_spec(), _observed_payload()
    )

    assert out["schema_version"] == "nextstat_beta_binomial_posterior_predictive_v0"
    assert (
        captured["spec"] and captured["spec"]["design_id"] == "binary_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"


def test_analyze_beta_binomial_prior_sensitivity_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str, campaign_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        captured["campaign"] = json.loads(campaign_json)
        return json.dumps(
            {
                "schema_version": "nextstat_beta_binomial_prior_sensitivity_report_v0",
                "variants": [],
            }
        )

    monkeypatch.setattr(
        ns_bayes_design, "_beta_binomial_design_prior_sensitivity_json", _fake
    )

    out = ns_bayes_design.analyze_beta_binomial_prior_sensitivity(
        _design_spec(),
        _observed_payload(),
        _beta_prior_sensitivity_campaign(),
    )

    assert out["schema_version"] == "nextstat_beta_binomial_prior_sensitivity_report_v0"
    assert (
        captured["spec"] and captured["spec"]["design_id"] == "binary_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"
    assert captured["campaign"] and len(captured["campaign"]["variants"]) == 2


def test_build_beta_binomial_design_report_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str, campaign_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        captured["campaign"] = json.loads(campaign_json)
        return json.dumps(
            {
                "schema_version": "nextstat_beta_binomial_design_report_v0",
                "design_family": "beta_binomial",
            }
        )

    monkeypatch.setattr(ns_bayes_design, "_beta_binomial_design_report_json", _fake)

    out = ns_bayes_design.build_beta_binomial_design_report(
        _design_spec(),
        _observed_payload(),
        _beta_prior_sensitivity_campaign(),
    )

    assert out["schema_version"] == "nextstat_beta_binomial_design_report_v0"
    assert (
        captured["spec"] and captured["spec"]["design_id"] == "binary_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"
    assert captured["campaign"] and len(captured["campaign"]["variants"]) == 2


def test_render_beta_binomial_design_report_accepts_paths(
    monkeypatch, tmp_path: Path
) -> None:
    report = {
        "schema_version": "nextstat_beta_binomial_design_report_v0",
        "design_family": "beta_binomial",
    }
    report_path = tmp_path / "report.json"
    report_path.write_text(json.dumps(report), encoding="utf-8")

    def _fake(report_json: str) -> str:
        assert json.loads(report_json) == report
        return "# Binary design report\n"

    monkeypatch.setattr(ns_bayes_design, "_beta_binomial_design_report_markdown", _fake)

    assert (
        ns_bayes_design.render_beta_binomial_design_report(report_path)
        == "# Binary design report\n"
    )


def test_build_beta_binomial_regulatory_appendix_accepts_path(tmp_path: Path) -> None:
    report_path = tmp_path / "report.json"
    report_path.write_text(json.dumps(_beta_report()), encoding="utf-8")

    out = ns_bayes_design.build_beta_binomial_regulatory_appendix(report_path)

    assert out["schema_version"] == "nextstat_bayesian_design_regulatory_appendix_v0"
    assert out["design_family"] == "beta_binomial"
    assert out["required_sections"] == [
        "design_summary",
        "prior_specification",
        "decision_rules",
        "current_analysis",
        "operating_characteristics",
        "posterior_predictive",
        "prior_sensitivity",
        "provenance",
    ]
    assert out["sections"]["design_summary"]["total_planned_sample_size"] == 80
    assert out["sections"]["current_analysis"]["look_id"] == "interim"
    assert out["sections"]["provenance"]["software_name"] == "nextstat"


def test_render_bayesian_regulatory_appendix_markdown_accepts_path(
    tmp_path: Path,
) -> None:
    appendix_path = tmp_path / "appendix.json"
    appendix_path.write_text(
        json.dumps(
            ns_bayes_design.build_beta_binomial_regulatory_appendix(_beta_report())
        ),
        encoding="utf-8",
    )

    out = ns_bayes_design.render_bayesian_regulatory_appendix_markdown(appendix_path)

    assert out.startswith("# Bayesian Regulatory Appendix")
    assert "binary_superiority_demo" in out
    assert "beta_binomial" in out


def test_build_beta_binomial_prior_conflict_diagnostic_accepts_path(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    report_path.write_text(json.dumps(_beta_report()), encoding="utf-8")

    out = ns_bayes_design.build_beta_binomial_prior_conflict_diagnostic(report_path)

    assert out["schema_version"] == "nextstat_bayesian_prior_conflict_diagnostic_v0"
    assert out["design_family"] == "beta_binomial"
    assert out["baseline_variant_id"] == "baseline"
    assert out["reported_variant_count"] == 2


def test_build_normal_normal_prior_conflict_diagnostic_accepts_path(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    report_path.write_text(json.dumps(_normal_report()), encoding="utf-8")

    out = ns_bayes_design.build_normal_normal_prior_conflict_diagnostic(report_path)

    assert out["schema_version"] == "nextstat_bayesian_prior_conflict_diagnostic_v0"
    assert out["design_family"] == "normal_normal"
    assert out["baseline_variant_id"] == "baseline"
    assert out["reported_variant_count"] == 2


def test_build_beta_binomial_historical_control_borrowing_review_accepts_paths(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    policy_path = tmp_path / "policy.json"
    report_path.write_text(json.dumps(_beta_report()), encoding="utf-8")
    policy_path.write_text(
        json.dumps(_beta_historical_borrowing_policy()), encoding="utf-8"
    )

    out = ns_bayes_design.build_beta_binomial_historical_control_borrowing_review(
        report_path,
        policy_path,
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_historical_control_borrowing_review_v0"
    )
    assert out["design_family"] == "beta_binomial"
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_historical_control_borrowing_policy_v0"
    )


def test_build_normal_normal_historical_control_borrowing_review_accepts_paths(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    policy_path = tmp_path / "policy.json"
    report_path.write_text(json.dumps(_normal_report()), encoding="utf-8")
    policy_path.write_text(
        json.dumps(_normal_historical_borrowing_policy()), encoding="utf-8"
    )

    out = ns_bayes_design.build_normal_normal_historical_control_borrowing_review(
        report_path,
        policy_path,
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_historical_control_borrowing_review_v0"
    )
    assert out["design_family"] == "normal_normal"
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_historical_control_borrowing_policy_v0"
    )


def test_simulate_beta_binomial_historical_control_borrowing_operating_characteristics_accepts_paths(
    tmp_path: Path,
) -> None:
    spec_path = tmp_path / "spec.json"
    campaign_path = tmp_path / "campaign.json"
    policy_path = tmp_path / "policy.json"
    spec_path.write_text(json.dumps(_extension_design_spec()), encoding="utf-8")
    campaign_path.write_text(
        json.dumps(_beta_extension_prior_sensitivity_campaign()),
        encoding="utf-8",
    )
    policy_path.write_text(
        json.dumps(_beta_historical_borrowing_policy()),
        encoding="utf-8",
    )

    out = (
        ns_bayes_design.simulate_beta_binomial_historical_control_borrowing_operating_characteristics(
            spec_path,
            campaign_path,
            policy_path,
        )
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_historical_control_borrowing_operating_characteristics_v0"
    )
    assert out["design_family"] == "beta_binomial"
    assert out["source_campaign_schema_version"] == (
        "nextstat_beta_binomial_prior_sensitivity_campaign_v0"
    )
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_historical_control_borrowing_policy_v0"
    )


def test_simulate_normal_normal_historical_control_borrowing_operating_characteristics_accepts_paths(
    tmp_path: Path,
) -> None:
    spec_path = tmp_path / "spec.json"
    campaign_path = tmp_path / "campaign.json"
    policy_path = tmp_path / "policy.json"
    spec_path.write_text(json.dumps(_extension_normal_design_spec()), encoding="utf-8")
    campaign_path.write_text(
        json.dumps(_normal_extension_prior_sensitivity_campaign()),
        encoding="utf-8",
    )
    policy_path.write_text(
        json.dumps(_normal_historical_borrowing_policy()),
        encoding="utf-8",
    )

    out = (
        ns_bayes_design.simulate_normal_normal_historical_control_borrowing_operating_characteristics(
            spec_path,
            campaign_path,
            policy_path,
        )
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_historical_control_borrowing_operating_characteristics_v0"
    )
    assert out["design_family"] == "normal_normal"
    assert out["source_campaign_schema_version"] == (
        "nextstat_normal_normal_prior_sensitivity_campaign_v0"
    )
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_historical_control_borrowing_policy_v0"
    )


def test_build_beta_binomial_robust_mixture_prior_review_accepts_paths(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    policy_path = tmp_path / "policy.json"
    report_path.write_text(json.dumps(_beta_report()), encoding="utf-8")
    policy_path.write_text(json.dumps(_beta_robust_mixture_policy()), encoding="utf-8")

    out = ns_bayes_design.build_beta_binomial_robust_mixture_prior_review(
        report_path,
        policy_path,
    )

    assert out["schema_version"] == "nextstat_bayesian_robust_mixture_prior_review_v0"
    assert out["design_family"] == "beta_binomial"
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_robust_mixture_prior_policy_v0"
    )


def test_simulate_beta_binomial_robust_mixture_prior_operating_characteristics_accepts_paths(
    tmp_path: Path,
) -> None:
    spec_path = tmp_path / "spec.json"
    campaign_path = tmp_path / "campaign.json"
    policy_path = tmp_path / "policy.json"
    spec_path.write_text(json.dumps(_extension_design_spec()), encoding="utf-8")
    campaign_path.write_text(
        json.dumps(_beta_extension_prior_sensitivity_campaign()),
        encoding="utf-8",
    )
    policy_path.write_text(
        json.dumps(_beta_robust_mixture_policy()),
        encoding="utf-8",
    )

    out = (
        ns_bayes_design.simulate_beta_binomial_robust_mixture_prior_operating_characteristics(
            spec_path,
            campaign_path,
            policy_path,
        )
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_robust_mixture_prior_operating_characteristics_v0"
    )
    assert out["design_family"] == "beta_binomial"
    assert out["source_campaign_schema_version"] == (
        "nextstat_beta_binomial_prior_sensitivity_campaign_v0"
    )
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_robust_mixture_prior_policy_v0"
    )


def test_build_normal_normal_robust_mixture_prior_review_accepts_paths(
    tmp_path: Path,
) -> None:
    report_path = tmp_path / "report.json"
    policy_path = tmp_path / "policy.json"
    report_path.write_text(json.dumps(_normal_report()), encoding="utf-8")
    policy_path.write_text(
        json.dumps(_normal_robust_mixture_policy()), encoding="utf-8"
    )

    out = ns_bayes_design.build_normal_normal_robust_mixture_prior_review(
        report_path,
        policy_path,
    )

    assert out["schema_version"] == "nextstat_bayesian_robust_mixture_prior_review_v0"
    assert out["design_family"] == "normal_normal"
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_robust_mixture_prior_policy_v0"
    )


def test_simulate_normal_normal_robust_mixture_prior_operating_characteristics_accepts_paths(
    tmp_path: Path,
) -> None:
    spec_path = tmp_path / "spec.json"
    campaign_path = tmp_path / "campaign.json"
    policy_path = tmp_path / "policy.json"
    spec_path.write_text(json.dumps(_extension_normal_design_spec()), encoding="utf-8")
    campaign_path.write_text(
        json.dumps(_normal_extension_prior_sensitivity_campaign()),
        encoding="utf-8",
    )
    policy_path.write_text(
        json.dumps(_normal_robust_mixture_policy()),
        encoding="utf-8",
    )

    out = (
        ns_bayes_design.simulate_normal_normal_robust_mixture_prior_operating_characteristics(
            spec_path,
            campaign_path,
            policy_path,
        )
    )

    assert (
        out["schema_version"]
        == "nextstat_bayesian_robust_mixture_prior_operating_characteristics_v0"
    )
    assert out["design_family"] == "normal_normal"
    assert out["source_campaign_schema_version"] == (
        "nextstat_normal_normal_prior_sensitivity_campaign_v0"
    )
    assert out["source_policy_schema_version"] == (
        "nextstat_bayesian_robust_mixture_prior_policy_v0"
    )


def test_write_bayesian_regulatory_appendix_pdf_accepts_dict(tmp_path: Path) -> None:
    out_pdf = tmp_path / "appendix.pdf"
    appendix = ns_bayes_design.build_normal_normal_regulatory_appendix(_normal_report())

    has_mpl = importlib.util.find_spec("matplotlib") is not None
    if not has_mpl:
        with pytest.raises(ImportError):
            ns_bayes_design.write_bayesian_regulatory_appendix_pdf(out_pdf, appendix)
        return

    ns_bayes_design.write_bayesian_regulatory_appendix_pdf(out_pdf, appendix)

    assert out_pdf.exists()
    assert out_pdf.read_bytes().startswith(b"%PDF-")


def test_analyze_normal_normal_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        return json.dumps(
            {
                "schema_version": "nextstat_normal_normal_design_analysis_v0",
                "decision": {"recommended_action": "continue"},
            }
        )

    monkeypatch.setattr(ns_bayes_design, "_normal_normal_design_analyze_json", _fake)

    out = ns_bayes_design.analyze_normal_normal_design(
        _normal_design_spec(),
        {
            "look_id": "interim",
            "control_sample_mean": 0.1,
            "treatment_sample_mean": 1.0,
        },
    )

    assert out["decision"]["recommended_action"] == "continue"
    assert (
        captured["spec"]
        and captured["spec"]["design_id"] == "continuous_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"


def test_simulate_normal_normal_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        return json.dumps(
            {
                "schema_version": "nextstat_normal_normal_operating_characteristics_v0",
                "scenarios": [],
            }
        )

    monkeypatch.setattr(
        ns_bayes_design,
        "_normal_normal_design_operating_characteristics_json",
        _fake,
    )

    out = ns_bayes_design.simulate_normal_normal_design(_normal_design_spec())

    assert (
        out["schema_version"] == "nextstat_normal_normal_operating_characteristics_v0"
    )
    assert captured["spec"] and captured["spec"]["simulation"]["seed"] == 456


def test_forecast_normal_normal_design_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        return json.dumps(
            {
                "schema_version": "nextstat_normal_normal_posterior_predictive_v0",
                "eventual_success_probability": 0.5,
            }
        )

    monkeypatch.setattr(
        ns_bayes_design, "_normal_normal_design_posterior_predictive_json", _fake
    )

    out = ns_bayes_design.forecast_normal_normal_design(
        _normal_design_spec(),
        {
            "look_id": "interim",
            "control_sample_mean": 0.1,
            "treatment_sample_mean": 1.0,
        },
    )

    assert out["schema_version"] == "nextstat_normal_normal_posterior_predictive_v0"
    assert (
        captured["spec"]
        and captured["spec"]["design_id"] == "continuous_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"


def test_analyze_normal_normal_prior_sensitivity_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str, campaign_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        captured["campaign"] = json.loads(campaign_json)
        return json.dumps(
            {
                "schema_version": "nextstat_normal_normal_prior_sensitivity_report_v0",
                "variants": [],
            }
        )

    monkeypatch.setattr(
        ns_bayes_design, "_normal_normal_design_prior_sensitivity_json", _fake
    )

    out = ns_bayes_design.analyze_normal_normal_prior_sensitivity(
        _normal_design_spec(),
        {
            "look_id": "interim",
            "control_sample_mean": 0.1,
            "treatment_sample_mean": 1.0,
        },
        _normal_prior_sensitivity_campaign(),
    )

    assert out["schema_version"] == "nextstat_normal_normal_prior_sensitivity_report_v0"
    assert (
        captured["spec"]
        and captured["spec"]["design_id"] == "continuous_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"
    assert captured["campaign"] and len(captured["campaign"]["variants"]) == 2


def test_build_normal_normal_design_report_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(spec_json: str, observed_json: str, campaign_json: str) -> str:
        captured["spec"] = json.loads(spec_json)
        captured["observed"] = json.loads(observed_json)
        captured["campaign"] = json.loads(campaign_json)
        return json.dumps(
            {
                "schema_version": "nextstat_normal_normal_design_report_v0",
                "design_family": "normal_normal",
            }
        )

    monkeypatch.setattr(ns_bayes_design, "_normal_normal_design_report_json", _fake)

    out = ns_bayes_design.build_normal_normal_design_report(
        _normal_design_spec(),
        {
            "look_id": "interim",
            "control_sample_mean": 0.1,
            "treatment_sample_mean": 1.0,
        },
        _normal_prior_sensitivity_campaign(),
    )

    assert out["schema_version"] == "nextstat_normal_normal_design_report_v0"
    assert (
        captured["spec"]
        and captured["spec"]["design_id"] == "continuous_superiority_demo"
    )
    assert captured["observed"] and captured["observed"]["look_id"] == "interim"
    assert captured["campaign"] and len(captured["campaign"]["variants"]) == 2


def test_render_normal_normal_design_report_accepts_dict(monkeypatch) -> None:
    captured: dict[str, object] = {}

    def _fake(report_json: str) -> str:
        captured["report"] = json.loads(report_json)
        return "# Continuous design report\n"

    monkeypatch.setattr(ns_bayes_design, "_normal_normal_design_report_markdown", _fake)

    out = ns_bayes_design.render_normal_normal_design_report(
        {
            "schema_version": "nextstat_normal_normal_design_report_v0",
            "design_family": "normal_normal",
        }
    )

    assert out == "# Continuous design report\n"
    assert captured["report"] and captured["report"]["design_family"] == "normal_normal"


def test_build_normal_normal_regulatory_appendix_accepts_dict() -> None:
    out = ns_bayes_design.build_normal_normal_regulatory_appendix(_normal_report())

    assert out["schema_version"] == "nextstat_bayesian_design_regulatory_appendix_v0"
    assert out["design_family"] == "normal_normal"
    assert out["sections"]["design_summary"]["total_planned_sample_size"] == 40
    assert out["sections"]["prior_specification"]["likelihood"] == {
        "known_sd_control": 1.0,
        "known_sd_treatment": 1.0,
    }
    assert (
        out["sections"]["current_analysis"]["recommended_action"] == "stop_for_success"
    )
    assert out["sections"]["posterior_predictive"]["expected_total_sample_size"] == 20.0


def test_write_beta_binomial_design_report_bundle_accepts_dict(
    monkeypatch, tmp_path: Path
) -> None:
    captured: dict[str, object] = {}

    def _fake(report_json: str) -> str:
        captured["report"] = json.loads(report_json)
        return "# Binary design report\n"

    monkeypatch.setattr(ns_bayes_design, "render_beta_binomial_design_report", _fake)

    summary = ns_bayes_design.write_beta_binomial_design_report_bundle(
        tmp_path / "bundle",
        _beta_report(),
    )

    assert summary["schema_version"] == "nextstat_bayesian_design_report_bundle_v0"
    assert summary["design_family"] == "beta_binomial"
    assert summary["deterministic"] is True
    assert (
        summary["artifact_paths"]["design_report_markdown"]
        == "outputs/design_report.md"
    )
    assert summary["artifact_paths"]["run_bundle_meta"] == "meta.json"
    assert summary["artifact_paths"]["run_bundle_manifest"] == "manifest.json"
    assert captured["report"] and captured["report"]["schema_version"] == (
        "nextstat_beta_binomial_design_report_v0"
    )

    meta = json.loads((tmp_path / "bundle" / "meta.json").read_text(encoding="utf-8"))
    assert meta["input"]["original_path"] == "<inline-beta_binomial-design-report.json>"
    assert meta["created_unix_ms"] == 0


def test_write_normal_normal_design_report_bundle_accepts_path(
    monkeypatch, tmp_path: Path
) -> None:
    report = _normal_report()
    report_path = tmp_path / "normal_report.json"
    report_path.write_text(json.dumps(report), encoding="utf-8")

    def _fake(report_json: str) -> str:
        assert json.loads(report_json)["design_family"] == "normal_normal"
        return "# Continuous design report\n"

    monkeypatch.setattr(ns_bayes_design, "render_normal_normal_design_report", _fake)

    summary = ns_bayes_design.write_normal_normal_design_report_bundle(
        tmp_path / "bundle",
        report_path,
    )

    assert summary["schema_version"] == "nextstat_bayesian_design_report_bundle_v0"
    assert summary["report_schema_version"] == "nextstat_normal_normal_design_report_v0"
    assert summary["artifact_paths"]["run_bundle_meta"] == "meta.json"
    meta = json.loads((tmp_path / "bundle" / "meta.json").read_text(encoding="utf-8"))
    assert meta["input"]["original_path"] == str(report_path)


def test_write_beta_binomial_design_report_bundle_rejects_family_mismatch(
    tmp_path: Path,
) -> None:
    with pytest.raises(ValueError, match="report.design_family must be beta_binomial"):
        ns_bayes_design.write_beta_binomial_design_report_bundle(
            tmp_path / "bundle",
            _normal_report(),
        )
