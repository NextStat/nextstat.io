...
def pytest_terminal_summary(terminalreporter: Any, exitstatus: int, config: pytest.Config) -> None:
    ...
    for nodeid in sorted(by_test.keys()):
        ...
        if "nextstat" not in labels:
            continue
        ref_total = sum(v for k, v in labels.items() if k != "nextstat")
        if ref_total <= 0.0:
            continue
        terminalreporter.write_line(
            f"{nodeid}: nextstat={_fmt(labels['nextstat'])} ref={_fmt(ref_total)} ({', '.join(sorted(k for k in labels.keys() if k != 'nextstat'))})"
        )
        shown += 1
        if shown >= 25:
