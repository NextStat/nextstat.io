...
def test_apex2_simplified_likelihood_report_public_fixture_matrix_smoke():
    ...
    # This smoke focuses on the committed public fixture matrix contract.
    # The base synthetic smoke case has its own exact return-code check above,
    # and its performance gate can drift slightly cross-platform.
    ...
    if proc.returncode != 0:
        assert generated["summary"]["status"] == "fail"
        assert generated["summary"]["public_fixture_matrix_status"] == "ok"
        assert generated["cases"][0]["gates"]["performance"]["end_to_end_upper_limit_speedup"] is False
...
def test_apex2_simplified_likelihood_report_export_matrix_with_public_cases_smoke():
    ...
    assert {case["name"] for case in public_cases} == {
        ...
        "cms_public_sr_vr_dual_cr_gaussian_export_stable_example",
    }
