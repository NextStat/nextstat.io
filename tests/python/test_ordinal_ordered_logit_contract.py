"""Contract tests for ordered logistic regression (Phase 9.C)."""
...
def _sample_ordered_logit_y(rng: random.Random, *, eta: float, cuts: list[float]) -> int:
    if len(cuts) < 1:
    ...
    cdf = [_sigmoid(float(cuts[0]) - float(eta))]
    for c in cuts[1:]:
        cdf.append(_sigmoid(float(c) - float(eta)))
    probs = [cdf[0]]
...
def test_ordered_logit_fit_recovers_sign_and_predict_proba_shapes():
    import nextstat

    rng = random.Random(0)

    beta_true = 1.25
    cuts_true = [-0.5, 0.5]  # 3 levels
    ...
    for _ in range(n):
        xi = _randn(rng)
        eta = beta_true * xi
        yi = _sample_ordered_logit_y(rng, eta=eta, cuts=cuts_true)
        x.append([xi])
        y.append(yi)
    ...
    # In practice this should converge for this synthetic dataset.
    ...
    # Monotonic sanity: larger x (with beta>0) should push mass to higher categories.
    assert float(ps[2][2]) > float(ps[0][2])


def test_ordered_logit_optional_parity_vs_statsmodels_predict_proba(ns_timing):
    ...
    with ns_timing.time("nextstat"):
        ns_fit = nextstat.ordinal.ordered_logit.fit(x, y, n_levels=3)
