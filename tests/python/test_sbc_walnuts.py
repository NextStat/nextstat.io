"""Simulation-Based Calibration (SBC) for WALNUTS.
...
Run with:
  NS_RUN_SLOW=1 NS_RUN_SBC=1 NS_SBC_RUNS=30 NS_SBC_WARMUP=300 NS_SBC_SAMPLES=300 \
    PYTHONPATH=bindings/ns-py/python ./.venv/bin/python -m pytest -q -m slow tests/python/test_sbc_walnuts.py
"""
...
def test_sbc_linear_regression_1d_mean_only():
    run_sbc_linear_regression_1d_mean_only("walnuts")


def test_sbc_linear_regression_2d():
    run_sbc_linear_regression_2d("walnuts")


def test_sbc_random_intercept_gaussian_smoke():
    run_sbc_random_intercept_gaussian_smoke("walnuts")


def test_sbc_random_intercept_bernoulli_smoke():
    run_sbc_random_intercept_bernoulli_smoke("walnuts")


def test_walnuts_quality_gate_glm_strict():
    run_quality_gate_glm_strict("walnuts")


def test_walnuts_moments_golden_gaussian_strict():
    run_moments_golden_gaussian_strict("walnuts")
