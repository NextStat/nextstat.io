...
def test_release_workflow_publishes_m15_artifacts() -> None:
    ...
    assert "dist/**/m15_config.json" in workflow
    assert "dist/**/m15_assessment_table.json" in workflow
    assert "dist/**/m15_map.json" in workflow
    assert "dist/**/m15_mar.json" in workflow
    assert "dist/**/m15_profile_diff_report.json" in workflow
    assert "dist/**/m15_profile_diff_report_v1.schema.json" in workflow
    assert "dist/**/m15_report.md" in workflow
    assert "dist/**/m15_report.pdf" in workflow
    assert "dist/**/m15_report.docx" in workflow
    assert "dist/**/m15_bundle_manifest.json" in workflow
    assert "dist/**/m15_bundle_manifest_v1.schema.json" in workflow
    assert "dist/**/m15_bundle_manifest.sha256" in workflow
    assert "dist/**/m15_bundle_manifest.sha256.bin" in workflow
    assert "dist/**/m15_snapshot_index.json" in workflow
    assert "dist/**/m15_reporting_benchmark.json" in workflow
    assert "dist/**/m15_reporting_benchmark.md" in workflow
    assert "dist/**/m15_reporting_compare.json" in workflow
    assert "dist/**/release_candidate_bundle_manifest.json" in workflow
    assert "dist/**/release_manifest.json" in workflow
    assert "dist/**/release_manifest_v1.schema.json" in workflow
