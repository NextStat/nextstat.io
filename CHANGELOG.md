...
- **Committed MAMS host-backed snapshot refreshed** — the tracked public snapshot set now also includes `manifests/snapshots/mams-publisher-20260309T173500Z/`, a host-backed `nextstat-bench` publish bundle with `mams_assessment.json`, `mams_benchmark_report.md`, a health-complete `README_snippet_mams.md`, and a committed registry row that now surfaces both `core_quality=passed` and `promotion_gate=passed` for the canonical MAMS stable surface.
- **Committed MAMS repeatability evidence refreshed** — the tracked host-backed repeatability bundle now lives at `benchmarks/nextstat-public-benchmarks/suites/mams/results_v1/`, validates under `validate_artifacts.py --strict`, keeps `dataset_seed=12345` fixed across canonical seeds, and surfaces aggregate health plus parity summaries from `nextstat-bench` evidence.
