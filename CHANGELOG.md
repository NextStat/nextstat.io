...
- **MAMS expanded stress lane is now fully green on nextstat-bench** — the supported hierarchical stress case `hier_random_intercept_non_centered` now runs with an explicit stress-lane-only `nextstat_mams init_l=2.0` override, which closes the remaining `max_r_hat` blocker without changing the canonical MAMS stable defaults. The stress harness now forwards that override explicitly, surfaces it in the suite/multiseed artifacts, and the checked-in host-backed bundle under `benchmarks/nextstat-public-benchmarks/suites/mams/stress_results_v1/` has been refreshed from `benchmarks/artifacts/mams_stress_validation_20260318T194110Z/nextstat-bench/` with `stress_readiness=passed`.
- **Unified Python sampling docs now match the landed MAMS stable contract** — `nextstat.sample()` no longer advertises the pre-stabilization `n_warmup=2000` MAMS default. The unified docstring now reflects the stabilized `n_warmup=3500` / `target_accept=0.985` public surface and explicitly lists `eps_jitter=0.0` as part of the CPU MAMS kwargs, with regression coverage to prevent future drift.
- **MAMS benchmark seed semantics are now explicit in tracked evidence** — `suites/mams/run.py` now writes `benchmark_seed`, `cold_start_seed`, `warm_start_seed`, `reported_draws_seed`, and `reported_draws_source` into each case artifact while keeping `config.seed` for backward compatibility. The canonical `mams_multiseed_summary.json` and `mams_stress_multiseed_summary.json` now also publish a schema-backed `seed_semantics` contract, the checked-in MAMS repeatability/stress bundles have been refreshed to carry those fields, and regression coverage now checks the runner and summary surfaces directly.
- **Committed MAMS host-backed snapshot refreshed again for seed-contract parity** — the tracked public snapshot set now also includes `manifests/snapshots/mams-publisher-20260318T201316Z/`, a fresh host-backed `nextstat-bench` publish bundle whose per-case MAMS artifacts carry the explicit `benchmark_seed` / `cold_start_seed` / `warm_start_seed` / `reported_draws_seed` contract already used in repeatability and stress evidence. The committed registry has been refreshed from that bundle, so the landed MAMS stable-surface snapshot is now contract-consistent across canonical publish, repeatability, and stress lanes.
- **Committed MAMS published snapshot now has a direct freshness guard** — `tests/python/test_snapshot_registry_freshness_smoke.py` now resolves the latest committed MAMS published snapshot through `manifests/snapshot_registry.json` and asserts the canonical case artifact carries `benchmark_seed`, `cold_start_seed`, `warm_start_seed`, `reported_draws_seed`, and `reported_draws_source`. This protects the landed MAMS stable-surface snapshot from silently falling behind the explicit seed contract on future publisher refreshes.

## [0.10.1] — 2026-03-17

### Changed

- **Apex2 prerelease gate split into governance vs performance outcomes** — `scripts/apex2/pre_release_gate.sh` now emits `tmp/apex2_pre_release_gate_summary.json` / `.md`, distinguishes governance/correctness failures from machine-specific performance baseline failures, and uses explicit exit codes for governance (`20`), performance (`21`), and infrastructure/baseline-state (`22`) outcomes. This makes local prerelease review and CI classification cleaner for the `0.10.1` release surface.

## [0.10.0] — 2026-03-10

### Added

- **Canonical Bayesian nextstat-bench runner** — added `scripts/benchmarks/bench_bayesian_suite_remote.sh` so the public Bayesian suite now has a dedicated host-backed lane that syncs a minimal snapshot, builds local `ns-py`, runs `suite.py` + `report.py` + `assess.py`, validates the returned bundle, and syncs artifacts back into `benchmarks/artifacts/`.
- **Canonical Bayesian publisher nextstat-bench runner** — added `scripts/benchmarks/publish_bayesian_snapshot_remote.sh` so the published Bayesian snapshot lane can be exercised end-to-end on `nextstat-bench` via `publish_snapshot.py --bayesian`, strict validation, and synced-back snapshot artifacts.
- **Canonical Bayesian multiseed repeatability lane** — added `scripts/benchmarks/bench_bayesian_multiseed_remote.sh` plus `suites/bayesian/derive_metrics.py`, so the checked-in Bayesian repeatability snapshot can now be refreshed from `nextstat-bench` with a schema-backed `bayesian_multiseed_summary.json` and `derived_metrics.json` derived directly from per-seed artifacts.
- **Canonical MAMS multiseed repeatability lane** — added `scripts/benchmarks/bench_mams_multiseed_remote.sh`, `suites/mams/multiseed.py`, and `nextstat.mams_multiseed_summary.v1`, so the checked-in MAMS repeatability bundle can now be refreshed from `nextstat-bench` with fixed `dataset_seed`, aggregated health metrics, and aggregated MAMS-vs-NUTS parity rows.
- **Expanded MAMS stress lane on nextstat-bench** — added `scripts/benchmarks/bench_mams_stress_multiseed_remote.sh`, `suites/mams/stress_suite.py`, `suites/mams/stress_multiseed.py`, `suites/mams/assess_stress_multiseed.py`, and the schema-backed `nextstat.mams_stress_*` artifact family. This lane tracks supported stress cases `neal_funnel_ncp_10d` and `hier_random_intercept_non_centered`, preserves `neal_funnel_10d_centered` as a pathological control, and refreshes a separate checked-in bundle under `suites/mams/stress_results_v1/` from `nextstat-bench` without rewriting the canonical MAMS promotion verdict.
- **ICH M15 stable reporting surface** — promoted the full frozen-artifact M15 layer: `nextstat m15 assessment-table`, `map`, `mar`, `profile-diff`, and `bundle` now ship as versioned deterministic contracts with jurisdiction profiles (`ich_core`, `ema_step5_2026`, `fda_draft_2024`), explicit human signoff roles, schema/export coverage, validation-pack integration, release assets, integrity sidecars, and dedicated regression/workflow gates.
- **Publishable M15 report outputs** — `validation-pack/render_validation_pack.sh --m15-config ...` now emits deterministic `m15_report.md`, `m15_report.pdf`, and `m15_report.docx` from frozen `assessment-table` / `MAP` / `MAR` / `profile-diff` / `bundle` artifacts only, without hidden model execution.
- **Ads-native conversion modeling surface** — added `ns_inference::{BetaBinomialModel, DelayCorrectionModel, hill, adstock_geometric}` plus the Python `nextstat.ads` convenience module / top-level native classes for overdispersed conversion priors, delay-censoring correction, and spend-response primitives. Added fixed weekly Kalman builder aliases across Rust, CLI JSON specs, and Python (`local_level_weekly`, `local_linear_trend_weekly`) and documented the stable reference surface.
- **Ads + weekly time-series stable-surface gate** — added a dedicated local/CI gate, benchmark harness, remote `nextstat-bench` runner, workflow, and release-governance docs for the promoted ads observation / weekly state-space surface.
- **Ads + weekly time-series accepted baseline runtime gate** — added committed `nextstat-bench` accepted baseline evidence plus compare / promotion / one-shot gate scripts, schemas, examples, and runtime-gate docs for release-grade baseline management of the promoted ads / weekly surface.
- **GVM stable-first verification lane** — dedicated gate script `scripts/gvm/stable_first_gate.sh`, GitHub Actions workflow `.github/workflows/gvm-stable-first.yml`, and `make gvm-stable-first-gate` target to protect the promoted scalar GVM subset across Rust core, CLI, Python, formatting, and required evidence documents.
- **GVM stable-first support matrix and release notes** — added `docs/benchmarks/gvm-stable-first-support-matrix-2026-03-07.md` and `docs/benchmarks/gvm-stable-first-release-notes-2026-03-07.md` to make the promoted stable subset operationally explicit.
- **GVM stable-first tabular + manifest ingress** — added `nextstat combine-measurements-build-spec`, `nextstat.hep.build_measurement_combination_spec(...)`, and `nextstat.hep.build_measurement_combination_spec_from_manifest(...)` so physicists can start from CSV/TSV bundles directly or from one short YAML/JSON manifest wrapper instead of hand-writing the canonical JSON spec.
- **GVM stable-first golden path runner** — added `make gvm-stable-first-example` and `scripts/gvm/run_stable_first_example.sh` so the committed example bundle can be executed end-to-end with one command.
- **Adoption playbook Route D for stable-first GVM** — added committed external-validation fixtures under `docs/guides/fixtures/route_d/` and extended `scripts/guides/verify_adoption_playbook.py` so scalar measurement combinations are now smoke-checked alongside routes A/B/C.
- **GVM stable-first release execution bundle** — added release PR and launch checklists (`docs/benchmarks/gvm-stable-first-release-pr-checklist-2026-03-07.md`, `docs/benchmarks/gvm-stable-first-launch-checklist-2026-03-07.md`) so the promoted subset now has explicit PR-ready and launch-ready maintainer checklists, not just readiness and policy notes.
- **GVM stable-first release candidate memo** — added `docs/benchmarks/gvm-stable-first-release-candidate-v0.10.0-2026-03-08.md` to pin the recommended milestone version, curated stable-scope changelog subset, and PR-ready release summary for the first stable GVM wave.

### Changed

- **Bayesian PyMC repeatability lane hardened** — `scripts/benchmarks/bench_bayesian_multiseed_remote.sh` now self-provisions `pymc` + `arviz` when requested, exports the same explicit `PYTENSOR_FLAGS=blas__ldflags=-lblas` default policy used by the single-run host-backed lane, and `suites/bayesian/derive_metrics.py` / `nextstat.bayesian_derived_metrics.v2` now include schema-backed PyMC ESS/leapfrog rows for multiseed repeatability bundles.
- **Bayesian multiseed summary made health-complete** — `suites/bayesian/multiseed.py` now exposes `divergence_rate`, `max_treedepth_rate`, `min_ebfmi`, and `min_ess_tail` arrays in `bayesian_multiseed_summary.json`, renders a worst-across-seeds health table in the Markdown summary, and adds `--reuse-existing` so existing `seed_*` bundles can be regenerated without paying for another benchmark rerun.
- **Bayesian canonical publisher lane made health-complete** — `suites/bayesian/assess.py` now emits a machine-readable `review_summary` inside `bayesian_assessment.json`, the assessment Markdown includes the same worst-case health table, `suites/bayesian/report.py` can consume the assessment artifact to surface a top-level health verdict in `README_snippet_bayesian.md`, and `scripts/publish_snapshot.py` now wires that assessment-backed verdict into published Bayesian snapshot summaries by default.
- **Snapshot index now surfaces suite health verdicts** — both `write_snapshot_index.py` entry points now auto-detect assessment artifacts and emit normalized `suite_health` rows in `snapshot_index.json`, so published snapshot registry surfaces can read `core_quality`, `promotion_gate`, failing cases, and assessment-backed review summaries without reopening nested `*_assessment.json` files.
- **Snapshot registry contract added** — both `write_snapshot_registry.py` entry points now consume one or more `snapshot_index.json` files and emit schema-backed `nextstat.snapshot_registry.v1` artifacts, so commit-backed registry/live-page automation can read the same normalized `suite_health` rows already published by each snapshot index instead of reparsing every snapshot tree.
- **Snapshot publisher now refreshes the registry by default** — `benchmarks/nextstat-public-benchmarks/scripts/publish_snapshot.py` now writes `snapshot_registry.json` automatically after a successful publish, keeping the canonical `manifests/` layout commit-backed without a manual follow-up step; custom `--out-root` runs default to a self-contained registry inside that root, and `scripts/benchmarks/publish_bayesian_snapshot_remote.sh` now syncs that full publish root back from `nextstat-bench`.
- **Snapshot registry freshness gate added** — `write_snapshot_registry.py` is now deterministic, supports `--check`, and the standalone public-benchmarks `Makefile` plus CI publish templates now fail if the committed `manifests/snapshot_registry.json` drifts behind the committed snapshot bundles. The committed registry now records relative `snapshot_index_path` values, making it repo-stable instead of host-path-specific.
- **Committed Bayesian host-backed snapshot refreshed** — the tracked public snapshot set now includes `manifests/snapshots/bayesian-publisher-20260309T111048Z/`, a health-complete `nextstat-bench` publish bundle with `bayesian_assessment.json`, case-level JSONs, and a promotion-passing Bayesian `suite_health` row surfaced in the committed `manifests/snapshot_registry.json`.
- **MAMS canonical publisher lane made health-complete** — `suites/mams/assess.py` now emits a machine-readable `review_summary` inside `mams_assessment.json`, `suites/mams/report.py` preserves the detailed suite-local `mams_benchmark_report.md` while also rendering a separate health-complete `README_snippet_mams.md`, and `scripts/publish_snapshot.py` now wires that assessment-backed summary into published MAMS snapshots by default.
- **Committed MAMS host-backed snapshot refreshed** — the tracked public snapshot set now also includes `manifests/snapshots/mams-publisher-20260312T003824Z/`, a host-backed `nextstat-bench` publish bundle with `mams_assessment.json`, `mams_benchmark_report.md`, a health-complete `README_snippet_mams.md`, and a committed registry row that surfaces both `core_quality=passed` and `promotion_gate=passed` for the canonical MAMS stable surface under the packaging-hardened remote runner contract.
- **Committed MAMS repeatability evidence refreshed** — the tracked host-backed repeatability bundle now lives at `benchmarks/nextstat-public-benchmarks/suites/mams/results_v1/`, validates under `validate_artifacts.py --strict`, keeps `dataset_seed=12345` fixed across canonical seeds, and is currently refreshed from `benchmarks/artifacts/mams_multiseed_validation_20260312T001153Z/nextstat-bench/` so the checked-in repeatability surface stays aligned with the packaging-hardened host-backed runner contract.
- **MAMS repeatability assessment split from raw summary** — `suites/mams/assess_multiseed.py` and `nextstat.mams_multiseed_assessment.v1` now emit a separate `mams_multiseed_assessment.json` / `.md` verdict beside the raw multiseed summary, `validate_artifacts.py` knows that schema, and `scripts/benchmarks/bench_mams_multiseed_remote.sh` now produces the assessment on `nextstat-bench` so repeatability failures do not need to be inferred from summary arrays.
- **MAMS stable CPU defaults retuned for repeatability-grade convergence** — the public MAMS surface now uses `n_warmup=3500`, `target_accept=0.985`, `max_leapfrog=1024`, and `eps_jitter=0.0` by default, and the canonical MAMS suite/multiseed lanes on `nextstat-bench` now use the same stabilized regime. The `3500` warmup point is the first host-backed setting in the current stabilization cycle that clears both `neal_funnel_2d` and `eight_schools` repeatability on the canonical seed set without regressing parity. The docs now also make `init_l=0.0` explicit as “use stable `sqrt(d)`” rather than an auto-tuning surface, and MAMS divergence telemetry now correctly counts early-terminated non-finite energy-error transitions instead of under-reporting them.
- **Host-backed benchmark runners now install local wheels without PyPI deps** — `nextstat-bench` remote runners no longer rely on `maturin develop` to resolve `nextstat-cli` from PyPI. They now build a local `nextstat-*.whl` from the synchronized source snapshot and install it with `pip install --no-deps`, so host-backed verification remains aligned with the monorepo even when published package versions lag behind the source tree.
- **Bayesian remote optional backend provisioning hardened** — `scripts/benchmarks/bench_bayesian_suite_remote.sh` now self-provisions the standard external-framework stack on `nextstat-bench`: requested `cmdstanpy` runs install `cmdstanpy`, provision or reuse a local tracked `vendor/cmdstan/cmdstan-*` toolchain inside the synced seed repo snapshot, and the Bayesian runner now prefers that vendor CmdStan toolchain over ambient host `~/.cmdstan` state at runtime; requested `pymc` runs now install `pymc` + `arviz` automatically and default to `PYTENSOR_FLAGS=blas__ldflags=-lblas` unless the operator overrides `PYTENSOR_FLAGS`. The canonical stable lane remains `backends=[nextstat]`, while `BENCH_EXTRA_PIP_PACKAGES` stays additive for non-default stacks such as `numpyro`.
- **Bayesian HistFactory external-backend contract tightened** — the canonical `histfactory_simple_8p` case now has an exact restricted mapping on the optional `cmdstanpy` and `pymc` lanes: `mu` stays bounded on `[0, 10]`, per-bin `shapesys` nuisances stay positive, and the auxiliary constraint remains the corresponding Poisson/Gamma Barlow-Beeston term from the tracked `simple_workspace.json` fixture. The runtime now validates that the fixture's `shapesys` name is a safe identifier for generated Stan/PyMC models, and `numpyro` explicitly reports `backend_not_supported_for_model` for this case instead of falling through an invalid model path.
- **Bayesian benchmark assessment split** — the public Bayesian suite now emits a separate `bayesian_assessment` artifact, publishes it through the snapshot seed publisher, validates it under its own schema, and documents the same `core_quality` vs `promotion_gate` separation already used for canonical MAMS evidence.
- **Bayesian checked-in repeatability artifacts made strict-schema-valid** — the multi-seed lane now has a dedicated `nextstat.bayesian_derived_metrics.v2` schema, so refreshed checked-in repeatability snapshots can validate cleanly under `validate_artifacts.py --strict` instead of carrying legacy unsigned JSON sidecars.
- **M15 release promotion path hardened** — CI and release workflows now validate the M15 profile-diff artifact, publish deterministic M15 report outputs, and rerender-check `m15_report.md` / `.pdf` / `.docx` alongside the existing M15 JSON bundle.
- **GVM stable-first surface formalized** — the foundational scalar measurement-combination path (`combine-measurements-build-spec`, `combine-measurements`, `...-calibrate`, `...-calibrate-study`, and matching `nextstat.hep` wrappers) is now backed by a dedicated repo verification lane and explicit release-hardening documentation, while scenario/campaign/parity/reporting layers remain research-grade by design.
- **MAMS stable-surface contract hardened** — CPU MAMS now reports non-zero warmup leapfrog totals, exposes per-draw MH acceptance probabilities instead of binary accept flags, rejects `metric="dense"` / `"auto"` explicitly so the public surface matches the actual diagonal-only implementation, ships with a canonical `nextstat-bench` remote suite runner plus schema-aligned `environment` benchmark artifacts, and now emits a separate MAMS assessment artifact so `core_quality` and `promotion_gate` are no longer conflated.

## [0.9.9] — 2026-03-04

### Added

- **TRExFitter template morphing (ghost samples)** — `Type: GHOST` samples with `Template: POI:value` are now parsed and used to build per-bin polynomial signal templates in HIST mode. Lagrange interpolation converts anchor points into polynomial coefficients attached as a `TemplateMorphing` modifier, making signal yield depend on the POI (e.g. Ytsq). All evaluation paths (f64, generic/AD, reverse-mode tape) support the new modifier.
...
- **PD models Python API** — `emax_predict()`, `emax_nll()`, `sigmoid_emax_predict()`, `sigmoid_emax_nll()`, `idr_simulate()`, `idr_nll()` exposed to Python. Emax and Sigmoid-Emax for dose-response, IDR Types I–IV for indirect response simulation and likelihood. IDR type specified as string (`"inhibit_production"`, `"stimulate_loss"`, etc.).
- **EGARCH/GJR-GARCH Python API** — `egarch11_fit()` and `gjr_garch11_fit()` exposed to Python via `nextstat.volatility.egarch()` and `nextstat.volatility.gjr_garch()`. Full EGARCH(1,1) and GJR-GARCH(1,1) MLE with conditional variance and sigma output.
...
- **CDISC .xpt (SAS Transport v5) reader/writer** — `read_xpt()` / `write_xpt()` for FDA-submission SDTM/ADaM datasets, pure Rust with IBM 370 floating-point conversion. `xpt_to_nonmem()` auto-detects SDTM/ADaM columns (USUBJID, AVAL, ATPTN, etc.) for direct PK modeling.
...
- **FOCE `omega_fixed` parameter** — `nlme_foce(omega_fixed=[False, False, True])` fixes specific random effects to their initial values during estimation (e.g. near-fix Ka inter-individual variability).
- **FOCE `diagonal_omega` parameter** — `nlme_foce(diagonal_omega=True)` forces a diagonal Ω matrix, disabling off-diagonal correlation estimation.
- **3-compartment SAEM** — `nlme_saem(model="3cpt_iv")` and `nlme_saem(model="3cpt_oral")` for 6- and 7-parameter population PK estimation via SAEM. Model dispatch also supports `"2cpt_iv"` and `"2cpt_oral"`.
- **FO / ITS / IMP estimation methods** — `nlme_foce(method="fo"|"its"|"imp")` adds First Order, Iterative Two-Stage, and Importance Sampling MC-EM estimation for all PK models (1/2/3-cpt × IV/oral). IMP uses true IS-EM: MAP + importance sampling from N(η̂, scale²Ω), weighted Ω M-step, ESS/max-weight diagnostics.
- **SAEM covariance step** — `nlme_saem()` now computes the covariance step automatically (R/S matrices, sandwich SE, RSE%, condition number) in the returned result dict.
- **NPDE diagnostics** — `pk_npde()` Python API for simulation-based Normalized Prediction Distribution Errors. Returns per-observation NPDE/PD, summary statistics, Shapiro-Wilk normality test.
- **3-compartment multi-dose superposition** — `DosingRegimen::conc_3cpt_iv()` and `conc_3cpt_oral()` with analytical eigenvalue decomposition for multi-dose 3-compartment PK.
- **SAEM covariate modeling** — `nlme_saem(covariates=[...])` accepts per-subject covariate dicts with power, exponential, proportional, and categorical relationships for all model types.
- **SAEM convergence diagnostics** — `return_theta_trace=True` stores per-iteration θ trace, Geweke z-scores, and relative parameter change in the `saem` sub-dict.
- **Bootstrap NLME** — `bootstrap_nlme()` computes nonparametric bootstrap SE and percentile/BCa confidence intervals for SAEM population PK parameters.
- **Pharma IQ/OQ/PQ auto-runner** — `python tests/pharma_validation/runner.py` auto-executes 84 test cases (5 IQ, 54 OQ, 25 PQ) across all pharma verticals: PK analytical correctness, error models, LLOQ handling, MLE, FOCE, SAEM, GOF, VPC, SCM, NONMEM I/O, bioequivalence, trial simulation, Theophylline reference fits, reproducibility, and timing benchmarks. Integrated into `validation-pack/render_validation_pack.sh` for one-command GxP validation pack generation.
...
- **RTX4000-node CUDA recovery snapshot (2026-02-13)** — archived new single-GPU artifacts at `benchmarks/unbinned/artifacts/2026-02-13/rtx4k_cuda_opt1_20260213T091253Z` and `benchmarks/unbinned/artifacts/2026-02-13/rtx4k_cuda_opt1_scale_20260213T091410Z`. On this branch snapshot, analytical toy fits route to `cuda_gpu_native` and outperform CPU for Gauss+Exp: ~6.3x at 1k toys, ~5.0x at 10k toys, ~4.6x at 50k toys, and ~2.9x on a ~2M-events/toy stress case (50 toys), with 100% convergence in listed runs.
- **RTX4000-node CB/DCB CUDA scale snapshot (2026-02-13)** — added CrystalBall/DoubleCrystalBall 10k-event toy matrix artifacts and summary at `benchmarks/unbinned/artifacts/2026-02-13/rtx4k_cuda_opt1_scale_20260213T091410Z/summary_cb_dcb_scale.json`. Measured 100% convergence for all listed runs (1k and 10k toys): CB CUDA (`cuda_gpu_native`) ~15-17x faster than CPU; DCB CUDA host path already ~28-33x faster than CPU, and explicit `--gpu-native` further improves DCB throughput by ~12% (1k toys) to ~56% (10k toys) vs DCB CUDA host mode.
...
- **PF3.1 apples-to-apples CPU baseline snapshot (4x A40 stand, 2026-02-13)** — added CPU runs for `pf31_gauss_exp_2m` on the same host used for the 4x A40 GPU matrix: `100 toys = 12.11s`, `500 toys = 47.72s`, `1000 toys = 94.10s`, all `n_error=0`. Extrapolated from steady-state (500/1000 toys), CPU throughput is ~`10.55 toys/s` (`~947.6s` for 10k toys), giving measured GPU speedups of ~`4.50x` (`cuda1_device_sh8`), `8.88x` (`cuda2_device_sh8`), and `16.12x` (`cuda4_device_sh16`) for the same workload family.
...
- Added a maintainer-facing external validation kit and report template for the
  stable-first GVM measurement-combination path.
- Added an external-validator outreach pack and invite template for the
  stable-first GVM rollout.
- Added a maintainer-facing tracker template for the first external validation
  wave of the stable-first GVM rollout.
- Added stable-first release PR and launch checklists for the GVM release
  milestone.
