...
#
# Optional env vars:
#   - SL_EXPORTER_PY: Python executable (default: ./.venv/bin/python, else python3)
...
if [[ -n "${SL_EXPORTER_MATURIN:-}" ]]; then
  maturin_bin="${SL_EXPORTER_MATURIN}"
elif [[ -e "./.venv/bin/maturin" && -x "./.venv/bin/maturin" ]] && "./.venv/bin/maturin" --version >/dev/null 2>&1; then
...
if [[ "${skip_maturin}" != "1" ]]; then
  ...
  (cd bindings/ns-cli-py && \
    CARGO_TARGET_DIR="${cargo_target_dir}" "${maturin_bin}" build --release -o "${sl_exporter_wheels}")
  (cd bindings/ns-py && \
    CARGO_TARGET_DIR="${cargo_target_dir}" "${maturin_bin}" build --release -o "${sl_exporter_wheels}")
...
if [[ "${skip_doc_checks}" != "1" ]]; then
  ...
  grep -qF 'export matrix public reinterpretation-style case count: `6`' docs/benchmarks/simplified-likelihood-benchmark-snapshot-2026-03-08.md
...
assert int(summary["export_matrix_case_count"]) >= 8
assert int(summary["export_matrix_public_reinterpretation_style_case_count"]) >= 6
...
assert int(export_summary["public_reinterpretation_style_case_count"]) >= 6
...
assert int(snapshot_report["source_summary"]["export_matrix_case_count"]) >= 8
assert int(snapshot_report["source_summary"]["export_matrix_public_reinterpretation_style_case_count"]) >= 6
...
assert int(public_validation_report["summary"]["public_case_count"]) >= 6
...
assert committed["status"] == "accepted"
assert committed["support_class"] == "stable"
assert committed["stable_evidence_floor"]["min_total_export_matrix_case_count"] == 8
assert committed["stable_evidence_floor"]["min_public_reinterpretation_style_case_count"] == 6
assert committed["maintenance_cadence"]["refresh_cadence"] == "on_every_exporter_release_pr_or_public_case_admission"
assert committed["current_evidence_summary"]["export_matrix_case_count"] >= 8
assert committed["current_evidence_summary"]["public_case_count"] >= 6
print(
    "validated",
    f"status={committed['status']}",
