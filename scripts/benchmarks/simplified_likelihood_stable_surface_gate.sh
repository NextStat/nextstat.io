...
#
# Optional env vars:
#   - SL_STABLE_PY: Python executable (default: ./.venv/bin/python, else python3)
...
if [[ "${skip_maturin}" != "1" ]]; then
  ...
  (cd bindings/ns-cli-py && \
    CARGO_TARGET_DIR="${cargo_target_dir}" "${maturin_bin}" build --release -o "${sl_wheels}")
  (cd bindings/ns-py && \
    CARGO_TARGET_DIR="${cargo_target_dir}" "${maturin_bin}" build --release -o "${sl_wheels}")
