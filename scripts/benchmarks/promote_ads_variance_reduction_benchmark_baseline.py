#!/usr/bin/env python3
"""Promote a vetted ads variance-reduction benchmark artifact to the accepted baseline."""
...
SCHEMA_VERSION = "nextstat.ads_variance_reduction_benchmark_baseline_promotion_report.v1"
...
def main(argv: list[str]) -> int:
    ...
    if _compare_has_review_cases(compare_doc) and compare_status == "review" and not args.allow_review:
        errors.append("review_cases_require_allow_review")
    ...
    previous_copy = history_dir / f"accepted_before_{stamp}.json"
    promoted_copy = history_dir / f"accepted_after_{stamp}.json"
    shutil.copy2(args.accepted, previous_copy)
    actions["archived_previous_baseline"] = True
    actions["archived_previous_baseline_path"] = str(previous_copy)
    shutil.copy2(args.current, args.accepted)
    actions["accepted_updated"] = True
    shutil.copy2(args.accepted, promoted_copy)
    actions["archived_promoted_snapshot"] = True
    actions["archived_promoted_snapshot_path"] = str(promoted_copy)
