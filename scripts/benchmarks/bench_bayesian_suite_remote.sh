...
# Canonical remote runner for the tracked public Bayesian suite on nextstat-bench.
#
# It rsyncs the current working tree snapshot to the remote host, builds the
# local ns-py bindings into an isolated venv, runs the canonical tracked
# Bayesian suite/report/assessment/validation flow, and syncs the resulting
...
BENCH_REMOTE_REPO="${BENCH_REMOTE_REPO:-/tmp/nextstat_bayesian_suite_repo_${STAMP}}"
BENCH_REMOTE_VENV="${BENCH_REMOTE_VENV:-/tmp/nextstat_bayesian_suite_venv_${STAMP}}"
BENCH_REMOTE_TARGET="${BENCH_REMOTE_TARGET:-/tmp/nextstat_bayesian_suite_target_${STAMP}}"
BENCH_REMOTE_OUT="${BENCH_REMOTE_OUT:-/tmp/nextstat_bayesian_suite_${STAMP}_${BENCH_HOST}}"
BENCH_LOCAL_OUT="${BENCH_LOCAL_OUT:-$ROOT_DIR/benchmarks/artifacts/bayesian_stable_surface_validation_${STAMP}/${BENCH_HOST}}"

BENCH_BACKENDS="${BENCH_BACKENDS:-nextstat}"
...
BENCH_PROMOTION_BACKEND="${BENCH_PROMOTION_BACKEND:-nextstat}"
BENCH_PROMOTION_MAX_RHAT="${BENCH_PROMOTION_MAX_RHAT:-1.01}"
BENCH_PROMOTION_MAX_DIVERGENCE_RATE="${BENCH_PROMOTION_MAX_DIVERGENCE_RATE:-0.0}"
BENCH_PROMOTION_MAX_TREEDEPTH_RATE="${BENCH_PROMOTION_MAX_TREEDEPTH_RATE:-0.0}"
BENCH_PROMOTION_MIN_EBFMI="${BENCH_PROMOTION_MIN_EBFMI:-0.3}"
BENCH_PROMOTION_MIN_ESS_BULK="${BENCH_PROMOTION_MIN_ESS_BULK:-0.0}"
BENCH_PROMOTION_MIN_ESS_TAIL="${BENCH_PROMOTION_MIN_ESS_TAIL:-0.0}"
BENCH_PROMOTION_MIN_ESS_BULK_PER_SEC="${BENCH_PROMOTION_MIN_ESS_BULK_PER_SEC:-0.0}"

mkdir -p "${BENCH_LOCAL_OUT}"

echo "[bayesian-suite-remote] host=${REMOTE_SPEC}"
echo "[bayesian-suite-remote] remote_repo=${BENCH_REMOTE_REPO}"
echo "[bayesian-suite-remote] remote_venv=${BENCH_REMOTE_VENV}"
echo "[bayesian-suite-remote] remote_target=${BENCH_REMOTE_TARGET}"
echo "[bayesian-suite-remote] remote_out=${BENCH_REMOTE_OUT}"
echo "[bayesian-suite-remote] local_out=${BENCH_LOCAL_OUT}"
echo "[bayesian-suite-remote] backends=${BENCH_BACKENDS}"
echo "[bayesian-suite-remote] seed=${BENCH_SEED}"
echo "[bayesian-suite-remote] dataset_seed=${BENCH_DATASET_SEED}"

echo "[bayesian-suite-remote] probe remote..."
"${SSH_BASE[@]}" "set -euo pipefail; hostname; uname -a; python3 --version; cargo --version"

echo "[bayesian-suite-remote] create remote directories..."
...
echo "[bayesian-suite-remote] rsync snapshot..."
...
if [[ "${rsync_rc}" -eq 24 ]]; then
  echo "[bayesian-suite-remote] rsync reported vanished source files from unrelated concurrent work; continuing with the synchronized snapshot"
fi

echo "[bayesian-suite-remote] create remote venv..."
...
if [[ -n "$EXTRA_PIP_PACKAGES" ]]; then
  # Extra packages remain additive for non-default backend stacks.
  "$VENV/bin/python" -m pip install $EXTRA_PIP_PACKAGES >/dev/null
fi
EOS

echo "[bayesian-suite-remote] build local nextstat into remote venv..."
...
echo "[bayesian-suite-remote] provision optional backends..."
...
echo "[bayesian-suite-remote] run canonical suite..."
"${SSH_BASE[@]}" bash -s -- \
  "${BENCH_REMOTE_REPO}" \
  "${BENCH_REMOTE_VENV}" \
  "${BENCH_REMOTE_TARGET}" \
  ...
  "${BENCH_DETERMINISTIC}" \
  "${LOCAL_GIT_COMMIT}" \
  "${BENCH_PROMOTION_BACKEND}" \
  "${BENCH_PROMOTION_MAX_RHAT}" \
  "${BENCH_PROMOTION_MAX_DIVERGENCE_RATE}" \
  "${BENCH_PROMOTION_MAX_TREEDEPTH_RATE}" \
  "${BENCH_PROMOTION_MIN_EBFMI}" \
  "${BENCH_PROMOTION_MIN_ESS_BULK}" \
  "${BENCH_PROMOTION_MIN_ESS_TAIL}" \
  "${BENCH_PROMOTION_MIN_ESS_BULK_PER_SEC}" <<'EOS'
set -euo pipefail
REPO="$1"
VENV="$2"
TARGET="$3"
...
DETERMINISTIC="${16}"
GIT_COMMIT="${17}"
PROMOTION_BACKEND="${18}"
PROMOTION_MAX_RHAT="${19}"
PROMOTION_MAX_DIVERGENCE_RATE="${20}"
PROMOTION_MAX_TREEDEPTH_RATE="${21}"
PROMOTION_MIN_EBFMI="${22}"
PROMOTION_MIN_ESS_BULK="${23}"
PROMOTION_MIN_ESS_TAIL="${24}"
PROMOTION_MIN_ESS_BULK_PER_SEC="${25}"
...
if [[ ",$BACKENDS," == *",cmdstanpy,"* ]]; then
  ...
  if [[ -d "$VENDOR_ROOT" ]]; then
    ...
    if [[ -n "$CMDSTAN_DIR" ]]; then
      ...
      echo "[bayesian-suite-remote] cmdstan_vendor=${CMDSTAN_DIR}"
    fi
  fi
fi
...
if [[ ",$BACKENDS," == *",pymc,"* ]]; then
  ...
  echo "[bayesian-suite-remote] pytensor_flags=${PYTENSOR_FLAGS}"
fi
...
"$VENV/bin/python" "suites/bayesian/report.py" --suite "$OUT/bayesian_suite.json" --out "$OUT/bayesian_benchmark_report.md"
"$VENV/bin/python" "suites/bayesian/assess.py" \
  "$OUT" \
  --promotion-backend "$PROMOTION_BACKEND" \
  --promotion-max-rhat "$PROMOTION_MAX_RHAT" \
  --promotion-max-divergence-rate "$PROMOTION_MAX_DIVERGENCE_RATE" \
  --promotion-max-treedepth-rate "$PROMOTION_MAX_TREEDEPTH_RATE" \
  --promotion-min-ebfmi "$PROMOTION_MIN_EBFMI" \
  --promotion-min-ess-bulk "$PROMOTION_MIN_ESS_BULK" \
  --promotion-min-ess-tail "$PROMOTION_MIN_ESS_TAIL" \
  --promotion-min-ess-bulk-per-sec "$PROMOTION_MIN_ESS_BULK_PER_SEC"
"$VENV/bin/python" "scripts/validate_artifacts.py" --strict "$OUT"
EOS

echo "[bayesian-suite-remote] sync artifacts..."
rsync -az --rsh="${RSYNC_RSH_CMD}" "${REMOTE_SPEC}:${BENCH_REMOTE_OUT}/" "${BENCH_LOCAL_OUT}/"

echo "[bayesian-suite-remote] done: ${BENCH_LOCAL_OUT}"
