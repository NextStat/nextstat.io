...
# Always prefer local repo package to avoid stale global wheels.
...
def _nlme_dose_kwarg_name() -> str:
    """Return supported NLME dose kwarg name (requires modern `doses`)."""
    ...
    module_path = getattr(nextstat, "__file__", "unknown")
    raise RuntimeError(
        "Unsupported nextstat NLME API: expected both nlme_foce/nlme_saem to expose "
        "`doses` keyword. "
        f"Detected module={module_path}, nlme_foce={foce_sig}, nlme_saem={saem_sig}. "
        "Rebuild/install current ns-py wheel."
    )


def _assert_nonmem_harness_contract() -> None:
    """Ensure runtime nextstat supports APIs required by this benchmark."""
    if getattr(nextstat, "nlme_foce", None) is None:
        raise RuntimeError("nextstat.nlme_foce is None (extension not built)")
    if getattr(nextstat, "nlme_saem", None) is None:
        raise RuntimeError("nextstat.nlme_saem is None (extension not built)")

    foce_params = inspect.signature(nextstat.nlme_foce).parameters
    if "doses" not in foce_params:
        raise RuntimeError(
            "nextstat.nlme_foce() missing `doses` kwarg; rebuild/install current ns-py wheel."
        )
    ...
    if "doses" not in saem_params:
        raise RuntimeError(
            "nextstat.nlme_saem() missing `doses` kwarg; rebuild/install current ns-py wheel."
        )


def _nlme_dose_kwargs(dose: float | list[float], dose_kw: str) -> dict[str, Any]:
    if isinstance(dose, list):
        doses = [float(d) for d in dose]
    else:
        doses = [float(dose)]
    if dose_kw == "doses":
        return {"doses": doses}
    if dose_kw == "dose":
        if len(doses) != 1:
            raise RuntimeError(
                "Legacy `dose` API does not support per-subject doses; use modern `doses` API."
            )
        return {"dose": float(doses[0])}
...
def main():
    ...
    # ---- Theophylline SAEM ----
    times, y, subject_idx, n_subjects, theo_doses = prepare_theophylline()
    theo_saem, t_theo_saem = run_saem_1cpt(
        times, y, subject_idx, n_subjects, theo_doses, 0.7,
