...
#
# Optional env vars:
#   - GVM_STABLE_PY: Python executable (default: ./.venv/bin/python, else python3)
...
if [[ -n "${GVM_STABLE_MATURIN:-}" ]]; then
  maturin_bin="${GVM_STABLE_MATURIN}"
...
if [[ "${skip_maturin}" != "1" ]] && ! command -v "${maturin_bin}" >/dev/null 2>&1; then
  echo "Missing maturin executable: ${maturin_bin}" >&2
...
if [[ "${skip_maturin}" != "1" ]]; then
  ...
  rm -rf "${wheelhouse}"
  mkdir -p "${wheelhouse}"
  (cd bindings/ns-cli-py && "${maturin_bin}" build --release -o "${wheelhouse}")
  (cd bindings/ns-py && "${maturin_bin}" build --release -o "${wheelhouse}")
