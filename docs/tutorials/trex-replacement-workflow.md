...
Realistic TREx export dirs (with `combination.xml` + ROOT histograms) are committed under `tests/baselines/trex/` (staged export under `histfactory_stage/`) and mirrored as minimal fixture packs under `tests/fixtures/trex_exports/`. Record/update baselines via `tests/record_trex_baseline.py`.
