...
**Important:** All NextStat parity runs should use `--parity` mode (or `nextstat.set_eval_mode("parity")`) to enable Kahan compensated summation, disable Accelerate, and force single-threaded execution. This ensures bit-exact reproducibility. See `docs/references/pyhf-parity-contract.md` for the full 7-tier tolerance hierarchy.
