...
### 3.1 Initial public baseline set (recommended)

For the first public Bayesian snapshots, we recommend a small baseline set that covers distinct NUTS regimes:

1. **Simple HistFactory (~8 parameters)** — fast and inference-like; validates basic ESS/sec + $\hat{R}$ behavior.
...
A larger HEP workspace (e.g. `tHu`, 184 parameters) is valuable, but typically too slow for nightly; keep it behind a slow/release-only flag and publish it as an occasional “smoke at scale”.
...
### 4.3 Diagnostics policy: compute in Rust, validate externally

To keep the seed harness light, NextStat computes core diagnostics in Rust (split rank-normalized folded $\hat{R}$, bulk/tail ESS via Geyer-style estimators, and E‑BFMI) and exports them as JSON.

For cross-framework comparisons, we treat “how diagnostics are computed” as part of the pinned protocol: either everyone uses the same Rust diagnostics library, or we publish a validation pack that recomputes metrics with a standard tool (e.g. ArviZ) and checks parity.
...
- ESS metrics (bulk + tail) + an ESS/sec summary (seed: `min(ESS_bulk) / wall_time`)
