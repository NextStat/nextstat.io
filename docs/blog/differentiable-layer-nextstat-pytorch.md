...
Selected background (non-exhaustive):

1. G. Cowan, K. Cranmer, E. Gross, O. Vitells. *Asymptotic formulae for likelihood-based tests of new physics.* Eur. Phys. J. C 71, 1554 (2011). (Defines discovery-style $q_0$ and its $Z_0=\sqrt{q_0}$ mapping under standard conventions.)
2. L. Heinrich et al. *pyhf: a pure-Python implementation of HistFactory statistical models.* Journal of Open Source Software (JOSS). (Reference for the HistFactory/pyhf likelihood model and modifier semantics.)
3. R. H. Byrd, P. Lu, J. Nocedal, C. Zhu. *A Limited Memory Algorithm for Bound Constrained Optimization.* SIAM Journal on Scientific Computing 16(5), 1190–1208 (1995). (L‑BFGS‑B.)
4. J. M. Danskin. *The theory of max-min, with applications.* SIAM Journal on Applied Mathematics 14(4) (1966/1967). (Value-function differentiation / Danskin’s theorem; envelope-type results.)
5. R. T. Rockafellar, R. J.-B. Wets. *Variational Analysis.* Springer (1998). (General theory underpinning envelope/value-function differentiation and constrained optima.)
