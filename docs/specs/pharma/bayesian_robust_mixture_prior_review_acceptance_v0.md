# Bayesian Robust Mixture Prior Review Acceptance v0

**Date**: 2026-03-08  
**Status**: public acceptance policy  
**Scope**: frozen-report robust-mixture prior policy and review surface

## 1. Purpose

This document defines the acceptance criteria for the published robust-mixture
prior review slice on the Bayesian design surface.

It answers one narrow question:

- what must be true before we treat the current robust-mixture
  policy/review contracts as an accepted public artifact surface?

## 2. Public entrypoints

The accepted Python entrypoints for this slice are:

- `nextstat.bayes_design.build_beta_binomial_robust_mixture_prior_review(report_or_path, policy_or_path) -> dict`
- `nextstat.bayes_design.build_normal_normal_robust_mixture_prior_review(report_or_path, policy_or_path) -> dict`
...
- deterministic mixture decision output:
  - `recommended_mixture_state`
  - `mixture_eligible`
...
- not a robust-mixture posterior update
- not a hidden adaptive-weight engine
...
- the published wrappers accept Python `dict`, JSON string, and filesystem path
  ingress
- the robust-mixture-review path does not call back into analysis, simulation,
  forecast, report, appendix, bundle, or public prior-conflict wrappers
...
- the fact that this slice is policy review and deterministic gating, not a
  robust-mixture posterior engine
...
robust-mixture review or robust-mixture posterior execution, that is a separate
acceptance scope.
...
- automatic component-weight adaptation beyond the published policy/review rules
- report, appendix, or validation-pack embedding of robust-mixture review artifacts
- historical borrowing posterior engines
