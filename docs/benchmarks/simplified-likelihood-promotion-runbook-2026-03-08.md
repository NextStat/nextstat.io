...
The machine-readable bundle verification report is:
...
- Output report: `promotion_evidence_check.json`

The machine-readable persistence/promotion report is:
...
- Output report: `promotion_bundle_promotion_report.json`
...
- verification report: `promotion_evidence_check.json`

Canonical accepted bundle layout after promotion:
...
3. verify the bundle with `--require-promotion-ready`
4. persist the accepted bundle under the stable `benchmarks/artifacts/.../accepted/` path
5. link the Apex2 artifact, `promotion_evidence.json`, `promotion_evidence_check.json`, and `snapshot_index.json` from the release PR / snapshot note
6. state whether the `~10x` speedup claim is still justified
7. note any deviation from the previous accepted artifact
...
python3 scripts/benchmarks/verify_simplified_likelihood_promotion_evidence_bundle.py \
  --bundle-dir tmp/simplified_likelihood_promotion_evidence_bundle_<timestamp>/<host> \
  --require-promotion-ready \
  --deterministic

python3 scripts/benchmarks/promote_simplified_likelihood_promotion_bundle.py \
  --source-bundle-dir tmp/simplified_likelihood_promotion_evidence_bundle_<timestamp>/<host> \
  --deterministic
```

Current first accepted promotion archive:

- accepted dir:
  `benchmarks/artifacts/simplified_likelihood_promotion_bundles/nextstat-bench/accepted/`
- promoted history snapshot:
  `benchmarks/artifacts/simplified_likelihood_promotion_bundles/nextstat-bench/history/accepted_20260308T173340Z_promoted/`
