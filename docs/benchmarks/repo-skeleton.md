...
nextstat-public-benchmarks/
  ...
  manifests/
    schema/
      ...
      bayesian_benchmark_result_v1.schema.json
      bayesian_benchmark_suite_result_v1.schema.json
      ml_benchmark_result_v1.schema.json
      ml_benchmark_suite_result_v1.schema.json
    ...
    snapshots/
      <snapshot_id>/
        ...
        snapshot_index.json
        nextstat_wheel.whl  # optional (self-contained DOI snapshot)
        README_snippet.md
        hep/
          hep_suite.json
          *.json
...
## Bootstrapping a standalone repo from the seed

The directory `benchmarks/nextstat-public-benchmarks/` is designed to be copied into its own repository.
...
rsync -a --delete \
  --exclude 'out/' \
  --exclude 'tmp/' \
  --exclude 'manifests/snapshots/' \
  --exclude '__pycache__/' \
  benchmarks/nextstat-public-benchmarks/ \
  /path/to/nextstat-public-benchmarks/
```

Alternatively (same exclusions, no `rsync` required), use the seed export helper:
...
Tip: GitHub Actions only runs workflows from `.github/workflows/`. The export helper can also
stage `ci/*.yml` into that folder via `--with-github-workflows`.

Then:

- initialize a new git repo in `/path/to/nextstat-public-benchmarks/`
- configure CI variables/inputs for a pinned NextStat wheel (see `ci/` and `docs/benchmarks/publishing.md`)
- publish your first `snapshot-YYYY-MM-DD` artifact set
