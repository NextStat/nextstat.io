...
| `ns-translate` | Format translators: pyhf JSON, HistFactory XML, TRExFitter config, cabinetry YAML, ntuple builder |
...
| `ns-viz-render` | Pure Rust SVG/PDF/PNG renderer for viz artifacts (17 plot types, ATLAS/CMS themes) |
| `ns-cli` | `nextstat` CLI |
...
- WALNUTS: `AdaptiveWalnutsConfig`, `WalnutsConfig`, `WalnutsTuning`, `WalnutsTransition`, `sample_walnuts`, `sample_walnuts_fixed`, `sample_walnuts_multichain`, and `#[cfg(feature = "cuda")] sample_walnuts_multichain_cuda` — Window-Adaptive NUTS with dyadic local step selection. Stable CPU surface supports Euclidean diagonal and dense metrics through `AdaptiveWalnutsConfig.metric_type`; `sample_walnuts_fixed` / `WalnutsTuning` remain the low-level diagonal fixed-tuning path used for deterministic kernel tests. The feature-gated CUDA stable surface is intentionally narrower: models must implement `WalnutsCudaModel`, metric support is currently diagonal-only, and the shipped model coverage is the public Python subset (`LinearRegressionModel`, `LogisticRegressionModel`, `PoissonRegressionModel`, `NegativeBinomialRegressionModel`, `IntervalCensoredWeibullAftModel`). The accepted March 12, 2026 V100 verification closed the first honest shipped CUDA stable surface for that subset. The public result contract matches `Chain` / `SamplerResult`, including `metric_type_name`, `mass_diag`, optional `inv_mass_matrix`, and per-chain `n_leapfrog_warmup_total`. `WalnutsConfig::default()` currently uses `max_step_halvings=4`, `min_micro_steps=1`, and `max_energy_error=2.0`. See `docs/references/walnuts-sampler.md` for the dedicated WALNUTS surface note.
...
Pure Rust visualization renderer — sub-millisecond SVG generation with optional PDF and PNG output. Replaces Python matplotlib for all 17 artifact types.

### Features
- **17 plot types**: pulls, ranking, correlation matrix, profile scan, CLs curve, distributions (stacked + ratio), gammas, separation, summary, uncertainty, significance, contour, pie, yields, unfolding (spectrum + response matrix), morphing, injection
- **4 themes**: `nextstat2026` (default), `atlas`, `cms`, `minimal`
- **YAML configuration**: `VizConfig` with experiment labels, palettes, colors, font sizes, grid, axes
- **Output formats**: SVG (always), PDF (`svg2pdf`), PNG (`resvg` + `tiny-skia`)
- **Embedded fonts**: Inter Regular/Bold/Italic (SIL OFL) via `include_bytes!`

### Public API

```rust
// Render artifact JSON to SVG string
pub fn render_svg(artifact_json: &str, kind: &str, config: &VizConfig) -> Result<String>;

// Render to bytes (svg/pdf/png)
pub fn render_to_bytes(artifact_json: &str, kind: &str, format: &str, config: &VizConfig) -> Result<Vec<u8>>;

// Render to file (format inferred from extension)
pub fn render_to_file(artifact_json: &str, kind: &str, path: &Path, config: &VizConfig) -> Result<()>;

// Config resolution from YAML
pub fn config::resolve_config(user_yaml: Option<&str>) -> Result<VizConfig>;
```

### Dependencies
- `ab_glyph` — font metrics and text measurement
- `resvg` 0.43 + `tiny-skia` — SVG→PNG rasterization (optional, feature `png`)
- `svg2pdf` 0.12 + `usvg` 0.43 — SVG→PDF conversion (optional, feature `pdf`)
