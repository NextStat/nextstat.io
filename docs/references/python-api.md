...
- `nextstat.scm(times, y, subject_idx, n_subjects, covariates, covariate_names, *, dose, bioavailability=1.0, error_model="proportional", sigma=0.1, sigma_add=None, theta_init, omega_init, param_names=None, relationships=None, forward_alpha=0.05, backward_alpha=0.01, max_outer_iter=100, max_inner_iter=20, tol=1e-4) -> ScmResult` — Stepwise Covariate Modeling (1-cpt oral). Forward selection + backward elimination of covariate–parameter relationships using ΔOFV (χ²(1) likelihood ratio test). `covariates` is a list of per-covariate observation vectors (each length `n_obs`); per-subject values extracted from first observation. Auto-centers at median. `relationships`: `"power"` (default), `"proportional"`, `"exponential"`. Returns `selected` (list of `ScmStepResult`), `forward_trace`, `backward_trace`, `base_ofv`, `final_ofv`, `n_forward_steps`, `n_backward_steps`, `theta`, `omega`.
