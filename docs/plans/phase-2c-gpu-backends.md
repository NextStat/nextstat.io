# Фаза II-C: GPU Backends (Metal + CUDA) (P1, optional)
...
**Goal:** Реализовать GPU-ускорение для Mac (Metal) и Linux/Cloud (CUDA) с единым API.
...
**Tech Stack:** Rust, metal-rs, cudarc, wgpu (fallback).
...
|-----------|--------------|---------|
| macOS (Apple Silicon) | Ученые на личных машинах, разработчики | Metal |
...
1. [ ] Metal backend проходит все тесты на macOS
2. [ ] CUDA backend проходит все тесты на Linux + NVIDIA
3. [ ] Backend auto-detection работает
4. [ ] Ranking plot < 30s для 100 NP на GPU
5. [ ] 1000 toy fits < 1 min на GPU
6. [ ] Python API поддерживает backend selection
